...
| Machine enforcement rules | `.ai/policy/docs-ownership.json` |
| Human procedure (this file) | `.ai/rules/40-docs-ownership.md` |
...
- Store durable docs only inside chat transcripts or `.ai/runs/`.
...
Use skill `docs-update` for doc edits. That skill must load this rule and `.ai/policy/docs-ownership.json` and refuse to create files that violate them.
