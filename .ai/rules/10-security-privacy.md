...
- Treat user codebases and workspace content handled by `mivia` as potentially sensitive; minimize retention in `.ai/runs/`.
...
- Agents must not bypass Git hooks (see `.ai/policy/agent-hook-bypass.json`).
...
- Subagent / concurrent work follows `.ai/rules/50-concurrency-subagents.md` (shared MCP, caps, no process farm).
...
- Project `.ai/` rules and skills win over global rules/skills of the same name.
