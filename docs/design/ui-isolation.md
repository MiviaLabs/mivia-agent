...
`internal/tui/view/**` and `internal/tui/kit/**` are self-contained view packages:
...
- They must not import the CLI family (`internal/cli*`), and they must not import
  `internal/tui/adapter`. `internal/tui/adapter` points the other way: it adapts real
  sessions to the ports the UI already speaks.
- The only integration seam for UI components is the ports surface in
  `internal/tui/kit/ports` (`Conversation`, `TurnHandle`, `Approver`,
  `CommandRunner`, `Settings`) plus the event vocabulary in
  `internal/tui/kit/uievent`.
- `internal/tui/kit/**` must not import bubbletea or lipgloss
  (`internal/tui/kit/ports/ports.go`), so the toolkit stays testable without a
  terminal.

## The adapter and the composition root

`internal/tui/adapter` connects real sessions (`internal/chat`, `internal/agent`) to
`internal/tui/kit/ports`. It is the one package that touches both sides. Per invariant
INV-TUI-29 (`.mivia/invariants.md`), `internal/tui/adapter` must never import
`internal/cli*`.

`internal/tui/run` is the composition root: `cmd/mivia` builds the UI there and wires
...
- Build the UI - every screen, component, key, mouse action, and command - with
  full logic and state, driven by the fakes in `internal/tui/kit/replay` and the
...
- A fake that grows a second behaviour (different replies per turn, streaming with
  ...
  integration, and belongs under `internal/tui/kit` with tests of its own.

## The command-dispatch seam (ports.CommandRunner)

Slash commands go through `ports.CommandRunner` (`Run`, `SelectModel`, in
`internal/tui/kit/ports/commandrunner.go`). A `conversation.Screen` calls the
interface when Enter submits a `/command` line; it never inspects harness state
directly (`internal/tui/view/screen/conversation/commands.go`). The test fakes implement
...
- `TestConversation_DoesNotImportCLIFamily` and
  `TestUIPackages_DoNotImportUIAdapter` (`internal/tui/adapter/conversation_test.go`)
  pin the two halves of the boundary: `internal/tui/adapter` never imports the CLI
  family, and `internal/tui/view/...` plus `internal/tui/kit/...` never import
  `internal/tui/adapter`. Both tests implement invariant INV-TUI-29
  (`.mivia/invariants.md`).
