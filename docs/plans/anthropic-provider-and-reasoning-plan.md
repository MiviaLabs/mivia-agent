...
1. **Thinking-block replay — RESOLVED DIFFERENTLY than originally planned, and a NEW open question replaces the old one.** The implementation shipped storing raw signed thinking blocks for byte-perfect replay, per this section's original plan — but that broke the reasoning panel (`internal/ui/component/transcript`), which renders `Response.ReasoningContent` as plain display text everywhere else in the codebase (matching every other provider's contract) and had no way to know this one field secretly carried a JSON envelope with an opaque signature. Fixed: `ReasoningContent` is now always plain extracted thinking text (`anthropicThinkingDisplayText` in `internal/provider/anthropic.go`), and thinking blocks are **not replayed at all** on later turns — `anthropicSystemAndMessages` emits only text/tool_use blocks for a history turn, never a reconstructed (unsigned) thinking block.
   **New open question, still unverified (no live API access from this codebase):** whether Anthropic accepts a continued conversation with NO thinking block on a turn that carried both reasoning AND a tool call, immediately continued by that call's `tool_result` — the common agentic shape, not the plain-text-turn shape this section originally worried about. §3.5's own example shows a thinking block replayed alongside `tool_use` as the normative shape, which this implementation now deliberately does not do. `TestAnthropicReplayOfToolCallTurnWithReasoning` (`internal/provider/anthropic_test.go`) locks in current behavior as a pin, not a correctness claim — if a live check finds Anthropic rejects the omission on this shape (a 400 naming `tool_use` where a `thinking` block was expected), that test is the one to update alongside the fix. Verify against real multi-step tool-calling traffic with reasoning enabled before treating this as settled.
