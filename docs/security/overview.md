...
mivia keeps your secrets private and your project safe. Four rules cover almost everything:
...
An API key is a secret code that lets a program use an AI service. Git is the tool that saves the history of your project files.

## Level 2: more detail

### Secrets

mivia never stores credentials in the settings file. API keys live in the process environment or an env file. An env file is a text file with lines in the form `NAME=value`. Nothing that looks like a secret is compiled into the binary. No credential pattern, key name, or value prefix is built in.
