...
This key is a project decision. A project that omits it leaves every path writable by workflow agents, including `.git`, the live Git hooks, this very blocklist, and the workflow definition the run executes. The recommended starting values are `.git`, `.githooks`, and `scripts/git-hooks`: install_git_hooks.sh points the hooks path at `.githooks`, so the hooks live outside `.git`, and only the three entries together stop a workflow agent from planting a hook that bypasses the host gates. A project that also wants workflow-run protection for its control-surface trees (`.mivia/`, `.agents/`, `.claude/`), its Go module files, or its workflow definitions lists them here too. Both the example set and this repository's own `.mivia/mivia.toml` ship in `.mivia/mivia.toml.example` and `.mivia/mivia.toml`.
