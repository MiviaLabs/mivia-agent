...
func (m StructuralPreparationManager) Prepare(ctx context.Context, input PrepareInput) (Preparation, error) {
	...
	// Manager-level fields act as defaults, mirroring Tools: a non-zero
	// manager value takes precedence over the input field, and the zero-value
	// manager keeps today's pass-through. Without this the documented
	// RecentTail and OutputReserve surface was silently ignored (DC-5).
	recentTail := input.RecentTail
	if m.RecentTail != 0 {
		recentTail = m.RecentTail
	}
	outputReserve := input.OutputReserve
	if m.OutputReserve != 0 {
