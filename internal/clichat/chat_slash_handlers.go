...
func handleSlashAgent(fields []string, sess *chat.Session, res *config.Resolved, term *Terminal, state *AgentSessionState) (bool, bool, error) {
	...
	if state == nil || state.Registry == nil || state.Registry.Len() == 0 {
		// Defensive: the compiled general-purpose agent keeps the registry
		// non-empty in production loads.
		sink.Info("no agents loaded; the built-in general-purpose agent is missing from this session (add .agents/agents/<name>.md)")
