...
func TestPrefixCacheStabilityAcrossToolLoopSteps(t *testing.T) {
	...
	if _, err := loop.Run(context.Background(), "please do the work", Options{Backend: "legacy",
		Model:              "prefix-model",
		MaxSteps:           6,
...
func runAndAssertTurn2(t *testing.T, systemPrompt string, turn1Wires []stepWire, injectedSummary provider.Message) {
	...
	if _, err := loop2.Run(context.Background(), "continue the work", Options{Backend: "legacy",
		Model:              "prefix-model",
		MaxSteps:           6,
		MaxConcurrentTools: 2,
		ToolTimeout:        5 * time.Second,
