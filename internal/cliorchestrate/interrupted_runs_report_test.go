...
// TestInterruptedRunReportSkipsStaleRuns locks the startup noise.
...
// TestInterruptedRunReportIgnoresTerminalRuns - Recover returns every run.
...
// run, so the listing must say how old each one is.
