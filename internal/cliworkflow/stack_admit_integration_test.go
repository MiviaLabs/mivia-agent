...
func TestIntegrationRunInputsAdmitAsStackModeSingle(t *testing.T) {
	inputs, snapshot := delivery.IntegrationRunInputs(map[string]string{"task": "whole feature"}, "master")
...
func TestIntegrationRunInputsStripsReplayedChunkPlan(t *testing.T) {
	inputs, snapshot := delivery.IntegrationRunInputs(map[string]string{
		"task":       "whole feature",
...
func TestClassifyStackPlanRunDeliveryNotApplicableForIntegrationRun(t *testing.T) {
	...
	snap := workflowledger.RunSnapshot{
		...
		InvocationKey: "s1:" + delivery.IntegrationChunkID,
	}
...
func TestClassifyStackPlanRunDeliveryFailedChunk(t *testing.T) {
	...
	if err := SeedStackLedgerFunc(ledger, runID, chunks); err != nil {
		t.Fatal(err)
	}
	if err := ledger.TransitionTask(runID, "c2", delivery.StatusFailed); err != nil {
		t.Fatal(err)
	}
...
func TestStackPlanRunFailureReasonCanceledChunk(t *testing.T) {
	...
	if err := ledger.TransitionTask(runID, "c2", delivery.StatusCanceled); err != nil {
		t.Fatal(err)
	}
