...
func TestWorkflowRunFailSettlesPlanRunFailed(t *testing.T) {
	...
	WorkflowStackDriveToCompletion = func(_ context.Context, _ *PreparedWorkflowRun, ledger *workflowledger.Store, stackID string, _ []delivery.ChunkPlan, _ bool, _ bool, _ string, _ map[string]string, _ bool, _, _ io.Writer) error {
		_ = ledger.TransitionTask(stackID, "c1", delivery.StatusFailed)
