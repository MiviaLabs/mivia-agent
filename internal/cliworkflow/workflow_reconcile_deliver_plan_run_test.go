...
func TestReconcileParkedDeliveryLeavesUndrivenStackParkedWithDeliverPlanRun(t *testing.T) {
	...
	// StackDecomposedChunksFunc (the F10 guard predicate) needs a succeeded
...
// StackDecomposedChunksFunc identifies a multi-chunk plan run, then
...
func TestReconcileParkedDeliveryGuardPassesDrivenStack(t *testing.T) {
	...
	ctx := context.Background()
	// Predicate 1: StackDecomposedChunksFunc recognizes a multi-chunk plan run.
	_, isStack := StackDecomposedChunksFunc(ctx, repo, planRunID)
	if !isStack {
		t.Fatal("StackDecomposedChunksFunc must report a multi-chunk stacking plan run after completeParkedStackDrive")
	}
