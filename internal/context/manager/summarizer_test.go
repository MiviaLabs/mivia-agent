...
func TestSummarizerUsesCapturedProviderAndTimeout(t *testing.T) {
	...
	request.RedactionPolicy = state.RedactionPolicy{Configured: true, Patterns: []string{"never-match"}}
	binding, err := state.NewBindingRevision("provider", "model", 3)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSummarizerDeniesUnconfiguredPolicyWithoutProviderCall(t *testing.T) {
	...
	summarizer, err := NewSummarizer(provider, binding, state.PolicySnapshot{})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := summarizer.Summarize(context.Background(), request); !errors.Is(err, state.ErrSummaryUnavailable) {
...
func TestSummarizerRejectsStaleBinding(t *testing.T) {
	...
	if _, err := summarizer.Summarize(context.Background(), request); !errors.Is(err, state.ErrStaleBinding) {
