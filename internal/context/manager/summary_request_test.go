...
func summaryBuildInputFixture() SummaryBuildInput {
	source := state.SourceID{SessionID: "summary-session", Sequence: 1}
	rangeValue, _ := state.NewSourceRange(source, source)
...
// SummaryRequest.Validate for exceeding state.MaxIdentifierBytes
...
func TestSourceExcerptsTruncatesOversizedToolNameToIdentifierBound(t *testing.T) {
	...
	for _, excerpt := range excerpts {
		...
		found = true
		if len(excerpt.Name) > state.MaxIdentifierBytes {
			t.Fatalf("excerpt.Name is %d bytes, want <= %d", len(excerpt.Name), state.MaxIdentifierBytes)
		}
	}
	if !found {
...
func TestBuildSummaryRequestNegativeCases(t *testing.T) {
	...
	cases := []struct {
		...
		{"bad policy digest", func(in *SummaryBuildInput) { in.PolicyDigest = "short" }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"zero source range", func(in *SummaryBuildInput) { in.SourceRange = state.SourceRange{} }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"inverted source range", func(in *SummaryBuildInput) {
			in.SourceRange.Start.Sequence = in.SourceRange.End.Sequence + 1
		}, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"empty provider", func(in *SummaryBuildInput) { in.Provider = " " }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"empty model", func(in *SummaryBuildInput) { in.Model = "" }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"oversized provider", func(in *SummaryBuildInput) { in.Provider = strings.Repeat("p", state.MaxIdentifierBytes+1) }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"oversized model", func(in *SummaryBuildInput) { in.Model = strings.Repeat("m", state.MaxIdentifierBytes+1) }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"blank endpoint", func(in *SummaryBuildInput) { in.EndpointAllowlist = []string{" "} }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"duplicate endpoint", func(in *SummaryBuildInput) {
			in.EndpointAllowlist = []string{"https://summary.invalid", "https://summary.invalid"}
		}, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"oversized envelope field", func(in *SummaryBuildInput) { in.State = strings.Repeat("x", MaxSummaryFieldBytes+1) }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"duplicate envelope list item", func(in *SummaryBuildInput) { in.Decisions = []string{"same", "same"} }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"control char envelope field", func(in *SummaryBuildInput) { in.Objective = "obj\x01ective" }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"zero budget", func(in *SummaryBuildInput) { in.Budget = 0 }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"zero output limit", func(in *SummaryBuildInput) { in.OutputLimit = 0 }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
		{"oversized output limit", func(in *SummaryBuildInput) { in.OutputLimit = 4096 }, func(err error) bool { return errors.Is(err, state.ErrInvalidDTO) }},
	}
...
func TestBuildSummaryRequestRedactionClassifiedField(t *testing.T) {
	input := summaryBuildInputFixture()
	input.RedactionPolicy = state.RedactionPolicy{
		Configured: true,
	...
	if _, err := BuildSummaryRequest(input); !errors.Is(err, state.ErrInvalidDTO) {
