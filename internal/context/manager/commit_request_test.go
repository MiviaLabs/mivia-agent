...
func TestCommitRangeAllowsEmptyRangeWhenNothingCommitted(t *testing.T) {
	rng := state.SourceRange{
		Start: state.SourceID{SessionID: "session"},
		End:   state.SourceID{SessionID: "session"},
	}
	got, err := commitRange(rng, "session", 0, 0)
	if err != nil {
		t.Fatalf("empty range commit with no events failed: %v", err)
	}
	want := state.SourceRange{
		Start: state.SourceID{SessionID: "session"},
		End:   state.SourceID{SessionID: "session"},
	}
	if got != want {
...
func TestBuildCommitRequestMapsCompleteTurn(t *testing.T) {
	...
	result := TurnResult{
		...
		SourceEvents: []state.SourceEvent{{ID: source, Kind: "message", Role: "user", Provenance: "host", RedactionStatus: "metadata", Size: 8}},
		TurnID:       7, Outcome: OutcomeComplete,
	}
	request, err := BuildCommitRequest(nil, preparation, result, principal, state.Revision{}, binding)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildCommitRequestCarriesWorktreeInstance(t *testing.T) {
	...
	instance := state.WorktreeInstance{Worktree: "feature", ID: "wt_1234567890abcdef"}
	preparation, err := CapturePreparation(
	...
	result := TurnResult{Ordered: []provider.Message{{Role: provider.RoleUser, Content: "question"}}, Active: []provider.Message{{Role: provider.RoleUser, Content: "question"}}, SourceEvents: []state.SourceEvent{{ID: source, Kind: "message", Role: "user", Provenance: "host", RedactionStatus: "metadata", Size: 8}}, TurnID: 1, Outcome: OutcomeComplete}
	request, err := BuildCommitRequest(nil, preparation, result, principal, state.Revision{}, binding)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildCommitRequestRejectsPrincipalAndToolShapeViolations(t *testing.T) {
	...
	binding, _ := state.NewBindingRevision("provider", "model", 1)
	...
	foreign, _ := state.NewPrincipal("other-workspace", "session", "subject")
	if _, err := BuildCommitRequest(nil, preparation, result, foreign, state.Revision{}, binding); !errors.Is(err, state.ErrPrincipalMismatch) {
		t.Fatalf("principal error = %v, want ErrPrincipalMismatch", err)
	}
	if _, err := BuildCommitRequest(nil, preparation, result, principal, state.Revision{}, binding); !errors.Is(err, state.ErrInvalidDTO) {
