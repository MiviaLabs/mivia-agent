...
func TestPlanThresholdAndTarget(t *testing.T) {
	...
	if _, err := Plan(PlanInput{Messages: messages, Budget: before - 1}); !errors.Is(err, state.ErrPromptBudgetExceeded) {
...
func TestPlanRetainsObjectiveToolExchangeAndSourceRange(t *testing.T) {
	...
	eventOne := state.SourceEvent{ID: state.SourceID{SessionID: "session", Sequence: 4}, Kind: "message", Role: "user", Provenance: "host", RedactionStatus: "metadata", Size: 1}
	eventTwo := eventOne
	eventTwo.ID.Sequence = 5
	plan, err := Plan(PlanInput{
		Messages: messages, Budget: 120, SourceEvents: []state.SourceEvent{eventOne, eventTwo}, RecentTail: 2,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanRejectsInvalidToolShapes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if _, err := Plan(PlanInput{Messages: tc.msgs, Budget: 100}); !errors.Is(err, state.ErrInvalidDTO) {
...
func TestPlanRejectsOversizedCurrentObjectiveLocally(t *testing.T) {
	...
	if !errors.Is(err, state.ErrPromptBudgetExceeded) {
...
func TestPlannerFingerprintDistinguishesReasoning(t *testing.T) {
	...
	k1, err := planIdempotencyKey(PlanInput{Budget: 10_000, OutputReserve: 0}, state.SourceRange{}, 5_000, base)
	if err != nil {
		t.Fatal(err)
	}
	k2, err := planIdempotencyKey(PlanInput{Budget: 10_000, OutputReserve: 0}, state.SourceRange{}, 5_000, alt)
	if err != nil {
		t.Fatal(err)
	}
	if k1 == "" || k2 == "" {
	...
	k1b, err := planIdempotencyKey(PlanInput{Budget: 10_000, OutputReserve: 0}, state.SourceRange{}, 5_000, base)
	if err != nil {
		t.Fatal(err)
	}
	if k1b != k1 {
