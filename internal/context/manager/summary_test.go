...
func TestValidateSummaryRejectsSensitiveAndOversizedData(t *testing.T) {
	...
	request.RedactionPolicy = state.RedactionPolicy{
		Configured: true,
	...
	if _, err := ValidateSummary(sensitive, request); !errors.Is(err, state.ErrInvalidDTO) {
		t.Fatalf("sensitive summary error = %v, want ErrInvalidDTO", err)
	}

	duplicate := base
	duplicate.Decisions = []string{"same", "same"}
	if _, err := ValidateSummary(duplicate, request); !errors.Is(err, state.ErrInvalidDTO) {
		t.Fatalf("duplicate summary error = %v, want ErrInvalidDTO", err)
	}

	oversized := base
	oversized.Objective = strings.Repeat("x", MaxSummaryFieldBytes+1)
	if _, err := ValidateSummary(oversized, request); !errors.Is(err, state.ErrInvalidDTO) {
...
func TestValidateSummaryRejectsMismatchedProvenance(t *testing.T) {
	...
	if _, err := ValidateSummary(summary, request); !errors.Is(err, state.ErrInvalidDTO) {
...
func summaryTestRequest(t *testing.T) SummaryRequest {
	...
	rangeValue, err := state.NewSourceRange(source, source)
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidateSummaryRejectsC1ControlCharacters(t *testing.T) {
	...
	if _, err := ValidateSummary(c1Low, request); !errors.Is(err, state.ErrInvalidDTO) {
	...
	if _, err := ValidateSummary(c1High, request); !errors.Is(err, state.ErrInvalidDTO) {
	...
	if _, err := ValidateSummary(c0, request); !errors.Is(err, state.ErrInvalidDTO) {
...
func TestSummaryMetadataEnvelopeRejection(t *testing.T) {
	restore := state.CurrentLimits()
	t.Cleanup(func() { state.SetLimits(restore) })
	state.SetLimits(state.Limits{})

	source := state.SourceID{SessionID: "summary-session", Sequence: 1}
	sourceRange, err := state.NewSourceRange(source, source)
	if err != nil {
		t.Fatal(err)
	}
	digest := strings.Repeat("a", 64)
	...
	} else if !errors.Is(err, state.ErrInvalidDTO) {
		t.Fatalf("oversized envelope error = %v, want ErrInvalidDTO", err)
	}

	state.SetLimits(state.Limits{SummaryMetadataBytes: 64 * 1024})
	if err := oversized.Validate(); err != nil {
...
func TestValidateSummaryAcceptsRealisticFullyPopulatedSummary(t *testing.T) {
	...
	encoded, err := state.MarshalCanonical(summary)
	if err != nil {
		t.Fatal(err)
	}
	if len(encoded) < 2000 {
