...
func summaryExcerptRange(t *testing.T) state.SourceRange {
	t.Helper()
	sourceRange, err := state.NewSourceRange(
		state.SourceID{SessionID: "sess-excerpt", Sequence: 1},
		state.SourceID{SessionID: "sess-excerpt", Sequence: 2})
	if err != nil {
		t.Fatal(err)
	}
	return sourceRange
...
func TestBuildSummaryRequestFiltersFlaggedExcerpts(t *testing.T) {
	request, err := BuildSummaryRequest(SummaryBuildInput{
		...
		RedactionPolicy:   state.RedactionPolicy{Configured: true, Patterns: []string{`token\s*=\s*\S+`}},
...
func TestOmittedEvidenceIsUnique(t *testing.T) {
	...
	if _, err := BuildSummaryRequest(SummaryBuildInput{
		...
		Objective:         "objective",
		Evidence:          evidence,
		SourceRange:       state.SourceRange{Start: state.SourceID{SessionID: "s", Sequence: 1}, End: state.SourceID{SessionID: "s", Sequence: 2}},
		PolicyDigest:      strings.Repeat("a", 64),
		Provider:          "fake",
		Model:             "model",
