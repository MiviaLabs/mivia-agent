...
type PlanInput struct {
	...
	SourceEvents     []state.SourceEvent
...
func invalidPlan(field, reason string) error {
	return fmt.Errorf("%w: planner %s: %s", state.ErrInvalidDTO, field, reason)
}
...
func promptOverflow(after, budget int, objective provider.Message, schemaCost int, ratio float64, profile provider.ContextAccountingProfile) error {
	...
	if objectiveCost > budget {
		return fmt.Errorf("%w: current objective cost %d exceeds budget %d", state.ErrPromptBudgetExceeded, objectiveCost, budget)
	}
	return fmt.Errorf("%w: retained request cost %d exceeds budget %d", state.ErrPromptBudgetExceeded, after, budget)
}
...
func currentObjective(messages []provider.Message, explicit string) (provider.Message, int, error) {
	...
	return provider.Message{}, -1, fmt.Errorf("%w: planner current objective is missing", state.ErrPromptBudgetExceeded)
}
...
func validateMessageShape(messages []provider.Message) error {
	if err := provider.ValidateToolPairing(maskUserMessageNames(messages)); err != nil {
		return fmt.Errorf("%w: message shape: %v", state.ErrInvalidDTO, err)
	}
	return nil
}
