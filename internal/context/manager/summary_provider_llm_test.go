...
func llmSummaryRequestFixture(t *testing.T) SummaryRequest {
	t.Helper()
	start := state.SourceID{SessionID: "session-alpha", Sequence: 4}
	end := state.SourceID{SessionID: "session-alpha", Sequence: 9}
...
func TestLLMSummaryProviderRefusesNilCompleter(t *testing.T) {
	if _, err := NewLLMSummaryProvider(nil, ""); !errors.Is(err, state.ErrSummaryUnavailable) {
		t.Fatalf("nil completer error = %v, want ErrSummaryUnavailable", err)
	}
	var zero LLMSummaryProvider
	if _, err := zero.Summarize(context.Background(), llmSummaryRequestFixture(t)); !errors.Is(err, state.ErrSummaryUnavailable) {
...
func TestLLMSummaryProviderRejectsInvalidRequestBeforeCall(t *testing.T) {
	...
	if _, err := adapter.Summarize(context.Background(), request); !errors.Is(err, state.ErrInvalidDTO) {
...
func TestLLMSummaryProviderRefusesMissingResponse(t *testing.T) {
	...
	if _, err := adapter.Summarize(context.Background(), llmSummaryRequestFixture(t)); !errors.Is(err, state.ErrInvalidDTO) {
