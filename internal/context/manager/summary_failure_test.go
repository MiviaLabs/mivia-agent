...
func buildClassificationCases(t *testing.T, secret string) []classificationTestCase {
	...
	redactionReq.RedactionPolicy = state.RedactionPolicy{
		Configured: true,
	...
	summarizer, err := NewSummarizer(&recordingSummaryProvider{}, binding, state.PolicySnapshot{
	...
	disabledSummarizer, err := NewSummarizer(&recordingSummaryProvider{}, binding, state.PolicySnapshot{
	...
	return []classificationTestCase{
		...
		{"raw invalid dto", state.ErrInvalidDTO, SummaryReasonRequestInvalid, false},
...
func TestSummaryReasonsFitTheCompactionEvent(t *testing.T) {
	...
	source := state.SourceID{SessionID: "session", Sequence: 1}
...
func testSummarizer(t *testing.T, provider SummaryProvider) Summarizer {
	...
	s, err := NewSummarizer(provider, binding, state.PolicySnapshot{
