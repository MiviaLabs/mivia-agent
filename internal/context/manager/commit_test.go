...
type publicationStore struct {
	commits int
	request state.CommitRequest
	err     error
}

func (s *publicationStore) EnsureSession(context.Context, state.EnsureSessionRequest) error {
	return nil
}

func (s *publicationStore) Commit(_ context.Context, request state.CommitRequest) error {
	s.commits++
	s.request = request
	return s.err
}

func (s *publicationStore) Advance(context.Context, state.AdvanceRequest) error { return nil }

func (s *publicationStore) Load(context.Context, state.Principal, string) (state.Snapshot, error) {
...
func publicationFixture(t *testing.T, summaryProvider SummaryProvider) (*publicationStore, Summarizer, SummaryRequest, Preparation, TurnResult) {
	t.Helper()
	principal, err := state.NewPrincipal("workspace", "summary-session", "subject")
	if err != nil {
		t.Fatal(err)
	}
	...
	source := state.SourceID{SessionID: principal.SessionID, Sequence: 1}
	...
	return &publicationStore{}, summarizer, summaryRequest, preparation, TurnResult{
		...
		SourceEvents: []state.SourceEvent{{ID: source, Kind: "message", Role: "user", Provenance: "host", RedactionStatus: "metadata", Size: 1}},
