...
func newCollapseFixture(t *testing.T) *collapseFixture {
	...
	routePrincipal, err := contextstate.NewPrincipal(cli.ContextWorkspaceID(root), "route-session", "local-user")
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationSessionListDisplaysTitlesNotRawIDs(t *testing.T) {
	...
	view := cli.StripANSI(newSessionsSidebar().viewWithActive(m.sessions, 40, 10, true, nil, liveStatusIdle))
