...
func (m *TUIModel) renderChatView() string {
	...
	if live := m.renderLivePanel(cli.Max(1, m.chatPaneWidth()), time.Now()); live != "" {
		base = cli.OverlayAt(base, live, m.livePanelOverlayRect(), cli.Max(1, m.width), cli.Max(8, m.height))
	}
	if m.modelDlg != nil {
		m.modelDlg.busy = m.waiting
		panel, layout := m.modelDlg.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
		return cli.OverlayAt(base, panel, layout.Rect, cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if m.agentDlg != nil {
		m.agentDlg.busy = m.waiting
		panel, layout := m.agentDlg.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
		return cli.OverlayAt(base, panel, layout.Rect, cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if m.effortDlg != nil {
		m.effortDlg.busy = m.waiting
		panel, layout := m.effortDlg.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
		return cli.OverlayAt(base, panel, layout.Rect, cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if m.worktreeDlg != nil {
		panel, layout := m.worktreeDlg.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
		return cli.OverlayAt(base, panel, layout.Rect, cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if m.workflowRunDlg != nil {
		panel, layout := m.workflowRunDlg.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
		return cli.OverlayAt(base, panel, layout.Rect, cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if m.overlay != nil {
		panel, layout := m.overlay.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
		return cli.OverlayAt(base, panel, layout.Rect, cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if panel, size := m.renderComposerPopup(); panel != "" {
		return cli.OverlayAt(base, panel, SuggestOverlayRect(m, panel, size), cli.Max(1, m.width), cli.Max(8, m.height))
	}
	return base
}
...
func (m *TUIModel) composerModelLabel() string {
	...
	model := cli.ShortenModel(sel.Model)
...
func (m *TUIModel) renderBaseChatView() string {
	...
	m.width = width
	padding := PaneSpacer(pane.dividerPadding, cli.Max(1, m.height))
	divider := SidebarDivider(pane.dividerWidth, cli.Max(1, m.height))
	var parts []string
	if pane.sidebarVisible {
		parts = append(parts,
			m.sessionsSidebar.viewWithActive(m.sessions, pane.sidebarWidth, cli.Max(1, m.height), m.focus == cli.FocusSidebar, m.activeSession, m.sidebarLiveStatus()),
	...
	if pane.rightSidebarVisible && m.workflowsSidebar != nil {
		parts = append(parts,
			padding, divider, padding,
			m.workflowsSidebar.view(pane.rightSidebarWidth, cli.Max(1, m.height), m.focus == cli.FocusWorkflowsSidebar),
		)
	}
...
func (m *TUIModel) renderChatPane() string {
	...
	// A manual /compact keeps the status bar busy (spinner, elapsed clock)
	// with no turn running; turnStart was reset when it began.
	busy := m.waiting || m.compacting
	phase := deriveBrandPhase(busy, open, m.streamBuf.Len(), len(m.pendingQueue), false, time.Since(m.turnStart))

	header := renderStatusBar(
		m.logoFrame, phase, busy, time.Since(m.turnStart),
	...
	m.viewport.Width = cli.Max(1, m.width)
	...
	if m.sessionsSidebar != nil || m.workflowsSidebar != nil {
		for i := range outLines {
			if lipgloss.Width(outLines[i]) > m.width {
				outLines[i] = lipgloss.NewStyle().MaxWidth(cli.Max(1, m.width)).Render(outLines[i])
			}
		}
	}
...
func (m *TUIModel) chatViewLayout(header string, phase brandPhase) chatViewLayout {
	...
	termH := cli.Max(8, m.height)
	composerW := cli.Max(18, m.width-2) // leave 1 col left + right for padding
	inputH := cli.Min(composerMaxHeight(termH), cli.Max(1, m.textarea.LineCount()))
	for inputH > 1 {
	...
	remain := cli.Max(minVp, termH-lipgloss.Height(header)-lipgloss.Height(input)-lipgloss.Height(hint)-padRows)
...
func (m *TUIModel) viewWelcome() string {
	...
	composerTop := cli.Max(1, lipgloss.Height(base)-1-lipgloss.Height(input))
	panel, size := RenderSuggestPanel(m.suggest, w, cli.Max(0, composerTop-1))
	if panel == "" {
		return base
	}
	y := cli.Max(1, composerTop-size.H)
	return cli.OverlayAt(base, panel, cli.Rect{X: cli.Max(0, cli.Min(2, w-size.W)), Y: y, W: size.W, H: size.H}, w, h)
}
...
func renderHeroBraille(frame, w int, modelName, workspace string) (block string, lines int) {
	...
	facts = cli.TruncateToWidth(facts, budget)
	slogan := cli.TruncateToWidth(heroSlogan, budget)
...
func renderHeroText(w int) (block string, lines int) {
	...
	slogan := margin + TUIDimStyle.Render(cli.TruncateToWidth(heroSlogan, w-len(margin)))
	return word + "\n" + slogan, 2
}
...
func (m *TUIModel) renderWelcomeBody(w, h int, status, heroBlock string, heroLines int) string {
	...
	inputH := cli.Min(composerMaxHeight(h), cli.Max(1, m.textarea.LineCount()))
	...
	if warningText != "" {
		...
		warningText = cli.TruncateToWidth(warningText, cli.Max(1, w))
	...
	maxRows := cli.Min(12, cli.Max(1, pickerBudget-5))
