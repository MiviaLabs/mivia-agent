...
// while blurred returns focus immediately (cli.RouteFocusKey), so no keystroke
...
func TestScrollAccept_ComposerNoFocusColorWhileWaiting(t *testing.T) {
	...
	if strings.TrimSpace(cli.StripANSI(focused)) == "" {
		t.Fatal("composer rendered empty")
	}
}

// TestToolPanelFocusEnablesExpand locks a dead affordance. focusLiveToolStrip set
// toolPanel.Focused without calling setFocus, so m.focus stayed cli.FocusComposer and
// the expand path - which requires focus != cli.FocusComposer - could never run. The
...
func TestSlashHelpMatchesRealBindings(t *testing.T) {
	...
	joined := cli.StripANSI(strings.Join(dlg.lines, "\n"))
...
func TestRunDashboardRendersHeldRuns(t *testing.T) {
	...
	out := cli.StripANSI(d.renderPanel(100))
	if out == "" {
