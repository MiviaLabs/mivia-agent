...
func TestLogoFramesBrandShape(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(wm), "mivia") {
		t.Fatalf("wordmark strip lost mivia: %q", cli.StripANSI(wm))
	}
}
...
func TestLogoStaticBrandIsOutlineOnly(t *testing.T) {
	...
	rasterDiamond(g, 1, 0)
	want := cli.StripANSI(styleBrailleFrame(g.renderBraille(), 0, "15"))
	if got := cli.StripANSI(logoStaticBrand(0)); got != want {
...
func TestWordmarkFallbackText(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(wm), "mivia") {
		t.Fatalf("strip lost mivia: %q", cli.StripANSI(wm))
	}
}
