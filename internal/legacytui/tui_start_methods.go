...
func (m *TUIModel) startSkillAI(spec SkillSlashSpec) {
	...
	if len(spec.definition.Resources) == 0 || !m.toolsOn || !m.session.AgentTurnEnabled() {
		m.startAIWithDisplay(cli.RenderSkillSlashPrompt(spec.definition.Instructions, spec.args), spec.display)
		return
	}
...
func (m *TUIModel) skillScope() cli.AgentSkillScope {
	if m == nil || m.agentState == nil {
		return cli.AgentSkillScope{}
	}
...
func (m *TUIModel) prepareSkillTurn(spec SkillSlashSpec) (string, *chat.TurnOptions, error) {
	...
	registry, err = cli.InjectSkillResourceTool(registry, activation)
	if err != nil {
		activation.Close()
		return "", nil, err
	}
	...
	return cli.RenderSkillSlashPrompt(activation.Prompt(true), spec.args), &chat.TurnOptions{
		Tools:      registry,
...
func (m *TUIModel) startAIWithPrepared(sent, display string, prepare func() (string, *chat.TurnOptions, error)) {
	...
	if len(m.blocks) > 0 {
		m.appendBlock(cli.ChatBlock{TurnID: uint64(m.session.UserTurns() + 1), Kind: cli.ChatBlockDivider})
	}
	m.appendBlock(cli.ChatBlock{TurnID: uint64(m.session.UserTurns() + 1), Kind: cli.ChatBlockUser, Text: display, SentAt: time.Now()})
	m.layout()
	m.renderVP()
	m.textarea.Reset()
	m.workerWG.Add(1)
	bridgeCB := agentEventBridgeCallback(bridge)
	genToken := cli.SetSubagentProgress(bridgeCB)
	if m.eventBus != nil {
		...
		m.eventBus.Publish(events.Event{Kind: events.KindTurnStart, Timestamp: time.Now(), SessionID: m.session.SessionID, TurnID: turnID, Detail: display, Identity: cli.SessionIdentity(m.session, m.agentState, m.session.CurrentModelGeneration())})
	}
	go func() {
		defer m.workerWG.Done()
		defer cli.ClearSubagentProgress(genToken)
		turnSent := sent
		...
		if ctx.Err() != nil && cli.CancellationCanReplaceTurnError(err) {
