...
func (m *TUIModel) renderFleetBox(width int, now time.Time) string {
	...
	head = cli.TruncateToWidth(head, width-4)
...
func fleetRowLine(r cli.SubagentRun, inner int, now time.Time) string {
	diamond := AgentBadgeStyle.Render(cli.GlyphDiamond)
	status := ""
	...
	if r.Done {
		diamond = ToolOkStyle.Render(cli.GlyphDiamond)
	...
	left := diamond + " " + lipgloss.NewStyle().Bold(true).Render(safeDialogText(r.Name)) + " " +
		TUIDimStyle.Render(cli.TruncateToWidth(activity, cli.Max(8, inner-lipgloss.Width(right)-14)))
...
func (m *TUIModel) openFleetOverlay() bool {
	...
	for i, r := range rows {
		...
		if r.LastTool != "" {
			fmt.Fprintf(&b, "\n  last tool: %s", RedactPreview(cli.SafeChatBlockText(r.LastTool, 0)))
		}
		if r.LastDetail != "" {
			fmt.Fprintf(&b, "\n  %s", RedactPreview(cli.SafeChatBlockText(r.LastDetail, 0)))
		}
		if !r.Started.IsZero() {
