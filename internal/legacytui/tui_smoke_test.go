...
func assertSmokeView(t *testing.T, m *TUIModel) {
	...
	if plain := cli.StripANSI(view); !strings.Contains(plain, "main.go") {
...
func TestTUISmoke_HelpCommand(t *testing.T) {
	...
	// Simulate user typing /help and cli.HandleSlash being called.
	ok := m.handleSlash("/help")
	if !ok {
		t.Fatal("cli.HandleSlash returned false for /help")
	}
...
func TestTUISmoke_StreamDrainEvents(t *testing.T) {
	...
	m.waiting = true
	m.turnStart = time.Now()
	m.appendBlock(cli.ChatBlock{TurnID: 1, Kind: cli.ChatBlockUser, Text: "list files"})
...
func seedBridgeToolsAndStream(b *cli.StreamBridge) {
...
func drainAndAssertLive(t *testing.T, b *cli.StreamBridge) (stream string, tools []cli.BridgeToolEvt) {
	t.Helper()
	d := b.Drain()
	if d.Stream == "" {
...
func TestTUISmoke_ViewRenderAtVariousHeights(t *testing.T) {
	for _, h := range []int{10, 24, 40} {
		...
		m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "hello", TurnID: 1})
		m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockAssistant, Text: "world", TurnID: 1})
		m.renderVP()
