...
func TestIntegration_LiveToolStatus_KNProgress(t *testing.T) {
	...
	m.updateFromDrain(cli.BridgeDrain{StepDetail: "tools 1/2 done · 2s", StepDetailAt: time.Now()})
...
func TestIntegration_LiveToolStatus_HeartbeatOnly(t *testing.T) {
	...
	m.updateFromDrain(cli.BridgeDrain{StepDetail: "tools 0/3 done · 4s", StepDetailAt: time.Now()})
	m.refreshLiveToolWaveStatus()
...
func lastWorkStatus(blocks []cli.ChatBlock) *cli.ChatBlock {
	for i := len(blocks) - 1; i >= 0; i-- {
		if cli.IsWorkStatusBlock(blocks[i]) {
			return &blocks[i]
		}
	}
	return nil
...
func TestFormatLiveToolWaveSummary(t *testing.T) {
	t.Parallel()
	s := cli.FormatLiveToolWaveSummary(2, 0, 2, 3*time.Second)
	if !strings.Contains(s, "0/2") || !strings.Contains(s, "Running 2") {
		t.Fatalf("%q", s)
	}
	s = cli.FormatLiveToolWaveSummary(0, 2, 2, time.Second)
