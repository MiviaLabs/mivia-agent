...
func TestOnEventForMultiStepPreservesOrigin(t *testing.T) {
	...
	var got []agent.Event
	fwd := cli.OnEventForMultiStep(func(e agent.Event) { got = append(got, e) })
...
func TestOnEventForMultiStepFeedsStepHeartbeatRegistry(t *testing.T) {
	...
	fwd := cli.OnEventForMultiStep(func(agent.Event) {})
...
func TestToolRowsCarryAgentFromDrain(t *testing.T) {
	...
	m.applyToolEventsOpts([]cli.BridgeToolEvt{
...
func TestToolBlockHistoryKeepsAgent(t *testing.T) {
	...
	block := cli.ChatBlock{Kind: cli.ChatBlockTool, ToolName: "grep", Text: "3 hits", AgentName: "audit", Collapsed: true}
	lines := cli.RenderOneChatBlock(block, "m", 80, false)
