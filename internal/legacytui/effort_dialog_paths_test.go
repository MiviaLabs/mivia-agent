...
func TestEmptyEffortDialogSurvivesNavigation(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(view), "reasoning_efforts") {
		t.Fatalf("empty footer must point at the config key:\n%s", cli.StripANSI(view))
	}
}
...
func TestEffortDialogRefusesWhileBusy(t *testing.T) {
	...
	view, _ := m.effortDlg.ViewAt(90, 24)
	if !strings.Contains(cli.StripANSI(view), "finish current work") {
...
func TestEffortDialogNoticeDoesNotOutliveTheBlockingState(t *testing.T) {
	...
	if got := cli.StripANSI(m.effortDlg.footer()); strings.Contains(got, "finish current work") {
...
func TestEffortDialogBusyFooter(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(d.footer()), "finish current work") {
		t.Fatalf("busy footer = %q", cli.StripANSI(d.footer()))
	}
}
...
// is where a modal that forgot to register would blow past its cli.Rect.
func TestEffortDialogRendersInTheChatViewAndClamps(t *testing.T) {
	m := effortTUI(t, effortThinker)
	m.width, m.height = 80, 20
	m.handleSlash("/effort")
	if !strings.Contains(cli.StripANSI(m.renderChatView()), "reasoning effort") {
...
func TestEffortSlashRejectsAnUnparseableLevel(t *testing.T) {
	...
	out := &strings.Builder{}
	if _, _, err := cli.HandleSlash("/effort turbo", sess, res, false, cli.NewTestTerminal(out)); err != nil {
		t.Fatal(err)
	}
...
func TestEffortDialogAddsNoUnsetRowWhenTheModelHasADefault(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(mustEffortView(t, m)), "high (default)") {
		t.Fatalf("configured default is not labelled:\n%s", cli.StripANSI(mustEffortView(t, m)))
	}
}
...
func TestEffortSlashArgumentClearsTheOverride(t *testing.T) {
	...
	out := &strings.Builder{}
	if _, _, err := cli.HandleSlash("/effort unset", sess, res, false, cli.NewTestTerminal(out)); err != nil {
		t.Fatal(err)
	}
	if got := sess.ReasoningEffort(); got != reasoning.High {
...
func TestEffortSlashArgumentRefusesWhileBusy(t *testing.T) {
	...
	last := ""
	if len(m.blocks) > 0 {
		last = cli.StripANSI(m.blocks[len(m.blocks)-1].Text)
	}
...
func TestStatusReportsTheReasoningDial(t *testing.T) {
	...
	text := cli.StripANSI(strings.Join(m.newStatusDialog().lines, "\n"))
	...
	out := &strings.Builder{}
	if _, _, err := cli.HandleSlash("/status", sess, res, false, cli.NewTestTerminal(out)); err != nil {
		t.Fatal(err)
	}
	plain := cli.StripANSI(out.String())
