...
func (m *TUIModel) applyToolEvents(evts []cli.BridgeToolEvt) {
...
func (m *TUIModel) applyToolEventsOpts(evts []cli.BridgeToolEvt, emptySpeechStatus bool) {
	openBefore := 0
	...
	for _, e := range evts {
		if e.Start {
			...
			if e.Name != "" && !IsLifecycleStatus(e.Detail) {
				m.stepDetail = cli.ToolStatusLine(e.Name, e.Detail)
...
func (m *TUIModel) appendEmptySpeechToolStatus(evts []cli.BridgeToolEvt) {
	detail := cli.ToolBatchStatusDetail(evts)
	if detail == "" {
		return
	}
	// Seed live k/n counters for this wave (completed tools leave toolRows).
	if n := len(cli.RealToolStarts(evts)); n > 0 {
		m.toolWaveTotal = n
		m.toolWaveDone = 0
	}
	summary := detail
...
func (m *TUIModel) applyToolStartEvent(e cli.BridgeToolEvt) {
...
func (m *TUIModel) applyToolEndEvent(e cli.BridgeToolEvt) int {
	for i := len(m.toolRows) - 1; i >= 0; i-- {
		...
		body := e.Detail
		failed := cli.ToolResultFailed(body) ||
		...
		if m.toolWaveTotal > 0 && !cli.IsBannerTool(m.toolRows[i].Name) {
			m.toolWaveDone++
		}
		return i
	}
...
func (m *TUIModel) appendOneToolBlock(r ToolRow) {
	...
	if !r.Start.IsZero() {
		line += " " + ToolTimeStyle.Render("· "+FormatDuration(toolRowElapsed(r, time.Now())))
	}
	rawContent := r.Detail
	if r.Result != "" {
		if rawContent != "" {
	...
	m.appendBlock(cli.ChatBlock{
		...
		Collapsed:  true,
		Failed:     r.Failed,
		Elapsed:    toolRowElapsed(r, time.Now()),
	})
}
