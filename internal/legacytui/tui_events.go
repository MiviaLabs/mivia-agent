...
func (m *TUIModel) publishTurnEnd(turnID string, err error) {
	...
	m.eventBus.Publish(events.Event{
		...
		Detail:    detail,
		Err:       err,
		Identity:  cli.SessionIdentity(m.session, m.agentState, m.session.CurrentModelGeneration()),
	})
}
...
func agentEventBridgeCallback(bridge *cli.StreamBridge) func(agent.Event) {
...
// EventPreview is relocated to internal/cli (needed there by the classic-mode
// UI and the JSON event writer); aliased here so this package's own call
// sites are unchanged.
var EventPreview = cli.EventPreview
