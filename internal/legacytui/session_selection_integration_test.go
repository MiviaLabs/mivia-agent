...
func persistedSessionForSelection(t *testing.T) (*chat.Session, *config.Resolved, []chat.SessionInfo, func()) {
	...
	sess.UseTools = true
	memClose, err := cli.ConfigureChatWorkspace(sess, root, true, res, nil, false, false, false)
	if err != nil {
		t.Fatalf("configure workspace: %v", err)
	}
	sess.SetBindingFactory(func(providerName, model string) (chat.ModelBinding, error) {
		return cli.BuildModelBinding(sess, res, root, providerName, model, &cli.AgentSessionState{AllowProjectSkills: true})
	})
	cleanup, err := cli.AttachSessionDispatcher(sess, root, res.Model, res.Subagents, &cli.AgentSessionState{AllowProjectSkills: true}, nil, cli.SessionRouting{})
	if err != nil {
...
func TestIntegrationSplashEnterLoadsPersistedSession(t *testing.T) {
	...
	if m.mode != modeChat {
		t.Fatalf("splash Enter left mode=%v, want chat; view=%q", m.mode, cli.StripANSI(m.View()))
	}
...
func TestIntegrationSessionsSidebarEnterLoadsPersistedSession(t *testing.T) {
	...
	if m.sessionsSidebar == nil {
		t.Fatalf("Enter unexpectedly closed the sessions sidebar; view=%q", cli.StripANSI(m.View()))
	}
	if got := sess.MessagesCopy(); len(got) != 2 || got[0].Content != "previous question" {
		t.Fatalf("dialog Enter did not restore persisted history: %#v; view=%q", got, cli.StripANSI(m.View()))
	}
	if got := strings.Count(cli.StripANSI(m.View()), "current"); got != 1 {
		t.Fatalf("loaded session current marker count = %d, want 1; view=%q", got, cli.StripANSI(m.View()))
	}
	...
	m.Update(tea.KeyMsg{Type: tea.KeyEnter})
	if m.sessionsSidebar == nil {
		t.Fatalf("second Enter unexpectedly closed the sessions sidebar; view=%q", cli.StripANSI(m.View()))
	}
}
...
func TestTUISlashLoadMarksCurrentSidebarSession(t *testing.T) {
	...
	if got := strings.Count(cli.StripANSI(m.View()), "current"); got != 1 {
		t.Fatalf("slash-loaded session current marker count = %d, want 1; view=%q", got, cli.StripANSI(m.View()))
	}
}
...
func TestIntegrationSessionsSidebarNewSessionStartsFreshConversation(t *testing.T) {
	...
	if len(m.blocks) == 0 || !strings.Contains(cli.StripANSI(m.blocks[len(m.blocks)-1].Text), "new session started") {
...
func TestIntegrationSessionsSidebarNewSessionBlocksWhileBusy(t *testing.T) {
	...
	if len(m.blocks) == 0 || !strings.Contains(cli.StripANSI(m.blocks[len(m.blocks)-1].Text), "finish the current turn") {
...
func TestIntegrationSplashEnterSurfacesLoadFailure(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(m.View()), "open failed") {
		t.Fatalf("splash load failure was not visible: %q", cli.StripANSI(m.View()))
	}
}
...
func TestIntegrationSessionsSlashTogglesFocusedSidebar(t *testing.T) {
	...
	if m.focus != cli.FocusSidebar {
		t.Fatalf("focus = %v, want %v", m.focus, cli.FocusSidebar)
	}
	if !m.handleSlash("/sessions") {
