...
func TestLivePanelBandFormula(t *testing.T) {
	...
	for _, h := range []int{8, 11, 12, 20, 24, 30, 34, 40, 44, 60} {
		band := cli.Min(livePanelMaxHeight, cli.Max(1, cli.Max(8, h)/3))
		...
		panel = cli.StripANSI(m2.renderLivePanel(90, time.Now()))
		if got := strings.Count(panel, "\n") + 1; got != band {
...
func TestLivePanelHeightConstantDuringTurn(t *testing.T) {
	...
	m.waiting = true
	m.turnStart = time.Now()
	band := cli.Min(livePanelMaxHeight, cli.Max(1, cli.Max(8, m.height)/3))
	if got := m.livePanelHeight(); got != band {
...
func TestTranscriptHeightStableDuringTurn(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{
		{ID: "b1", Kind: cli.ChatBlockUser, Text: "do the thing"},
	}
	m.waiting = true
	m.turnStart = time.Now()
	m.renderVP()
