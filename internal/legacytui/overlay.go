...
func detailDialogPrefs() cli.DialogPrefs {
	return cli.DialogPrefs{PreferredWPct: 90, PreferredHPct: 85, MinW: 40, MinH: 8, FrameCols: 4, FrameRows: 2, Pager: true}
}

func safeDialogText(text string) string {
	text = RedactPreview(cli.SafeChatBlockText(text, 0))
...
func newBlockOverlay(block cli.ChatBlock) *blockOverlay {
	toolName := safeDialogText(block.ToolName)
	title := ToolIconForName(toolName) + " " + toolName
	if block.Kind == cli.ChatBlockThinking {
		title = "▾ thinking"
	}
	if block.AgentName != "" {
	...
	case block.Failed:
		title += "  ✗"
	case block.Kind == cli.ChatBlockTool && block.Elapsed > 0:
		title += "  ✓"
	}
	content := RedactPreview(cli.SafeChatBlockText(block.Text, 0))
	return &blockOverlay{title: title, lines: strings.Split(content, "\n"), prefs: detailDialogPrefs(), kind: "detail"}
}

func (o *blockOverlay) layout(w, h int) cli.DialogLayout {
	layout := cli.MakeDialogLayout(w, h, o.prefs, func(innerW int) (int, int) {
		rows := cli.WrapDisplayRows(o.lines, innerW)
		maxW := 0
...
func (o *blockOverlay) rowsForLayout(innerW, pageH int) []string {
	...
	rows := cli.WrapDisplayRows(compact, innerW)
	if len(rows) > pageH {
		...
		for _, row := range compact {
			if plain := strings.TrimSpace(cli.StripANSI(row)); plain != "" {
...
func packStatusFacts(facts []string, innerW, pageH int) []string {
	innerW = cli.Max(1, innerW)
	pageH = cli.Max(1, pageH)
	...
	for _, fact := range facts {
		...
		compact = append(compact, fields[0]+"="+ansi.Truncate(value, cli.Max(4, innerW/2), "…"))
	}
...
func narrowStatusRows(facts []string, innerW, pageH int) []string {
	...
	short := func(key, alias string) string {
		...
		return alias + ansi.Truncate(value, cli.Max(1, innerW-len(alias)), "…")
	}
	if pageH == 3 {
		rows := []string{
...
func (o *blockOverlay) clamp(pageH int) {
	...
	rows := o.renderedRows
	if len(rows) == 0 {
		rows = o.displayRows(cli.Max(1, o.lastInnerW))
	}
	maxOffset := len(rows) - pageH
	if maxOffset < 0 {
		maxOffset = 0
	}
	o.yOffset = cli.Min(cli.Max(0, o.yOffset), maxOffset)
}
...
func (o *blockOverlay) ViewAt(w, h int) (string, cli.DialogLayout) {
	layout := o.layout(w, h)
	previousSource := -1
	if o.lastInnerW > 0 && o.lastInnerW != layout.InnerW && o.yOffset < len(o.renderedSources) {
		previousSource = o.renderedSources[o.yOffset]
	}
	rows := o.rowsForLayout(cli.Max(1, layout.InnerW), layout.PageH)
	o.renderedRows = rows
	_, o.renderedSources = cli.WrapDisplayRowsWithSources(o.lines, cli.Max(1, layout.InnerW))
	if previousSource >= 0 {
	...
	o.lastInnerW = cli.Max(1, layout.InnerW)
	o.clamp(layout.PageH)
	start := cli.Min(o.yOffset, len(rows))
	end := cli.Min(len(rows), start+layout.PageH)
	visible := rows[start:end]
	pos := "all"
	if len(rows) > layout.PageH {
		pct := 100 * (o.yOffset + layout.PageH) / len(rows)
		pos = fmt.Sprintf("%d%%", cli.Min(100, pct))
	}
	return cli.RenderDialogFrame(o.title, visible, dialogFooter(pos, len(rows), o.prefs.Pager), layout), layout
}
...
func (o *blockOverlay) View(w, h int) string {
	view, _ := o.ViewAt(cli.Max(1, w), cli.Max(1, h))
	return view
}
