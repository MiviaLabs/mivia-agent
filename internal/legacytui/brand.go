...
const (
	...
	// BrandColorThinking is relocated to internal/cli (needed there by the
	// rail-role resolver); aliased here so this package's own call sites are
	// unchanged.
	BrandColorThinking = cli.BrandColorThinking // vivid cyan    #00d7d7 (thinking ramp)
...
//
// Relocated to internal/cli (needed there by the rail-role resolver);
// aliased here so this package's own call sites are unchanged.
var BrandWorkFrames = cli.BrandWorkFrames
...
func renderWorkChrome(
	...
	if detail != "" && width > 0 {
		...
		if available < 1 {
			...
			detail = cli.TruncateToWidth(detail, available)
		}
	}
	rightParts := core
	if detail != "" {
