...
func TestCopyBlockTextIsPlain(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{
		{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "the **answer** is 42"},
	}
	m.renderVP()
...
func TestCtrlCCopiesOnlyWhenIdleWithSelection(t *testing.T) {
	...
	m2 := newReadyChatModel(30, 80)
	m2.blocks = []cli.ChatBlock{{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "x"}}
	m2.renderVP()
	m2.focus = cli.FocusScrollback
...
func TestRightClickCopiesBlock(t *testing.T) {
	withWorkingClipboard(t)
	m := newReadyChatModel(30, 80)
	m.blocks = []cli.ChatBlock{{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "right click me"}}
	m.layout()
	m.renderVP()
	m.View()
