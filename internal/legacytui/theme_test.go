...
func TestThemeANSIVocab(t *testing.T) {
	...
	if !strings.Contains(cli.AnsiBgDiffAdd, themeColorDiffAddBg) {
		t.Fatalf("cli.AnsiBgDiffAdd %q should embed %s", cli.AnsiBgDiffAdd, themeColorDiffAddBg)
	}
	if !strings.Contains(cli.AnsiBgDiffDel, themeColorDiffDelBg) {
		t.Fatalf("cli.AnsiBgDiffDel %q should embed %s", cli.AnsiBgDiffDel, themeColorDiffDelBg)
	}
}
...
func TestThemeByteStabilityMarkdown(t *testing.T) {
	...
	if !strings.HasPrefix(got, want) {
		t.Fatalf("markdown prefix drift\ngot  %q\nwant %q", got[:cli.Min(len(got), len(want)+20)], want)
	}
...
// TestThemeByteStabilityHighlight locks highlightLine / cli.HighlightCodeBlock bytes.
func TestThemeByteStabilityHighlight(t *testing.T) {
	goCode := "func main() {\n\tvar x int = 42\n}\n"
	got := cli.HighlightCodeBlock("go", goCode)
	...
	diff := "--- a/x\n+++ b/x\n@@ -1 +1 @@\n-old\n+new\n context\n"
	gotDiff := cli.HighlightCodeBlock("diff", diff)
	...
	if !strings.Contains(gotDiff, cli.AnsiBgDiffDel+AnsiRed+"-old") {
		t.Fatalf("diff del missing tokens: %q", gotDiff)
	}
	if !strings.Contains(gotDiff, cli.AnsiBgDiffAdd+AnsiGreen+"+new") {
...
func TestThemeByteStabilityToolAndUserStyles(t *testing.T) {
	...
	// Full user-card path through cli.FormatUserMessageCard.
	lines := cli.FormatUserMessageCard("hello", 40, time.Date(2026, 1, 15, 15, 4, 0, 0, time.UTC))
	if len(lines) < 2 {
		t.Fatalf("expected ≥2 card lines, got %d", len(lines))
	}
	joined := strings.Join(lines, "\n")
	if !strings.Contains(joined, "hello") || !strings.Contains(cli.StripANSI(joined), "you") {
