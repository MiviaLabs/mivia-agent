...
func TestTableHasHeaderRuleAndBorders(t *testing.T) {
	out := cli.StripANSI(cli.RenderMarkdown("| Col A | Col B |\n|---|---|\n| one | two |", 60))
...
func TestTableWithoutOuterPipesRenders(t *testing.T) {
	...
	src := "Col A | Col B\n--- | ---\none | two"
	out := cli.StripANSI(cli.RenderMarkdown(src, 60))
...
func TestTableSingleLineNotTreatedAsTable(t *testing.T) {
	...
	out := cli.StripANSI(cli.RenderMarkdown("value a | value b", 60))
	if strings.Contains(out, "┌") {
...
func TestTableNeverExceedsWidth(t *testing.T) {
	...
	for _, w := range []int{40, 60, 100} {
		out := cli.StripANSI(cli.RenderMarkdown(src, w))
		for _, line := range strings.Split(out, "\n") {
			if cli.VisibleWidth(line) > w {
				t.Fatalf("width=%d: line %d wide: %q", w, cli.VisibleWidth(line), line)
			}
		}
	}
}
...
func TestEditToolBlockShowsDiffStat(t *testing.T) {
	block := cli.ChatBlock{
		Kind: cli.ChatBlockTool, ToolName: "search_replace", Collapsed: true,
		Text:    "updated internal/cli/x.go (1 replacement, +3 −1)\n--- a/internal/cli/x.go\n+++ b/internal/cli/x.go\n-old\n+new\n+more\n+lines",
		Elapsed: 200 * time.Millisecond,
	}
	out := cli.StripANSI(strings.Join(cli.RenderOneChatBlock(block, "m", 80, false), "\n"))
...
func TestDiffBodyGuttersAndTruncationIsExplicit(t *testing.T) {
	...
	lines := cli.RenderDiffBody(b.String(), 80, 20)
	if len(lines) > 22 {
		t.Fatalf("diff body ignored its cap: %d lines", len(lines))
	}
	joined := cli.StripANSI(strings.Join(lines, "\n"))
...
func manyToolBlocks(n int) []cli.ChatBlock {
	var blocks []cli.ChatBlock
	for i := 0; i < n; i++ {
		blocks = append(blocks, cli.ChatBlock{
			ID: fmt.Sprintf("b%d", i), Kind: cli.ChatBlockTool,
...
func TestExpandedWorkGroupIsBoundedWindow(t *testing.T) {
	...
	groups := cli.FindWorkGroups(blocks)
	if len(groups) != 1 {
		t.Fatalf("groups=%d", len(groups))
	}
	key := groups[0].Key
	out := cli.RenderChatBlocksWithWorkGroupsWindow(blocks, "m", 90, false,
		map[string]bool{key: false}, map[string]int{}, cli.RailView{})
	plain := cli.StripANSI(strings.Join(out.Lines, "\n"))
	if got := strings.Count(plain, "read_file"); got > cli.WorkGroupWindowRows {
		t.Fatalf("window shows %d rows, cap is %d", got, cli.WorkGroupWindowRows)
	}
...
func TestWorkGroupWindowScrolls(t *testing.T) {
	blocks := manyToolBlocks(120)
	key := cli.FindWorkGroups(blocks)[0].Key
	scroll := map[string]int{key: 40}
	out := cli.RenderChatBlocksWithWorkGroupsWindow(blocks, "m", 90, false,
		map[string]bool{key: false}, scroll, cli.RailView{})
	plain := cli.StripANSI(strings.Join(out.Lines, "\n"))
	if strings.Contains(plain, "file-000.go") {
...
func TestTableHasRowSeparators(t *testing.T) {
	out := cli.StripANSI(cli.RenderMarkdown("| A | B |\n|---|---|\n| one | two |\n| three | four |", 60))
	lines := strings.Split(out, "\n")
	rules := 0
	for _, l := range lines {
...
func TestTableCellsWrapInsteadOfTruncating(t *testing.T) {
	...
	src := "| Key | Notes |\n|---|---|\n| k | " + long + " |"
	out := cli.StripANSI(cli.RenderMarkdown(src, 50))
	if strings.Contains(out, "…") {
...
func TestTableWrapKeepsStyledCellsAligned(t *testing.T) {
	...
	out := cli.RenderMarkdown(src, 46)
	var w int
	for _, line := range strings.Split(cli.StripANSI(out), "\n") {
		if w == 0 {
			w = cli.VisibleWidth(line)
		}
		if cli.VisibleWidth(line) != w {
			t.Fatalf("ragged table edges (%d vs %d):\n%s", cli.VisibleWidth(line), w, cli.StripANSI(out))
		}
	}
}
...
func TestEmptyTurnDividerRendersNothing(t *testing.T) {
	...
	lines := cli.RenderOneChatBlock(cli.ChatBlock{Kind: cli.ChatBlockDivider}, "m", 60, false)
	for _, l := range lines {
		if strings.Contains(cli.StripANSI(l), "·") {
			t.Fatalf("empty divider still renders a rule: %q", cli.StripANSI(l))
		}
	}
	// A divider WITH text (the turn footer) still renders.
	lines = cli.RenderOneChatBlock(cli.ChatBlock{Kind: cli.ChatBlockDivider, Text: "  ─ turn 3 · 4.0s ─"}, "m", 60, false)
	if len(lines) == 0 || !strings.Contains(cli.StripANSI(strings.Join(lines, "")), "turn 3") {
...
func TestAssistantProseHasNoRail(t *testing.T) {
	lines := cli.RenderOneChatBlock(
		cli.ChatBlock{Kind: cli.ChatBlockAssistant, Text: "the answer\n\nsecond paragraph"}, "m", 60, false)
	for _, l := range lines {
		plain := strings.TrimRight(cli.StripANSI(l), " ")
...
func TestMarkedKindsKeepTheirRails(t *testing.T) {
	...
	user := cli.StripANSI(strings.Join(
		cli.RenderOneChatBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "hi"}, "m", 60, false), "\n"))
	if !strings.Contains(user, "▌") {
		t.Fatalf("user rail dropped: %q", user)
	}
	blocks := []cli.ChatBlock{
		{ID: "t1", Kind: cli.ChatBlockTool, ToolName: "read_file", Text: "a", Collapsed: true},
		{ID: "t2", Kind: cli.ChatBlockTool, ToolName: "grep", Text: "b", Collapsed: true, Failed: true},
	}
	work := cli.StripANSI(strings.Join(
		cli.RenderChatBlocksWithWorkGroups(blocks, "m", 60, false, map[string]bool{}).Lines, "\n"))
