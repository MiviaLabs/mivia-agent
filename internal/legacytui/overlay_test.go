...
func overlayTestBlock(lines int) cli.ChatBlock {
	var b strings.Builder
	for i := 0; i < lines; i++ {
		fmt.Fprintf(&b, "line-%03d\n", i)
	}
	return cli.ChatBlock{
		ID: "turn-1-block-2", Kind: cli.ChatBlockTool, ToolName: "run_command",
...
func TestOverlayShowsFullContentBeyondInlineCap(t *testing.T) {
	...
	view := cli.StripANSI(o.View(80, 24))
...
func TestOverlayScrollAndClamp(t *testing.T) {
	...
	h := 20
	o.scroll(5, h)
	v := cli.StripANSI(o.View(80, h))
	if strings.Contains(v, "line-000") {
		t.Fatalf("scroll did not move content:\n%s", v)
	}
	o.scroll(-1000, h)
	if v := cli.StripANSI(o.View(80, h)); !strings.Contains(v, "line-000") {
		t.Fatalf("scroll up must clamp to top:\n%s", v)
	}
	o.scroll(100000, h)
	if v := cli.StripANSI(o.View(80, h)); !strings.Contains(v, "line-099") {
...
func TestOverlayRedactsSecrets(t *testing.T) {
	...
	block := cli.ChatBlock{Kind: cli.ChatBlockTool, ToolName: "run_command",
		Text: `export API_KEY=sk-live-abc123def456`}
	o := newBlockOverlay(block)
	view := cli.StripANSI(o.View(100, 20))
...
func TestOverlayKeyFlow(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{overlayTestBlock(80)}
	m.renderVP()
	m.View()
