...
func (m *TUIModel) loadMoreMessages() {
	...
	var newBlocks []cli.ChatBlock
	maxIdx := len(msgs) - 1
	for i := m.msgOffset - 1; i >= newOffset && i <= maxIdx; i-- {
		msg := msgs[i]
		hydrated := cli.HydrateChatBlocksForView([]provider.Message{msg})
		if len(hydrated) == 0 {
			continue
		}
	...
	addedVisual := cli.VisualLineCount(cli.RenderChatBlocksWithWorkGroups(newBlocks, m.modelName, cli.Max(20, m.chatPaneWidth()-2), m.thinkingExpandDefault, m.workGroupCollapsed).Lines)
	...
	if m.msgOffset <= 0 {
		for i := range m.blocks {
			if m.blocks[i].Kind == cli.ChatBlockSystem && strings.Contains(m.blocks[i].Text, "showing last") {
				noticeVisual := cli.VisualLineCount(cli.RenderChatBlocksWithWorkGroups([]cli.ChatBlock{m.blocks[i]}, m.modelName, cli.Max(20, m.chatPaneWidth()-2), m.thinkingExpandDefault, m.workGroupCollapsed).Lines)
				m.blocks = append(m.blocks[:i], m.blocks[i+1:]...)
				m.messages = m.renderBlocksForView().Lines
				m.viewport.SetContent(m.buildViewportContent())
				m.viewport.YOffset = cli.Max(0, m.viewport.YOffset-noticeVisual)
				break
			}
		}
	}
