...
func workflowRunDialogPrefs() cli.DialogPrefs {
	return cli.DialogPrefs{PreferredWPct: 78, PreferredHPct: 78, MinW: 40, MinH: 8, FrameCols: 4, FrameRows: 3, Pager: true}
}

func (d *workflowRunDialog) layout(w, h int) cli.DialogLayout {
	return cli.MakeDialogLayout(w, h, workflowRunDialogPrefs(), func(innerW int) (int, int) {
		rows := cli.WrapDisplayRows(d.contentLines(), innerW)
		maxW := 0
...
func (d *workflowRunDialog) maxScroll(w, h int) int {
	l := d.layout(cli.Max(1, w), cli.Max(1, h))
	rows := cli.WrapDisplayRows(d.contentLines(), cli.Max(1, l.InnerW))
	return cli.Max(0, len(rows)-cli.Max(1, l.PageH))
}

func (d *workflowRunDialog) move(delta, w, h int) {
	d.scroll = cli.Min(cli.Max(0, d.scroll+delta), d.maxScroll(w, h))
}

func (d *workflowRunDialog) clampScroll(w, h int) {
	d.scroll = cli.Min(cli.Max(0, d.scroll), d.maxScroll(w, h))
}

// ViewAt renders the dialog frame over the paged content rows.
func (d *workflowRunDialog) ViewAt(w, h int) (string, cli.DialogLayout) {
	l := d.layout(cli.Max(1, w), cli.Max(1, h))
	rows := cli.WrapDisplayRows(d.contentLines(), cli.Max(1, l.InnerW))
	d.clampScroll(cli.Max(1, w), cli.Max(1, h))
	start := cli.Min(d.scroll, len(rows))
	end := cli.Min(len(rows), start+cli.Max(1, l.PageH))
	return cli.RenderDialogFrame("◆ workflow run "+d.runID, rows[start:end], d.footer(), l), l
}
...
func (m *TUIModel) handleWorkflowRunDialogKey(key string) (bool, bool, []tea.Cmd) {
	...
	d.notice = ""
	d.noticeErr = false
	w, h := cli.Max(1, m.width), cli.Max(1, m.height)
	if d.confirm != workflowConfirmNone {
		switch key {
		case "y":
