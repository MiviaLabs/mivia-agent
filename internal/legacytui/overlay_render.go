...
func RenderOverlayWindow(rows []string, selected, windowRows, termW, maxH int, title, footer string, width ...int) (string, cli.Rect) {
	if len(rows) == 0 || termW <= 0 || maxH < 1 {
		return "", cli.Rect{}
	}
	w := cli.Min(termW, cli.Max(24, cli.Min(72, termW-4)))
	if len(width) > 0 && width[0] > 0 {
		w = cli.Min(termW, cli.Max(24, cli.Min(width[0], termW-4)))
	}
	visible := cli.Min(windowRows, len(rows))
	...
	if (footer == "" && maxH < 3) || (footer != "" && maxH < 4) {
		...
		row := "› " + rows[cli.Min(selected, len(rows)-1)]
		if len(rows) > 1 {
			row += fmt.Sprintf("  +%d", len(rows)-1)
		}
		return cli.FitDialogRow(row, w), cli.Rect{W: w, H: 1}
	}
	frameRows := 2
	if footer != "" {
		frameRows = 3
	}
	h := cli.Min(maxH, visible+frameRows)
	if h < frameRows+1 {
		return "", cli.Rect{}
	}
	pageRows := cli.Max(0, h-frameRows)
	start := 0
	...
	l := cli.DialogLayout{Rect: cli.Rect{W: w, H: h}, InnerW: cli.Max(0, w-4), PageH: pageRows, FrameCols: 4, FrameRows: frameRows}
	panel := cli.RenderDialogFrame(title, out, footer, l)
	return panel, cli.Rect{W: lipgloss.Width(panel), H: lipgloss.Height(panel)}
}
...
func (m *TUIModel) renderComposerPopup() (string, cli.Rect) {
	pane := NewChatPaneLayout(m.width, m.sessionsSidebar != nil, m.workflowsSidebar != nil)
	termW := cli.Max(1, pane.chatWidth)
	maxH := cli.Max(0, m.suggestComposerTop()-1)
	if m.queueMgr.open {
...
func renderHistoryPanel(state HistoryState, entries []string, termW, maxH int) (string, cli.Rect) {
	if !state.Open || len(entries) == 0 || termW <= 0 || maxH < 1 {
		return "", cli.Rect{}
	}
	rows := make([]string, 0, len(entries))
	for _, entry := range entries {
		safe := cli.SafeChatBlockText(entry, 0) // strips CSI/OSC escapes and NUL
