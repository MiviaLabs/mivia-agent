...
// publish paths end to end: cli.EmitSubagentProgress → bus → UIAdapter → applyEvent
...
func nowPanelText(m *TUIModel) string {
	return cli.StripANSI(m.renderLivePanel(100, time.Now()))
}
...
func TestIntegrationFleetOverlayKeepsFinishedRunsLabelled(t *testing.T) {
	...
	got := cli.StripANSI(strings.Join(m.overlay.lines, "\n"))
...
func TestIntegrationNowPanelDropsFinishedTool(t *testing.T) {
	...
	m.updateFromDrain(cli.BridgeDrain{Tools: []cli.BridgeToolEvt{
	...
	for _, b := range m.blocks {
		if b.Kind == cli.ChatBlockTool && b.ToolName == "grep" {
			committed = true
		}
	}
	if !committed {
...
func TestIntegrationLivePanelDoesNotShrinkViewport(t *testing.T) {
	...
	// height.
	plain := strings.Split(cli.StripANSI(m.View()), "\n")
