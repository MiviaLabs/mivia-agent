...
var updateMessageImpl = func(m *TUIModel, msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if m.mode == modeChat || m.mode == modeWelcome {
		if !m.modalOpen() && m.focus == cli.FocusComposer {
			m.syncSuggest()
		}
	}
	if m.mode == modeChat {
...
func (m *TUIModel) clampModalState() {
	if m.overlay != nil {
		_, _ = m.overlay.ViewAt(cli.Max(1, m.width), cli.Max(1, m.height))
	}
	if m.modelDlg != nil {
		layout := m.modelDlg.layout(cli.Max(1, m.width), cli.Max(1, m.height))
		m.modelDlg.clampScroll(layout.PageH)
	}
	if m.agentDlg != nil {
		layout := m.agentDlg.layout(cli.Max(1, m.width), cli.Max(1, m.height))
		m.agentDlg.clampScroll(layout.PageH)
	}
	if m.effortDlg != nil {
		layout := m.effortDlg.layout(cli.Max(1, m.width), cli.Max(1, m.height))
		m.effortDlg.clampScroll(layout.PageH)
	}
	if m.worktreeDlg != nil {
		layout := m.worktreeDlg.layout(cli.Max(1, m.width), cli.Max(1, m.height))
		m.worktreeDlg.clampScroll(layout.PageH)
	}
	if m.workflowRunDlg != nil {
		m.workflowRunDlg.clampScroll(cli.Max(1, m.width), cli.Max(1, m.height))
	}
	m.clampQueueManager()
}
