...
func replSession(t *testing.T, level reasoning.Level) (*chat.Session, *config.Resolved, *strings.Builder, *cli.Terminal) {
	t.Helper()
	...
	return sess, res, out, cli.NewTestTerminal(out)
}
...
func TestIntegrationPlainModelSlashReportsDiscardedEffort(t *testing.T) {
	...
	if _, _, err := cli.HandleSlash("/model glm-5.2-air", sess, res, false, term); err != nil {
		t.Fatal(err)
	}
	if got := sess.ReasoningEffort(); got == reasoning.Low {
		t.Fatal("the outgoing model's effort survived the switch")
...
func TestIntegrationSameModelSwitchReportsNoDiscardOnAnySurface(t *testing.T) {
	...
	if _, _, err := cli.HandleSlash("/model glm-5.2", sess2, res, false, term); err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationFailedModelSwitchReportsNoDiscard(t *testing.T) {
	...
	sess2, res, out, term := replSession(t, reasoning.Low)
	if _, _, err := cli.HandleSlash("/model glm-nonexistent", sess2, res, false, term); err != nil {
		t.Fatal(err)
	}
	if strings.Contains(out.String(), "discarded") {
...
func TestIntegrationDefaultOnlyChangeReportsNoDiscard(t *testing.T) {
	...
	out := new(strings.Builder)
	if _, _, err := cli.HandleSlash("/model glm-5.2-air", sess2, res2, false, cli.NewTestTerminal(out)); err != nil {
		t.Fatal(err)
	}
	if strings.Contains(out.String(), "discarded") {
...
func TestIntegrationDiscardIsSilentWhenTheNewDefaultMatchesTheChoice(t *testing.T) {
	...
	if _, _, err := cli.HandleSlash("/model glm-5.2-air", sess2, res, false, term); err != nil {
		t.Fatal(err)
	}
	if strings.Contains(out.String(), "discarded") {
