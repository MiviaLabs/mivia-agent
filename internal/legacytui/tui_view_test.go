...
func newReadyChatModel(height, width int) *TUIModel {
	...
	m.width = width
	m.height = height
	m.viewport = viewport.New(width, cli.Max(2, height/2))
	m.viewport.SetContent("hello from viewport\nsecond line")
	m.textarea.SetWidth(cli.Max(20, width-4))
				...
				for _, nTools := range toolCounts {
					...
					if got > maxH {
						t.Fatalf("height=%d width=%d waiting=%v tools=%d: line count %d > max %d\nplain:\n%s",
							h, w, wait, nTools, got, maxH, cli.StripANSI(view))
					}
...
func TestView_StatusOutsideViewport(t *testing.T) {
	...
	bodyOnly := cli.StripANSI(m.viewport.View())
...
func TestRenderScrollIndicator(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(got), "↓") {
		t.Fatalf("expected ↓ in indicator, got %q", cli.StripANSI(got))
	}
}
...
func TestLivePanelOverlayDoesNotReflowTranscript(t *testing.T) {
	...
	idleLines := strings.Split(cli.StripANSI(m.View()), "\n")
	...
	waitLines := strings.Split(cli.StripANSI(m.View()), "\n")
...
func TestShortenModel(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := cli.ShortenModel(tc.in); got != tc.want {
