...
func TestFocusableBlockIDs_OrderAndExcludesDivider(t *testing.T) {
	blocks := []cli.ChatBlock{
		{ID: "u1", Kind: cli.ChatBlockUser, Text: "hi"},
		{ID: "d1", Kind: cli.ChatBlockDivider, Text: "───"},
		{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "yo"},
	}
...
func TestCycleChatFocus_TabFromComposerSelectsFirst(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{
		{ID: "u1", Kind: cli.ChatBlockUser, Text: "hello user"},
		{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "hello asst"},
	}
	m.renderVP()
...
func TestCycleChatFocus_TabThroughBlocksThenComposer(t *testing.T) {
	...
	m.ready = true
	m.blocks = []cli.ChatBlock{
		{ID: "u1", Kind: cli.ChatBlockUser, Text: "u"},
		{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "a"},
	}
	m.renderVP()
	ids := focusableBlockIDs(m.chatBlockRanges, m.blocks)
	if len(ids) < 2 {
	...
	if m.focus != cli.FocusComposer || m.selectedBlockID != "" {
...
func TestUserBubbleTime_NoSecondsBracketedDimTrailing(t *testing.T) {
	...
	got := cli.FormatUserBubbleTime(sent)
	...
	lines := cli.UserBubble.Render("hello body", 40, sent)
	var content []string
...
func TestHandleChatKey_TabCyclesFocus(t *testing.T) {
	...
	if m.focus != cli.FocusScrollback || m.selectedBlockID == "" {
...
func TestCycleChatFocus_WrapToComposerRepaints(t *testing.T) {
	...
	m.ready = true
	m.blocks = []cli.ChatBlock{
		{ID: "u1", Kind: cli.ChatBlockUser, Text: "only"},
	}
	m.renderVP()
	ids := focusableBlockIDs(m.chatBlockRanges, m.blocks)
	if len(ids) == 0 {
