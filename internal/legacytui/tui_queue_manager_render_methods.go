...
func (m *TUIModel) renderQueuePanel(termW, maxH int) (string, cli.Rect) {
	...
	for i := 0; i < m.queueCount(); i++ {
		...
		text = cli.SafeChatBlockText(text, 0) // strips CSI/OSC escapes and NUL
	...
	width := cli.Max(24, cli.Min(termW-4, 90))
