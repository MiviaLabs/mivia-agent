...
func writeToolPanelRow(
	...
	if opts := terminalToolRenderOptions(); !opts.Color {
		...
		if r.Agent != "" {
			line = "  " + cli.GlyphDiamond + " " + BoundedToolText(r.Agent, 24) + " " + strings.TrimSpace(line)
		}
	...
	case r.Failed:
		iconStyled = ToolErrStyle.Render(cli.GlyphCross)
	default:
		iconStyled = ToolOkStyle.Render(cli.GlyphCheck)
	}
...
func writePreviewSection(b *strings.Builder, header, body string, width, maxLines int, colorDiff bool) int {
	...
	n := 1
	if colorDiff {
		lines := cli.RenderDiffBody(body, width, maxLines)
...
func expandSectionLabel(name string, input bool) string {
	...
	switch name {
	case cli.HandlerDelegate:
		return "task"
	case cli.ToolDispatchTasks:
		return "tasks"
	default:
		return "input"
	}
