...
func TestWorkflowsSidebarEmptyListRendersNoRunsRow(t *testing.T) {
	...
	view := cli.StripANSI(s.view(28, 20, false))
...
func TestWorkflowsSidebarSelectedRowExpands(t *testing.T) {
	...
	s.rows = workflowTestRows()
	s.cursor = 0
	view := cli.StripANSI(s.view(28, 24, true))
...
func TestWorkflowsSidebarTruncationRuneSafe(t *testing.T) {
	...
	for _, line := range strings.Split(cli.StripANSI(view), "\n") {
		if cli.RuneWidth(line) > 24 {
			t.Fatalf("line width %d exceeds 24: %q", cli.RuneWidth(line), line)
		}
	}
}
