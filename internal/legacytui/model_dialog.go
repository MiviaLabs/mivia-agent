...
func modelDialogPrefs() cli.DialogPrefs {
	return cli.DialogPrefs{PreferredWPct: 80, PreferredHPct: 75, MinW: 32, MinH: 8, FrameCols: 4, FrameRows: 3, Pager: true}
}
...
func (d *modelDialog) layout(w, h int) cli.DialogLayout {
	return cli.MakeDialogLayout(w, h, modelDialogPrefs(), func(innerW int) (int, int) {
...
func (d *modelDialog) ViewAt(w, h int) (string, cli.DialogLayout) {
	l := d.layout(w, h)
	rows := d.rowLines(l.InnerW, l.PageH)
	return cli.RenderDialogFrame("◇ models", rows, d.footer(), l), l
}
...
func (m *TUIModel) selectModelDialogRow(row modelDialogRow) {
	...
	m.appendInfo(fmt.Sprintf("model set to %s/%s", row.provider, row.model) + cli.EffortDiscardedSuffix(discarded))
}
...
func (m *TUIModel) switchModel(providerName, model string) (reasoning.Level, error) {
	...
	return cli.SwitchModelCommand(m.session, m.config, providerName, model)
}
