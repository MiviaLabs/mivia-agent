...
func TestStreamBridgePendingReportsUndrainedWithoutConsuming(t *testing.T) {
	...
	var nilBridge *cli.StreamBridge
...
func TestUpdateFromDrainResetStreamClearsStreamBuf(t *testing.T) {
	...
	m.updateFromDrain(cli.BridgeDrain{ResetStream: true})
...
func TestInterimAssistantBecomesChatBubble(t *testing.T) {
	...
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "find bugs"})
	...
	for _, b := range m.blocks {
		if b.Kind == cli.ChatBlockAssistant && strings.Contains(b.Text, "I'll search") {
			foundSpeech = true
		}
	}
	if !foundSpeech {
