...
func newSidebarLayout(rows, width, height int) sidebarLayout {
	height = cli.Max(1, height)
	space := cli.Max(0, height-sidebarChromeRows)
	...
	if width >= 24 && height >= 10 && space >= 4 {
		...
		layout.rowCapacity = cli.Max(1, space/layout.rowHeight)
	}
	if rows > layout.rowCapacity && space > layout.rowHeight {
		layout.showCue = true
		layout.rowCapacity = cli.Max(1, (space-1)/layout.rowHeight)
	}
	return layout
}
...
func (s *sessionsSidebar) clampScroll(rows int, visible int) {
	...
	s.scroll = cli.Min(cli.Max(0, s.scroll), rows-visible)
}
...
func (s *sessionsSidebar) cursorAt(rows []chat.SessionInfo, width, height, y int) (int, bool) {
	if y < 0 || y >= cli.Max(1, height) {
		return 0, false
	}
	layout := newSidebarLayout(len(rows), width, height)
	if y == layout.newSessionY && y < cli.Max(1, height) {
		return 0, true
	}
	...
	index := s.scroll + row
	if row >= layout.rowCapacity || index < s.scroll || index >= cli.Min(len(rows), s.scroll+layout.rowCapacity) {
		return 0, false
	}
	return index + 1, true
}
...
func (s *sessionsSidebar) viewWithActive(rows []chat.SessionInfo, width, height int, focused bool, active *chat.SessionInfo, status sidebarLiveStatus) string {
	...
	end := cli.Min(len(rows), s.scroll+cli.Max(0, layout.rowCapacity))
...
func (s *sessionsSidebar) renderSessionRow(row chat.SessionInfo, selected bool, width int, focused, showMetadata, active bool, latestAuto string, status sidebarLiveStatus) []string {
	...
	budget := cli.Max(1, width-cli.RuneWidth(marker))
	if active {
		budget = cli.Max(1, budget-1)
	}
	name = cli.TruncateToWidth(name, budget)
	line := marker + name
	if active {
		line = marker + sidebarLiveDot(status, width >= sidebarStatusColorMinWidth) + name
	}
	line = line + strings.Repeat(" ", cli.Max(0, width-lipgloss.Width(line)))
	if selected {
...
func sidebarPad(text string, width int) string {
	text = cli.TruncateToWidth(text, width)
	return text + strings.Repeat(" ", cli.Max(0, width-cli.RuneWidth(text)))
}
