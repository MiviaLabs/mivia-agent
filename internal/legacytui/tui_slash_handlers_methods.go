...
func (m *TUIModel) handleTuiInfoSlash(cmd string, fields []string) bool {
	...
	case "/help", "/h", "/?":
		...
		return true
	case "/agents":
		m.appendInfo(cli.FormatAgentCurrent(cli.CurrentAgentName(m.agentState), cli.RegistryForState(m.agentState)))
		return true
	case "/status":
...
func (m *TUIModel) handleTuiModelSlash(cmd string, fields []string) bool {
	...
	providerName, modelName, hasArg := cli.ParseModelArgs(fields, m.session.CurrentSelection().ProviderName, defaultProvider)
	if !hasArg {
		m.openModelDialog()
		return true
	}
	choices := cli.ModelSwitchChoices(m.config, providerName, defaultProvider)
	discarded, err := m.switchModel(providerName, modelName)
	if err != nil {
		m.appendInfo(cli.FormatModelUnavailable(providerName, choices))
		return true
	}
	m.modelName = cli.ShortenModel(m.session.CurrentModel())
	m.appendInfo(cli.FormatModelSet(m.session.CurrentSelection().ProviderName, m.session.CurrentModel(), discarded))
	return true
}
...
func (m *TUIModel) handleTuiAgentSlash(cmd string, fields []string) bool {
	...
	m.appendInfo(cli.FormatAgentSet(name))
	return true
}
...
func (m *TUIModel) handleTuiLimitsSlash(cmd string, fields []string) bool {
	...
	case "/budget":
		...
		n, hasArg, ok := cli.ParseNonNegInt(fields)
		if hasArg {
			if !ok {
				arg := ""
				if len(fields) >= 2 {
				...
				m.appendInfo(cli.FormatBudgetInvalid(arg))
				return true
			}
			...
			m.appendInfo(cli.FormatBudgetSet(m.session.PromptBudget()))
			return true
		}
		m.appendInfo(cli.FormatBudgetSummary(m.session.PromptBudget()))
		return true
	case "/steps":
		n, hasArg, ok := cli.ParseNonNegInt(fields)
		if hasArg {
			if !ok {
				...
				m.appendInfo(cli.FormatStepsInvalid(arg))
				return true
			}
			...
			m.appendInfo(cli.FormatStepsSet(n))
			return true
		}
		m.appendInfo(cli.FormatStepsSummary(m.session.MaxStepsValue()))
		return true
	default:
		return false
	}
...
func (m *TUIModel) handleTuiSessionStoreSlash(cmd string, fields []string) bool {
	...
	case "/save":
		if len(fields) >= 2 {
			if err := m.session.Save(fields[1]); err != nil {
				m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: TUIErrorStyle.Render("save error: " + err.Error()), Rendered: TUIErrorStyle.Render("save error: " + err.Error())})
			} else {
				if err := m.refreshSessionList(); err != nil {
					m.appendInfo("sessions refresh failed: " + err.Error())
				}
				m.appendInfo(cli.SaveSessionResult(fields[1], m.session.MessagesCount(), m.session.UserTurns()))
			}
		} else {
	...
	case "/list":
		...
		if err != nil {
			m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: TUIErrorStyle.Render("list error: " + err.Error()), Rendered: TUIErrorStyle.Render("list error: " + err.Error())})
		} else if len(sessions) == 0 {
			m.appendInfo("no saved sessions")
		} else {
			m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: TUIHeaderStyle.Render("── saved sessions ──"), Rendered: TUIHeaderStyle.Render("── saved sessions ──")})
			for _, si := range sessions {
				...
				m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: TUIDimStyle.Render(line), Rendered: TUIDimStyle.Render(line)})
			}
		}
		return true
	case "/delete":
		if len(fields) >= 2 {
			if err := m.session.DeleteSession(fields[1]); err != nil {
				m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: TUIErrorStyle.Render("delete error: " + err.Error()), Rendered: TUIErrorStyle.Render("delete error: " + err.Error())})
			} else {
				if err := m.refreshSessionList(); err != nil {
					m.appendInfo("sessions refresh failed: " + err.Error())
				}
				m.appendInfo(cli.DeleteSessionResult(fields[1]))
			}
		} else {
...
func (m *TUIModel) runLoadSlash(fields []string) {
	...
	if err := m.session.Load(fields[1]); err != nil {
		m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: TUIErrorStyle.Render("load error: " + err.Error()), Rendered: TUIErrorStyle.Render("load error: " + err.Error())})
		return
	}
	...
	if m.session.LoadedContextSession() {
		m.appendInfo(cli.LoadContextSessionResult(fields[1], m.session.MessagesCount(), m.session.UserTurns()))
	} else {
		m.appendInfo(cli.LoadSessionResult(fields[1], m.session.MessagesCount(), m.session.UserTurns()))
	}
	m.msgOffset = 0 // all messages loaded
	msgs := m.session.MessagesCopy()
	for _, block := range cli.HydrateChatBlocksForView(msgs) {
		m.appendBlock(block)
	}
...
func (m *TUIModel) appendModelRestoreNotice() {
	if saved, current, ok := m.session.ModelRestoreNotice(); ok {
		m.appendInfo(cli.ModelRestoreNoticeText(saved, current))
	}
