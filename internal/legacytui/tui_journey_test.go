...
// cli.ChatBlockUser block (not cli.ChatBlockSystem pre-rendered card lines).
func TestStartAI_LiveUserCreatesChatBlockUser(t *testing.T) {
	...
	// Should have exactly 1 user block (no cli.ChatBlockSystem blocks).
	if len(m.blocks) != 1 {
		t.Fatalf("expected 1 block, got %d: %#v", len(m.blocks), m.blocks)
	}
	if m.blocks[0].Kind != cli.ChatBlockUser {
		t.Fatalf("expected cli.ChatBlockUser, got %s", m.blocks[0].Kind)
	}
...
func TestStartAI_UserBlockAcrossWidths(t *testing.T) {
	for _, w := range []int{40, 80, 120} {
		...
		plain := cli.StripANSI(strings.Join(m.messages, "\n"))
		if !strings.Contains(plain, userText) {
...
func TestTUIJourneyWelcomeToChatAndStream(t *testing.T) {
	...
	joined := strings.Join(m.messages, "\n")
	if !strings.Contains(joined, "hello") && !strings.Contains(cli.StripANSI(joined), "hello") {
...
func TestTUIJourneyHistoricalBlockMouseAndKeyboardActivation(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{{ID: "history-1", Kind: cli.ChatBlockTool, ToolName: "read_file", Text: "historical"}}
	m.renderVP()
	m.View()

	// Y=1: first transcript row under the one-line diamond header.
	m.Update(tea.MouseMsg{X: 1, Y: 1, Type: tea.MouseLeft})
	if m.selectedBlockID != "history-1" || m.focus != cli.FocusScrollback {
