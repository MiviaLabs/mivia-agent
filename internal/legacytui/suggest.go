...
// SuggestWindowRows is the visible row count of the slash-suggest overlay.
const SuggestWindowRows = 8

// SuggestState holds the slash-command suggestion overlay's open state.
...
func (m *TUIModel) syncSuggest() {
	...
	commands := rankSlashCommands(token, cli.SlashCommands(cli.SlashSurfaceTUI, binding.SkillRegistry))
	if len(commands) == 0 {
		m.closeSuggest()
		return
	}
...
func (m *TUIModel) handleSuggestKey(key string) (bool, bool, []tea.Cmd) {
	if !m.suggest.open || m.focus != cli.FocusComposer || len(m.suggest.commands) == 0 {
...
func suggestHasNoArguments(value string, commandEnd int) bool {
	runes := []rune(value)
	commandEnd = cli.Min(cli.Max(0, commandEnd), len(runes))
...
func rankSlashCommands(query string, commands []cli.SlashCommand) []cli.SlashCommand {
	if query == "/" {
		return orderBareSlashCommands(commands)
	}
	type scored struct {
		command     cli.SlashCommand
		tier, score int
	}
	...
	for _, command := range commands {
		...
		case command.Kind == cli.SlashKindSkill && fuzzyScore(query, strings.ToLower(command.Description)) > 0:
	...
	out := make([]cli.SlashCommand, len(matches))
	for i := range matches {
...
func orderBareSlashCommands(commands []cli.SlashCommand) []cli.SlashCommand {
	order := make(map[string]int)
	for i, name := range []string{"/help", "/clear", "/model", "/agent", "/agents", "/status", "/sessions", "/save", "/load", "/search"} {
		order[name] = i
	}
	out := append([]cli.SlashCommand(nil), commands...)
	...
	sort.SliceStable(out, func(i, j int) bool {
		...
		if out[i].Kind == cli.SlashKindBuiltin {
...
func applyTokenReplace(ta *textarea.Model, from, to int, insert string) {
	full := []rune(ta.Value())
	from = cli.Min(cli.Max(0, from), len(full))
	to = cli.Min(cli.Max(from, to), len(full))
