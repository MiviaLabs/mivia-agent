...
func TestSidebarFocusCycleReturnsPrintableAndEscapeToComposer(t *testing.T) {
	for _, key := range []string{"x", "esc"} {
		focus, consumed := cli.RouteFocusKey(cli.FocusSidebar, key)
		if focus != cli.FocusComposer {
			t.Errorf("cli.RouteFocusKey(sidebar, %q) focus = %v, want composer", key, focus)
		}
		wantConsumed := key == "esc"
		if consumed != wantConsumed {
			t.Errorf("cli.RouteFocusKey(sidebar, %q) consumed = %t, want %t", key, consumed, wantConsumed)
		}
	}
}
...
func TestVisibleSidebarFocusCycleIncludesSidebar(t *testing.T) {
	...
	if got := m.nextTUIFocus(cli.FocusComposer, false); got != cli.FocusScrollback {
		t.Fatalf("composer next focus = %v, want scrollback", got)
	}
	if got := m.nextTUIFocus(cli.FocusScrollback, false); got != cli.FocusSidebar {
...
func TestWorkflowsSidebarFocusCycle(t *testing.T) {
	...
	want := []cli.TuiFocus{cli.FocusComposer, cli.FocusScrollback, cli.FocusSidebar, cli.FocusWorkflowsSidebar}
...
func TestWorkflowsSidebarFocusCycleHiddenRightSidebar(t *testing.T) {
	...
	if got := m.nextTUIFocus(cli.FocusSidebar, false); got != cli.FocusComposer {
...
func TestSetFocusWorkflowsSidebarVisible(t *testing.T) {
	...
	if m.focus != cli.FocusWorkflowsSidebar {
		t.Fatalf("workflows sidebar focus = %v, want cli.FocusWorkflowsSidebar", m.focus)
	}
}
...
func TestRouteFocusKeyWorkflowsSidebar(t *testing.T) {
	for _, key := range []string{"x", "esc"} {
		focus, consumed := cli.RouteFocusKey(cli.FocusWorkflowsSidebar, key)
		if focus != cli.FocusComposer {
			t.Errorf("cli.RouteFocusKey(workflows, %q) focus = %v, want composer", key, focus)
		}
		wantConsumed := key == "esc"
		if consumed != wantConsumed {
			t.Errorf("cli.RouteFocusKey(workflows, %q) consumed = %t, want %t", key, consumed, wantConsumed)
		}
	}
}
