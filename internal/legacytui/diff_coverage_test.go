...
// dialog routing, status chrome, history paging edges, and the worktree
// dialog's error paths.
...
func TestVisualLineCount(t *testing.T) {
	if got := cli.VisualLineCount([]string{"a\nb", "c"}); got != 3 {
		t.Fatalf("visualLineCount = %d, want 3", got)
	}
	if got := cli.VisualLineCount(nil); got != 0 {
