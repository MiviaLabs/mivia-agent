...
func TestEmptyContentToolsGetStatusLine(t *testing.T) {
	...
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "list files"})
	...
	if !kindOrderContains(blockKinds(m.blocks),
		cli.ChatBlockUser, cli.ChatBlockSystem, cli.ChatBlockTool, cli.ChatBlockAssistant, cli.ChatBlockDivider,
	) {
...
func TestShortInterimRejectedUsesStatus(t *testing.T) {
	...
	m.waiting = true
	m.turnStart = time.Now()
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "go"})
...
func TestInterimAcceptedSkipsStatus(t *testing.T) {
	...
	m.waiting = true
	m.turnStart = time.Now()
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "find"})
...
func TestAwaitingFirstActivityPlanning(t *testing.T) {
	...
	m.renderStreamVP()
	plain2 := cli.StripANSI(m.viewport.View())
...
func TestCancelKeepsInterimAndToolsInHistory(t *testing.T) {
	...
	m.waiting = true
	m.turnStart = time.Now()
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "explore"})
	m.updateFromDrain(cli.BridgeDrain{
	...
	foundCancel := false
	for _, b := range m.blocks {
		if b.Kind == cli.ChatBlockDivider && strings.Contains(b.Text, "cancelled") {
			foundCancel = true
		}
	}
	if !foundCancel {
...
func TestCancelBeforeFirstActivity(t *testing.T) {
	...
	m.awaitingFirstActivity = true
	m.turnStart = time.Now()
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "hi"})
	_, _, _ = m.handleChatCancel()
	if m.waiting {
		t.Fatal("must not wait after cancel")
	}
	// User + cancelled divider; no panic.
	if !hasBlockKind(m.blocks, cli.ChatBlockUser) {
...
func countCancelDividers(blocks []cli.ChatBlock) int {
	n := 0
...
func blockTexts(blocks []cli.ChatBlock) []string {
