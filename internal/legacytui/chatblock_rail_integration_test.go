...
func TestIntegration_FullHeightRail_UserMultiLine(t *testing.T) {
	...
	blocks := []cli.ChatBlock{{
		ID: "u1", Kind: cli.ChatBlockUser, Text: long, SentAt: time.Now(),
	}}
	r := cli.RenderChatBlocks(blocks, "m", 36, true)
	var content []string
...
func TestIntegration_FullHeightRail_ToolExpanded(t *testing.T) {
	...
	body := "line-one\nline-two\nline-three"
	blocks := []cli.ChatBlock{{
		ID: "t1", Kind: cli.ChatBlockTool, ToolName: "read_file",
		Text: body, Collapsed: false,
	}}
	r := cli.RenderChatBlocks(blocks, "m", 60, true)
	var content []string
	for _, ln := range r.Lines {
		p := cli.StripANSI(ln)
		if strings.TrimSpace(p) == "" {
...
func TestIntegration_FullHeightRail_ToolRenderedProduction(t *testing.T) {
	...
	blocks := []cli.ChatBlock{{
		ID: "t1", Kind: cli.ChatBlockTool, ToolName: "grep",
		Text: "match\nmatch2", Rendered: rendered, Collapsed: true,
	}}
	r := cli.RenderChatBlocks(blocks, "m", 80, true)
	if len(r.Lines) != 1 {
		t.Fatalf("collapsed tool must be 1 line, got %d %v", len(r.Lines), r.Lines)
	}
	p := cli.StripANSI(r.Lines[0])
	if !hasRailPrefix(p) {
...
func TestIntegration_Collapsed_OnlyOneLineWithRail(t *testing.T) {
	...
	kinds := []struct {
		...
		{cli.ChatBlock{ID: "t", Kind: cli.ChatBlockTool, ToolName: "run_command", Text: "out1\nout2\nout3", Collapsed: true}},
		{cli.ChatBlock{ID: "th", Kind: cli.ChatBlockThinking, Text: "a\nb\nc\nd\ne\nf\ng", Collapsed: true}},
	}
	...
	for _, tc := range kinds {
		r := cli.RenderChatBlocks([]cli.ChatBlock{tc.b}, "m", 50, true)
		// User/assistant may append a trailing empty lane; content is first non-empty.
		var content []string
		for _, ln := range r.Lines {
			if strings.TrimSpace(cli.StripANSI(ln)) != "" {
				content = append(content, cli.StripANSI(ln))
			}
		}
		if len(content) != 1 {
...
func TestIntegration_AssistantBubblePadding_Expanded(t *testing.T) {
	t.Setenv("NO_COLOR", "1")
	t.Setenv("TERM", "dumb")
	r := cli.RenderChatBlocks([]cli.ChatBlock{{
		ID: "a", Kind: cli.ChatBlockAssistant, Text: "assistant body pad",
	}}, "m", 40, true)
	p := cli.AssistantBubble.Style.Padding
	if p.Top != 0 || p.Bottom != 0 {
		t.Fatalf("assistant vertical pad should be 0 (lane after): %+v", p)
	}
	joined := cli.StripANSI(strings.Join(r.Lines, "\n"))
	...
	// Trailing empty lane
	if len(r.Lines) < 2 || strings.TrimSpace(cli.StripANSI(r.Lines[len(r.Lines)-1])) != "" {
...
func TestIntegration_UserPadding_VerticalAndHorizontal(t *testing.T) {
	...
	const w = 40
	r := cli.RenderChatBlocks([]cli.ChatBlock{{
		ID: "u", Kind: cli.ChatBlockUser, Text: "pad me",
	}}, "m", w, true)
	p := cli.UserBubble.Style.Padding
...
func TestIntegration_SystemCollapse_AndToggle(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{
		{ID: "s1", Kind: cli.ChatBlockSystem, Text: "long system status " + strings.Repeat("x", 40), Collapsed: false},
	}
	...
	r := cli.RenderChatBlocks([]cli.ChatBlock{m.blocks[0]}, "m", 50, true)
	if len(r.Lines) != 1 {
...
func TestIntegration_ToggleCollapse_AllKinds(t *testing.T) {
	...
	m.width = 60
	m.blocks = []cli.ChatBlock{
		{ID: "t1", Kind: cli.ChatBlockTool, ToolName: "delegate", Text: "sub\nagent\nout", Collapsed: false},
		{ID: "th1", Kind: cli.ChatBlockThinking, Text: "t1\nt2\nt3\nt4\nt5\nt6\nt7", Collapsed: false},
	}
	...
	for _, id := range []string{"t1", "th1"} {
		...
		r := cli.RenderChatBlocks([]cli.ChatBlock{*b}, "m", 50, true)
		nContent := 0
		for _, ln := range r.Lines {
			if strings.TrimSpace(cli.StripANSI(ln)) != "" {
				nContent++
...
func TestIntegration_SubagentToolSameAsToolRail(t *testing.T) {
	...
	t.Setenv("TERM", "dumb")
	blocks := []cli.ChatBlock{{
		ID: "d1", Kind: cli.ChatBlockTool, ToolName: "delegate",
		Text: "task output\nline2", Collapsed: false,
	}}
	r := cli.RenderChatBlocks(blocks, "m", 50, true)
...
func TestIntegration_MixedTimeline_RailsAndPadding(t *testing.T) {
	...
	t.Setenv("TERM", "dumb")
	blocks := []cli.ChatBlock{
		{ID: "u", Kind: cli.ChatBlockUser, Text: "do the thing", SentAt: time.Now()},
		{ID: "th", Kind: cli.ChatBlockThinking, Text: "plan\nstep", Collapsed: false},
		{ID: "t", Kind: cli.ChatBlockTool, ToolName: "read_file", Text: "a\nb", Collapsed: false},
		{ID: "a", Kind: cli.ChatBlockAssistant, Text: "here is the answer with more words"},
	}
	r := cli.RenderChatBlocks(blocks, "m", 50, true)
	plain := cli.StripANSI(strings.Join(r.Lines, "\n"))
...
func TestConversationBlocksNeverCollapse(t *testing.T) {
	...
	m.width = 60
	m.blocks = []cli.ChatBlock{
		{ID: "u1", Kind: cli.ChatBlockUser, Text: strings.Repeat("user text ", 30)},
		{ID: "a1", Kind: cli.ChatBlockAssistant, Text: strings.Repeat("asst text ", 30)},
	}
	m.renderVP()
