...
func TestSummarizeToolDetail(t *testing.T) {
	...
	s := cli.SummarizeToolDetail("search_replace", `{"path":"x.go"}`, "updated x.go (1 replacement, +2 −1)\n--- a/x.go\n a")
	if s != "updated (1 replacement, +2 −1)" {
		t.Fatalf("summary=%q", s)
	}
	s = cli.SummarizeToolDetail("write_file", `{"path":"x.go"}`, "wrote x.go (4 bytes, create +1)")
	if s != "wrote (4 bytes, create +1)" {
		t.Fatalf("write summary=%q", s)
	}
	s = cli.SummarizeToolDetail("read_file", `{"path":"only.go"}`, "")
...
func TestSummarizeDelegateAndDispatchOperatorFacing(t *testing.T) {
	s := cli.SummarizeToolDetail("delegate", `{"task":"analyze auth module for JWT bugs","multi_step":true}`, "")
	if !strings.Contains(s, "multi_step") || !strings.Contains(s, "analyze auth") {
		t.Fatalf("delegate summary=%q", s)
	}
	s = cli.SummarizeToolDetail("delegate", `{"task":"quick q"}`, "")
	if !strings.Contains(s, "oneshot") || !strings.Contains(s, "quick q") {
		t.Fatalf("oneshot summary=%q", s)
	}
	s = cli.SummarizeToolDetail("dispatch_tasks", `{"tasks":[{"id":"t1","prompt":"map package layout"},{"id":"t2","prompt":"find race bugs"}]}`, "")
...
func TestPrintToolLinesStripControlSequences(t *testing.T) {
	for _, tc := range []struct {
		name  string
		print func(*cli.ChatRenderer)
	}{
		{"start", func(r *cli.ChatRenderer) { r.PrintToolStart("read_file", "\x1b[2Jevil.txt") }},
		{"end", func(r *cli.ChatRenderer) { r.PrintToolEnd("read_file", "error: open \x1b[2Jevil.txt: no such file") }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			buf := new(bytes.Buffer)
			r := cli.NewChatRenderer(cli.NewTestTerminal(buf), "m")
			tc.print(r)
