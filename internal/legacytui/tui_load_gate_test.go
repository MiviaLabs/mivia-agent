...
func TestTuiLoadReportsAContextSessionDifferently(t *testing.T) {
	...
	if err := cli.EnableSessionContext(m.session, t.TempDir(), store, &config.Resolved{}); err != nil {
		t.Fatal(err)
	}
	...
	want := cli.LoadContextSessionResult(name, m.session.MessagesCount(), m.session.UserTurns())
