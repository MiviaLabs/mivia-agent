...
func (m *TUIModel) livePanelSections(termH int) (fleet, tools, thinking, stream int) {
	...
	if n := len(m.subagents.ActiveRows()); n > 0 {
		fleet = cli.Min(n, liveMaxFleetRows)
	}
	if n := len(m.toolRows); n > 0 {
		tools = cli.Min(n, liveMaxToolRows)
	}
	if m.thinkingBuf.Len() > 0 {
		// One label row + the rolling tail beneath it.
		thinking = 1 + cli.Min(liveMaxThinkingRows, len(thinkingTailLines(m.thinkingBuf.String(), liveMaxThinkingRows)))
	...
	if m.streamBuf.Len() > 0 {
		stream = cli.Min(liveMaxStreamRows, len(strings.Split(strings.TrimRight(m.streamBuf.String(), "\n"), "\n")))
	}
	...
	budget := m.livePanelBand(cli.Max(8, termH)) - 2
	if budget < 2 {
...
func (m *TUIModel) livePanelBand(termH int) int {
	...
	return cli.Min(livePanelMaxHeight, cli.Max(1, termH/3))
}
...
func (m *TUIModel) livePanelHeight() int {
	return m.livePanelBand(cli.Max(8, m.height))
}
...
func (m *TUIModel) livePanelOverlayRect() cli.Rect {
	pane := NewChatPaneLayout(cli.Max(1, m.width), m.sessionsSidebar != nil, m.workflowsSidebar != nil)
	return cli.Rect{X: pane.chatX, Y: 1, W: pane.chatWidth, H: m.livePanelHeight()}
}
...
func (m *TUIModel) renderLivePanel(width int, now time.Time) string {
	band := m.livePanelBand(cli.Max(8, m.height))
	if band == 0 {
		return ""
	}
	if width < 30 {
	...
	fleetN, toolN, thinkingN, streamN := m.livePanelSections(cli.Max(8, m.height))
	...
	if thinkingN > 0 {
		...
		if len(tail) > 0 {
			...
			for i, line := range tail {
				...
				rows = append(rows, style.Render(cli.TruncateToWidth("    "+line, inner)))
			}
		} else if plan := m.livePlanningLine(); plan != "" {
			rows = append(rows, TUIDimStyle.Render(cli.TruncateToWidth(plan, inner)))
		}
	}
	if streamN > 0 {
		lines := strings.Split(strings.TrimRight(m.streamBuf.String(), "\n"), "\n")
		tail := lines[cli.Max(0, len(lines)-streamN):]
		for i, ln := range tail {
			prefix := "  "
			if i == len(tail)-1 {
				prefix = TUIDimStyle.Render("▌ ")
			}
			rows = append(rows, cli.TruncateToWidth(prefix+ln, inner))
		}
	}
	...
	b.WriteString(border.Render("┌─" + head + strings.Repeat("─", cli.Max(0, width-3-lipgloss.Width(head))) + "┐"))
	for _, r := range rows {
		b.WriteByte('\n')
	...
	b.WriteByte('\n')
	b.WriteString(border.Render("└" + strings.Repeat("─", cli.Max(0, width-2)) + "┘"))
	return b.String()
}
...
func (m *TUIModel) liveToolRows(n, inner int, now time.Time) []string {
	...
	for i := 0; i < n && i < len(ordered); i++ {
		...
		icon := toolRunStyle.Render(toolRowIcon(r, now))
		if r.Done {
			icon = ToolOkStyle.Render(cli.GlyphCheck)
			if r.Failed {
				icon = ToolErrStyle.Render(cli.GlyphCross)
			}
		}
		...
		line := icon + " " + ToolIconForName(r.Name) + " " + ToolNameStyle.Render(r.Name) + " " +
			TUIDimStyle.Render(item.Summary(cli.Max(10, inner-28))) + " " +
			ToolTimeStyle.Render(FormatDuration(toolRowElapsed(r, now)))
		rows = append(rows, cli.TruncateToWidth(line, inner))
	}
