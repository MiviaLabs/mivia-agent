...
func TestRenderWorkChrome_IncludesSemanticStateLabel(t *testing.T) {
	for _, phase := range []brandPhase{phaseThinking, phaseStreaming, phaseTools, phaseMulti, phaseQueued, phaseError, phaseCancel} {
		out := cli.StripANSI(renderWorkChrome(0, phase, time.Second, 0, 0, 0, 0, 80, "", "", ""))
...
func TestRenderWorkChrome_ShowsThinkingProgressDetail(t *testing.T) {
	out := cli.StripANSI(renderWorkChrome(
...
func TestRenderWorkChrome_BoundsAndSanitizesProgressDetail(t *testing.T) {
	...
	wide := cli.StripANSI(renderWorkChrome(
	...
	if got := cli.VisibleWidth(out); got > 20 {
...
func TestRenderStatusBarSimpleDiamond(t *testing.T) {
	...
	out := cli.StripANSI(renderStatusBar(3, phaseThinking, true, time.Second, 0, 0, 0, 0, 0, 80, "", "", ""))
	...
	idle := cli.StripANSI(renderStatusBar(0, phaseIdle, false, 0, 0, 0, 0, 0, 4, 80, "", "", ""))
	...
	errBar := cli.StripANSI(renderStatusBar(7, phaseError, false, 0, 0, 0, 0, 0, 0, 80, "", "", ""))
