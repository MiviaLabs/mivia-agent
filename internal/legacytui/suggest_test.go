...
func TestSuggestWorksInWelcomeComposer(t *testing.T) {
	...
	if view := cli.StripANSI(m.View()); !strings.Contains(view, "/help") {
...
func TestSuggestPopupStaysInsideChatPaneWithSessionsSidebar(t *testing.T) {
	...
	panel, size := RenderSuggestPanel(m.suggest, pane.chatWidth, cli.Max(0, m.suggestComposerTop()-1))
	got := SuggestOverlayRect(m, panel, size)
	if got.X < pane.chatX || got.X+got.W > pane.chatX+pane.chatWidth {
		t.Fatalf("suggestion cli.Rect %#v is outside chat pane %#v", got, pane)
	}
}
...
func TestSuggestWorkflowsCommandInCatalog(t *testing.T) {
	...
	for _, c := range m.suggest.commands {
		if c.Name == "/workflows" && c.Surface == cli.SlashSurfaceTUI && c.AutoExecute {
			found = true
			break
		}
	}
...
func TestSkillSlashTurnSendsInstructionsButDisplaysOnlyLabel(t *testing.T) {
	...
	for _, want := range []string{cli.SkillTurnPreamble, "SECRET-LIKE-SKILL-BODY", "internal/cli"} {
	...
	for _, block := range cli.HydrateChatBlocksForView(m.session.MessagesCopy()) {
		if strings.Contains(block.Text, "SECRET-LIKE-SKILL-BODY") {
...
func TestUnknownSlashDoesNotStartTurn(t *testing.T) {
	...
	for _, block := range m.blocks {
		if block.Kind == cli.ChatBlockUser {
...
func TestSuggestRetainsAllLargeCatalogCandidatesAndRendersSelectedWindow(t *testing.T) {
	commands := make([]cli.SlashCommand, 300)
	for i := range commands {
		commands[i].Name = "/skill-" + strconv.Itoa(i)
		commands[i].Kind = cli.SlashKindSkill
	}
