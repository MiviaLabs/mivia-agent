...
func TestTUIMouseHitTranscriptBlockClick(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{
		{ID: "turn-1-block-1", Kind: cli.ChatBlockAssistant, Text: "Hello, world!"},
	}
	m.layout()
	m.renderVP()
	...
	if m.focus != cli.FocusScrollback {
		t.Errorf("focus=%v, want cli.FocusScrollback", m.focus)
	}
}
...
func TestTUIMouseOverLivePanelOverlayPassesThroughToTranscript(t *testing.T) {
	...
	if m.focus != cli.FocusScrollback {
		t.Fatalf("focus=%v, want cli.FocusScrollback after clicking through the overlay", m.focus)
	}
}
...
func TestTUIMouseDoubleClickTogglesWorkGroup(t *testing.T) {
	...
	plain1 := cli.StripANSI(strings.Join(m.messages, "\n"))
	...
	plain2 := cli.StripANSI(strings.Join(m.messages, "\n"))
...
func TestTUIMouseDoubleClickTogglesToolBlock(t *testing.T) {
	...
	m.ready = true
	m.blocks = []cli.ChatBlock{
		{ID: "t1", Kind: cli.ChatBlockTool, ToolName: "read_file", Text: "line1\nline2\nline3", Collapsed: true},
	}
	m.renderVP()
...
func TestTUIMouseDoubleClickTogglesThinkingBlock(t *testing.T) {
	...
	m.ready = true
	m.blocks = []cli.ChatBlock{
		{ID: "th-1", Kind: cli.ChatBlockThinking, Text: "line1\nline2\nline3", Collapsed: true},
	}
	m.renderVP()
	if !m.blocks[0].Collapsed {
		t.Fatal("start collapsed")
...
func TestTUIMouseHitComposerClick(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{
		{ID: "turn-1-block-1", Kind: cli.ChatBlockAssistant, Text: "Hello"},
	}
	m.layout()
	m.renderVP()
	m.View() // rebuilds the hit map
	...
	if m.focus != cli.FocusComposer {
		t.Errorf("focus=%v, want cli.FocusComposer after clicking composer", m.focus)
	}
}
...
func TestTUISidebarKeyboardNewSessionRefreshesTranscript(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(m.View()), "old sidebar transcript") {
		t.Fatal("precondition: old transcript is not visible")
	}

	m.Update(tea.KeyMsg{Type: tea.KeyEnter})

	view := cli.StripANSI(m.View())
...
func newSidebarNewSessionModel() *TUIModel {
	...
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockAssistant, Text: "old sidebar transcript"})
	m.renderVP()
	return m
}
