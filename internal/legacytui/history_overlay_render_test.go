...
func TestOverlayWindowShortTerminalFallback(t *testing.T) {
	...
	if r.H != 1 {
		t.Fatalf("renderOverlayWindow: expected cli.Rect h == 1 on a 1-row terminal, got %+v", r)
	}
	if !strings.Contains(cli.StripANSI(panel), "item-00") {
		t.Fatalf("renderOverlayWindow: fallback must show the selected row, panel:\n%s", cli.StripANSI(panel))
	}
}
...
func TestOverlayWindowEmpty(t *testing.T) {
	...
	if r != (cli.Rect{}) {
		t.Fatalf("renderOverlayWindow: expected a zero cli.Rect for nil rows, got %+v", r)
	}
}
...
// cli.StripANSI would strip CSI itself and make the check vacuous, so the raw
...
func TestHistoryOverlayRenderSanitizes(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(panel), "hiclear") {
...
func TestHistoryOverlayRenderFooterHint(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(panel), "enter recall") {
...
func TestHistoryOverlayRenderEmpty(t *testing.T) {
	...
	if r != (cli.Rect{}) {
		t.Fatalf("renderHistoryPanel: expected a zero cli.Rect for nil entries, got %+v", r)
	}
}
...
func TestHistoryOverlayRenderSelectedFollowsState(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(panel), "item-09") {
