// Package legacytui - Bubble Tea TUI for mivia chat (agent mode).
package legacytui
...
var (
	...
	// sites are unchanged.
	TUIThinkingStyle  = cli.TUIThinkingStyle
...
// Messages
// ---------------------------------------------------------------------------
type tuiTickMsg struct{ bridge *cli.StreamBridge }

// ---------------------------------------------------------------------------
// TUIModel is the Bubble Tea model driving the full-screen chat TUI.
// ---------------------------------------------------------------------------
...
// envMouse is the environment variable that forces mouse tracking on or off,
// overriding mouseAvailable's terminal-capability probe.
const envMouse = "MIVIA_MOUSE"
...
func newTUIModel(sess *chat.Session, res *config.Resolved, toolsOn bool) *TUIModel {
	...
	m := &TUIModel{
		...
		config:                res,
		toolsOn:               toolsOn,
		modelName:             cli.ShortenModel(sess.CurrentModel()),
		workspaceDir:          cli.ShortenWorkspacePath(),
		...
		subagents:             cli.NewSubagentTracker(),
	...
	m.prevAutoSaveWarn = cli.ReadAutosaveStatus(m.session.SessionDir)
...
func (m *TUIModel) toggleSelectedBlock() bool {
		...
		if v, ok := m.workGroupCollapsed[m.selectedBlockID]; ok {
			...
			for _, g := range cli.FindWorkGroups(m.blocks) {
				if g.Key == m.selectedBlockID {
					cur = cli.WorkGroupCollapsedDefault(g, nil)
					break
				}
			}
		}
	...
	for i := range m.blocks {
		...
		case cli.ChatBlockDivider, cli.ChatBlockUser, cli.ChatBlockAssistant:
			return true
		}
		...
		if cli.IsWorkStatusBlock(m.blocks[i]) && !m.blocks[i].Collapsed {
...
func (m *TUIModel) blockByID(id string) *cli.ChatBlock {
	for i := range m.blocks {
		if m.blocks[i].ID == id {
			return &m.blocks[i]
		}
...
func (m *TUIModel) adjustThinkingScroll(blockID string, dir int) bool {
	if blockID == "thinking-live" {
		...
		if n <= cli.MaxThinkingLines {
			return false
		}
		maxOffset := n - cli.MaxThinkingLines
	...
	if block == nil || block.Kind != cli.ChatBlockThinking {
		return false
	}
