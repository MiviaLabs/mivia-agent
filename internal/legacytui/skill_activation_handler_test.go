...
func TestResourceSkillInvocationGetsOnlyItsScopedReader(t *testing.T) {
	...
	dispatcher, err := cli.NewSessionDispatcherMinimal(registry, completer, "model", config.DefaultSubagentConfig, 0, skillRegistry)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTaskRoutedResourceSkillGetsScopedReader(t *testing.T) {
	...
	completer := &resourceSkillCompleter{}
	handler := cli.NewAgentTaskHandler(definition, digest,
		tools.NewDefaultRegistry(tools.DefaultOptions{Workspace: ws}), runtime.New(runtime.Policy{}),
		cli.SessionDispatcherOpts{Completer: completer, Model: "model", Config: config.DefaultSubagentConfig, SkillReg: skillRegistry})
...
func TestInjectSkillResourceToolConflict(t *testing.T) {
	...
	_, err := cli.InjectSkillResourceTool(reg, nil)
	if err == nil {
...
func TestInjectSkillResourceToolSuccess(t *testing.T) {
	reg := tools.NewRegistry()
	clone, err := cli.InjectSkillResourceTool(reg, nil)
	if err != nil {
		t.Fatal(err)
	}
	if clone == reg {
