...
func focusableBlockIDs(ranges map[string][2]int, blocks []cli.ChatBlock) []string {
	if len(ranges) == 0 {
		return nil
	}
	skip := map[string]bool{}
	for _, b := range blocks {
		if b.Kind == cli.ChatBlockDivider && b.ID != "" {
			skip[b.ID] = true
		}
	}
	type item struct {
...
func (m *TUIModel) ensureSelectedVisible() {
	...
	if r[1] > bottom {
		m.viewport.YOffset = cli.Max(0, r[1]-h)
	}
}
...
func applySelectionChrome(lines []string, ranges map[string][2]int, selectedID string, focused bool) []string {
	...
	for i := start; i < end; i++ {
		plain := cli.StripANSI(out[i])
		if strings.TrimSpace(plain) == "" {
			continue
		}
		// Preserve width: pad plain to original visible width then paint bg.
		vis := cli.VisibleWidth(out[i])
		if vis < 1 {
			vis = len([]rune(plain))
		}
		row := plain
		if cli.VisibleWidth(row) < vis {
			row += strings.Repeat(" ", vis-cli.VisibleWidth(row))
		}
