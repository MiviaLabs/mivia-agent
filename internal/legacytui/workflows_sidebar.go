...
func (s *workflowsSidebar) clampScroll(rows []workflowRunRow, space int) {
	...
	s.scroll = cli.Min(cli.Max(0, s.scroll), cli.Max(0, last-space))
}
...
func (s *workflowsSidebar) renderRunRow(row workflowRunRow, selected bool, width int, focused bool) []string {
	...
	name := cli.TruncateToWidth(row.run.WorkflowName, budget)
	...
	metadata := "   step " + cli.TruncateToWidth(step, cli.Max(1, width-8))
	lines := []string{line, TUIDimStyle.Render(sidebarPad(metadata, width))}
	if selected {
		if row.description != "" {
			lines = append(lines, TUIDimStyle.Render(sidebarPad("   "+cli.TruncateToWidth(row.description, cli.Max(1, width-4)), width)))
		}
		if row.nextStep != "" {
			lines = append(lines, TUIDimStyle.Render(sidebarPad("   next: "+cli.TruncateToWidth(row.nextStep, cli.Max(1, width-10)), width)))
		}
	}
	return lines
}
