...
const (
	// MinCardWidth is relocated to internal/cli (needed there by the
	// chat-block renderer); aliased here so this package's own call sites
	// are unchanged.
	MinCardWidth        = cli.MinCardWidth
...
func composerMaxHeight(termH int) int {
	return cli.Min(5, cli.Max(1, termH/6))
}
...
func composerTopBorder(width int, border lipgloss.Style) string {
	return border.Render("┌" + strings.Repeat("─", cli.Max(1, width-2)) + "┐")
}
