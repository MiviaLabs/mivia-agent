...
// LatestAutoSaveName, DisplaySessionName, formatSessionAge are relocated to
// internal/cli (needed there for conversation collapsing); aliased here so
// this package's own call sites are unchanged.
var (
	LatestAutoSaveName = cli.LatestAutoSaveName
	DisplaySessionName = cli.DisplaySessionName
	formatSessionAge   = cli.FormatSessionAge
)
...
func normalizePickerSize(width, maxRows int) (int, int) {
	return cli.Max(20, width), cli.Max(1, maxRows)
}

func normalizePickerSelection(selected, scroll, maxRows, sessionCount int) (int, int) {
	selected = cli.Min(cli.Max(selected, 0), sessionCount-1)
	scroll = cli.Max(0, scroll)
	if selected < scroll {
		scroll = selected
	}
	...
	return selected, cli.Min(scroll, cli.Max(0, sessionCount-maxRows))
}
...
func renderSessionRows(sessions []chat.SessionInfo, selected, scroll, width, maxRows, yBase int) ([]string, []sessionRowHit, int) {
	...
	end := cli.Min(len(sessions), scroll+maxRows)
	for i := scroll; i < end; i++ {
		...
		nameWidth := cli.Max(1, width-cli.RuneWidth(prefix)-cli.RuneWidth(meta)-2)
		if cli.RuneWidth(name) > nameWidth {
			name = cli.TruncateToWidth(name, cli.Max(1, nameWidth-1)) + "…"
		}
		line := prefix + name + strings.Repeat(" ", cli.Max(0, nameWidth-cli.RuneWidth(name))) + "  " + meta
		lines = append(lines, style.Render(cli.TruncateToWidth(line, width)))
...
func (m *TUIModel) hydrateHistory() {
	...
	for _, block := range cli.HydrateChatBlocksForView(msgs[start:]) {
		m.appendBlock(block)
	}
	m.renderVP()
}
...
func (m *TUIModel) resetForNewSession() error {
	...
	cli.SetActiveSessionCaller(runtime.Caller{SessionID: newID})
...
func (m *TUIModel) restoreSessionDir(dir string) {
	...
	m.workspaceDir = cli.ShortenWorkspacePath()
	m.refreshGitContext()
	m.appendInfo(fmt.Sprintf("switched to %s", cli.ShortenWorkspacePath()))
}
