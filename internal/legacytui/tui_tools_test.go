...
func TestApplyToolEventsFocusedPreservesSelection(t *testing.T) {
	...
	m.applyToolEvents([]cli.BridgeToolEvt{{
		Start:  true,
		Name:   "new-tool",
		Detail: "args",
		At:     time.Now(),
...
func TestApplyToolEventsMatchesDuplicateNamesByCallID(t *testing.T) {
	...
	found := false
	for _, b := range m.blocks {
		if b.Kind == cli.ChatBlockTool && b.ToolCallID == "call-a" {
			found = true
	...
	if !found {
		t.Fatal("expected call-a as cli.ChatBlockTool in history")
	}
}
...
func TestApplyToolEndEvent_UppercaseFailedLifecycleSetsFailed(t *testing.T) {
	...
	if got := m.applyToolEndEvent(cli.BridgeToolEvt{ToolCallID: "c1", Name: "run_command", Detail: "FAILED", At: time.Now()}); got != 0 {
...
func TestPruneBannerDoesNotStayActive(t *testing.T) {
	...
	found := false
	for _, b := range m.blocks {
		if b.Kind == cli.ChatBlockTool && b.ToolName == "prune" {
			found = true
			break
		}
	}
	if !found {
		t.Fatal("expected prune cli.ChatBlock in history")
	}
	if got := m.bridge.ActiveTools(); got != 0 {
...
func TestChatTimelineProgressiveBlocks(t *testing.T) {
	...
	m.waiting = true
	m.turnStart = time.Now()
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockUser, Text: "what next", SentAt: time.Now()})
	...
	if !hasToolBlock(m.blocks, "list_dir") {
		t.Fatal("expected list_dir cli.ChatBlock in history")
	}
	...
	if !kindOrderContains(kinds,
		cli.ChatBlockUser, cli.ChatBlockAssistant, cli.ChatBlockTool,
		cli.ChatBlockAssistant, cli.ChatBlockTool, cli.ChatBlockAssistant, cli.ChatBlockDivider,
	) {
		t.Fatalf("unexpected multi-bubble timeline: %v", kinds)
	}
}

func hasAssistantText(blocks []cli.ChatBlock, substr string) bool {
	for _, b := range blocks {
		if b.Kind == cli.ChatBlockAssistant && strings.Contains(b.Text, substr) {
			return true
		}
	}
	return false
...
func hasBlockKind(blocks []cli.ChatBlock, k cli.ChatBlockKind) bool {
	for _, b := range blocks {
		if b.Kind == k {
			return true
		}
...
func hasToolBlock(blocks []cli.ChatBlock, name string) bool {
	for _, b := range blocks {
		if b.Kind == cli.ChatBlockTool && b.ToolName == name {
			return true
		}
	}
	return false
...
func blockKinds(blocks []cli.ChatBlock) []cli.ChatBlockKind {
	out := make([]cli.ChatBlockKind, len(blocks))
...
func kindOrderContains(have []cli.ChatBlockKind, want ...cli.ChatBlockKind) bool {
	i := 0
	for _, h := range have {
...
func TestOnEventForMultiStepForwardsHeartbeat(t *testing.T) {
	t.Parallel()
	var got []agent.Event
	fn := cli.OnEventForMultiStep(func(e agent.Event) { got = append(got, e) })
