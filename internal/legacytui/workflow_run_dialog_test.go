...
func TestWorkflowRunDialogOpenShowsLoadingPlaceholder(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(panel), "loading run details") {
...
func TestWorkflowRunDialogGeometry(t *testing.T) {
	...
	for _, size := range [][2]int{{1, 1}, {2, 8}, {24, 2}, {90, 24}} {
		...
		for _, line := range strings.Split(cli.StripANSI(panel), "\n") {
			if cli.RuneWidth(line) > w {
				t.Fatalf("%dx%d: line width %d exceeds canvas: %q", w, h, cli.RuneWidth(line), line)
			}
		}
		if layout.Rect.W > w || layout.Rect.H > h {
			t.Fatalf("%dx%d: dialog cli.Rect %dx%d exceeds the canvas", w, h, layout.Rect.W, layout.Rect.H)
		}
	}
}
