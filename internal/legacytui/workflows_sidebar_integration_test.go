...
func TestIntegrationWorkflowsSidebarOpensAndRefreshes(t *testing.T) {
	...
	if m.focus != cli.FocusWorkflowsSidebar {
		t.Fatalf("focus = %v, want cli.FocusWorkflowsSidebar", m.focus)
	}
...
func TestIntegrationWorkflowRunDialogOpensByEnter(t *testing.T) {
	...
	if !strings.Contains(cli.StripANSI(m.workflowRunDlg.footer()), "c cancel") {
		t.Fatalf("pending run footer missing cancel:\n%s", cli.StripANSI(m.workflowRunDlg.footer()))
	}
}
