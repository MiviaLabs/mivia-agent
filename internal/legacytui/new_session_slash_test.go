...
func TestHandleSlashNewClassic(t *testing.T) {
	...
	handled, exit, err := cli.HandleSlash("/new", sess, nil, false, nil)
	if err != nil {
		t.Fatalf("cli.HandleSlash /new error: %v", err)
	}
	if !handled {
...
func TestNewInHelpSurfaces(t *testing.T) {
	// TUI help content.
	for _, sec := range cli.TuiHelpCommands() {
	...
	t.Fatal("/new not found in cli.TuiHelpCommands")
classic:
	// Classic REPL help content (catalog-driven sections).
	found := false
	for _, sec := range cli.ReplHelpContent() {
		for _, it := range sec.Items {
			if strings.Contains(it.Key, "/new") {
				found = true
			}
	...
	if !found {
		t.Fatal("/new not found in cli.ReplHelpContent")
	}
	// Inline/stderr path uses the same content structure.
	if !strings.Contains(cli.RenderReplHelpInline(), "/new") {
		t.Fatal("/new not found in cli.RenderReplHelpInline")
	}
}
