...
func (m *TUIModel) layout() {
	...
	inputHeight := cli.Min(composerMaxHeight(m.height), cli.Max(1, m.textarea.LineCount()))
	...
	if !m.ready {
		...
		m.viewport = newTranscriptViewport(cli.Max(1, chatWidth), vpHeight)
		m.textarea.SetWidth(composerInnerWidth(chatWidth))
		m.ready = true
	} else {
		m.viewport.Width = cli.Max(1, chatWidth)
...
func (m *TUIModel) updateFromDrain(d cli.BridgeDrain) {
	...
	if interim := strings.TrimSpace(d.Interim); cli.ShouldCommitInterim(interim) {
		m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockAssistant, Text: interim})
...
func (m *TUIModel) finishStream(err error) []tea.Cmd {
	...
	if strings.TrimSpace(raw) != "" {
		m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockAssistant, Text: raw})
	}
...
func (m *TUIModel) appendTurnFooter(err error, total time.Duration) {
	if err != nil && err != context.Canceled {
		text := "error: " + cli.SafeChatBlockText(err.Error(), 240)
		m.appendBlock(cli.ChatBlock{
			Kind:     cli.ChatBlockDivider,
			Text:     text,
...
func (m *TUIModel) appendMsg(s string) {
	m.appendBlock(cli.ChatBlock{Kind: cli.ChatBlockSystem, Text: s, Rendered: s})
}
...
func (m *TUIModel) turnActionTally() (tools, agents, failed int) {
	for i := len(m.blocks) - 1; i >= 0; i-- {
		...
		if cli.ActionKindForTool(b.ToolName) == cli.ActionAgent {
			agents++
		} else {
			tools++
		}
