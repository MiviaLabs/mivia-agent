...
func fourToolWorkBlocks() []cli.ChatBlock {
	return []cli.ChatBlock{
		{ID: "u1", Kind: cli.ChatBlockUser, Text: "do work"},
		{ID: "t1", Kind: cli.ChatBlockTool, ToolName: "read_file", Text: "a", Collapsed: true},
		{ID: "t2", Kind: cli.ChatBlockTool, ToolName: "run_command", Text: "b", Collapsed: true},
		{ID: "t3", Kind: cli.ChatBlockTool, ToolName: "grep", Text: "c", Collapsed: true},
		{ID: "t4", Kind: cli.ChatBlockTool, ToolName: "glob", Text: "d", Collapsed: true},
		{ID: "a1", Kind: cli.ChatBlockAssistant, Text: "done answer"},
	}
}
...
func TestIntegration_WorkGroup_RendersEvenWhenCollapseMapNil(t *testing.T) {
	...
	if _, ok := r.Ranges[cli.FindWorkGroups(m.blocks)[0].Key]; !ok {
...
func TestIntegration_WorkGroup_ToggleExpandCollapse(t *testing.T) {
	...
	r0 := m.renderBlocksForView()
	plain0 := cli.StripANSI(strings.Join(r0.Lines, "\n"))
	...
	r1 := m.renderBlocksForView()
	plain1 := cli.StripANSI(strings.Join(r1.Lines, "\n"))
