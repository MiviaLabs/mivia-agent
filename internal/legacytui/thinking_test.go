...
func TestRenderThinkingBlockWrapsLongLinesToWidth(t *testing.T) {
	...
	lines := cli.RenderThinkingBlock(longLine, false, 0, false, 40)
	for _, l := range lines {
		if w := lipgloss.Width(cli.StripANSI(l)); w > 40 {
...
func TestLivePanelShowsRollingThinkingWindow(t *testing.T) {
	...
	panel := cli.StripANSI(m.renderLivePanel(90, time.Now()))
...
func TestCollapsedThinkingBlockSummarises(t *testing.T) {
	...
	lines := cli.RenderThinkingBlock(strings.TrimRight(text, "\n"), true, 0, false, 80)
	plain := cli.StripANSI(strings.Join(lines, "\n"))
...
func TestFlushThinkingCommitsCollapsedSummaryOnly(t *testing.T) {
	...
	b := m.blocks[0]
	if b.Kind != cli.ChatBlockThinking {
		t.Fatalf("kind=%s, want thinking", b.Kind)
	}
	if !b.Collapsed {
		t.Fatal("committed thinking must be collapsed by default")
	}
	out := cli.StripANSI(strings.Join(cli.RenderChatBlocksWithWorkGroups([]cli.ChatBlock{b}, "m", 90, m.thinkingExpandDefault, nil).Lines, "\n"))
...
func TestFlushThinkingEmptyAppendsNothing(t *testing.T) {
	...
	out := cli.StripANSI(strings.Join(cli.RenderThinkingBlock("", false, 0, false, 80), "\n"))
...
func TestCtrlTTogglesGlobalDefaultAndSelectedBlock(t *testing.T) {
	...
	m.blocks = []cli.ChatBlock{{ID: "th-1", Kind: cli.ChatBlockThinking, Text: "reasoning", Collapsed: true}}
