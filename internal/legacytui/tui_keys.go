...
func (m *TUIModel) handleWelcomeKey(key string) bool {
	...
	if nav {
		...
		case "pgup":
			m.sessionSel = cli.Max(0, m.sessionSel-10)
		case "pgdown":
			m.sessionSel = cli.Min(len(m.sessions)-1, m.sessionSel+10)
			if m.sessionSel < 0 {
				m.sessionSel = 0
			}
		case "home":
...
func (m *TUIModel) handleBlockActionKey(key string) (bool, []tea.Cmd) {
	scrollback := m.focus == cli.FocusScrollback
	switch {
...
func (m *TUIModel) handleChatKey(key string, alt bool) (bool, bool, []tea.Cmd) {
	...
	if key == "enter" || key == " " {
		if m.focus == cli.FocusScrollback && m.toggleSelectedBlock() {
	...
	focus, consumed := cli.RouteFocusKey(m.focus, key)
	m.setFocus(focus)
	...
	return skipTextarea, skipViewport || focus != cli.FocusScrollback, cmds
}
...
func (m *TUIModel) handleChatToggleKey(key string) []tea.Cmd {
	...
	case "ctrl+t":
		...
		if m.selectedBlockID != "" {
			if block := m.blockByID(m.selectedBlockID); block != nil && block.Kind == cli.ChatBlockThinking {
...
func (m *TUIModel) handleChatControlKey(key string, alt, skipTextarea bool) (bool, bool, []tea.Cmd) {
	...
	case "home", "shift+home":
		...
		if key == "home" && m.focus == cli.FocusComposer {
			break
		}
		m.viewport.GotoTop()
	...
	case "end", "shift+end":
		...
		if key == "end" && m.focus == cli.FocusComposer && m.textarea.Value() != "" {
			break
		}
		m.jumpToLatest()
		m.renderVP()
