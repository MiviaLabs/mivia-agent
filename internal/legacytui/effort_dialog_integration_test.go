		...
		for _, size := range []struct{ width, height int }{{1, 1}, {2, 8}, {24, 2}, {90, 24}} {
			...
			for _, line := range strings.Split(view, "\n") {
				if ansi.StringWidth(line) > layout.Rect.W {
					t.Fatalf("%s %dx%d line width=%d cli.Rect=%d: %q",
						model, size.width, size.height, ansi.StringWidth(line), layout.Rect.W, cli.StripANSI(line))
				}
			}
		}
	}
...
func TestIntegrationEffortSlashOnThePlainSurface(t *testing.T) {
	...
	out := &strings.Builder{}
	term := cli.NewTestTerminal(out)

	if _, _, err := cli.HandleSlash("/effort", sess, res, false, term); err != nil {
		t.Fatal(err)
	}
	summary := out.String()
	...
	out.Reset()
	if _, _, err := cli.HandleSlash("/effort low", sess, res, false, term); err != nil {
		t.Fatal(err)
	}
	...
	out.Reset()
	if _, _, err := cli.HandleSlash("/effort xhigh", sess, res, false, term); err != nil {
		t.Fatal(err)
	}
	if got := sess.ReasoningEffort(); got != reasoning.Low {
		t.Fatalf("a refused level changed the effort to %q", got)
...
func TestIntegrationEffortSlashPlainEmptyState(t *testing.T) {
	...
	sess := chat.NewSession(res, welcomeStubCompleter{})
	out := &strings.Builder{}
	if _, _, err := cli.HandleSlash("/effort", sess, res, false, cli.NewTestTerminal(out)); err != nil {
		t.Fatal(err)
	}
