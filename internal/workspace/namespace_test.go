...
var namespaceAllowlist = map[string]string{
	...
	"internal/config/load.go\x00return \"\", ProviderConfig{}, \"\", fmt.Errorf(\"provider %q is not configured: no [providers.%s] section in the active config (declare it there or in ~/.mivia/mivia.toml; run 'mivia setup')\", name, name)": "user-facing config error pointing the operator at the user config file",
