...
var namespaceAllowlist = map[string]string{
	"internal/hooks/session/session.go\x00const hookProjectNotice = \"hooks marked [project] came from this workspace's .mivia/mivia.toml, not from your \" +":                                                                                           "user-facing /hooks notice naming the workspace hook config file",
	"internal/hooks/session/session.go\x00\"workspace's .mivia/mivia.toml runs. Delete or comment out the [[hooks]] entry to stop one.\"":                                                                                                                "user-facing /hooks trust explanation naming the workspace hook config file",
	"internal/hooks/session/session.go\x00\"a hook declared in ~/.mivia/mivia.toml runs without confirmation.\")":                                                                                                                                        "user-facing --bypass-hook-trust notice naming the user hook config file",
	"internal/hooks/session/session.go\x00return \"no lifecycle hooks configured (they load from ~/.mivia/mivia.toml and <workspace>/.mivia/mivia.toml)\"":                                                                                               "user-facing /hooks empty listing naming both hook config surfaces",
	...
	"internal/config/load_provider.go\x00return \"\", ProviderConfig{}, \"\", fmt.Errorf(\"provider %q is not configured: no [providers.%s] section in the active config (declare it there or in ~/.mivia/mivia.toml; run 'mivia setup')\", name, name)": "user-facing config error pointing the operator at the user config file",
	"internal/tui/view/screen/settings/agents_detail.go\x00originLabel = \"Global (user home: ~/.mivia/agents/\" + ag.Name + \".md)\"":                                                                                                                   "user-facing agent detail origin label naming user home agent path",
	"internal/tui/view/screen/settings/projects.go\x00description: \"Project configuration file path (.mivia/mivia.toml).\",":                                                                                                                            "user-facing project setting description naming project config path",
	"internal/tui/view/screen/settings/projects.go\x00description: \"SQLite database file path (leave empty to use default .mivia/context.db).\",":                                                                                                       "user-facing project setting description naming default storage path",
	"internal/tui/view/screen/settings/projects_detail.go\x00scopeLabel := \"Project (.mivia/mivia.toml)\"":                                                                                                                                              "user-facing project setting detail scope label naming project config file",
}
