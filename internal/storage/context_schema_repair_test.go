...
func seedContextCrashState(t *testing.T, v int) *sql.DB {
	...
	apply := []func(*sql.DB) error{applyContextSchemaV1, applyContextSchemaV2, applyContextSchemaV3, applyContextSchemaV4, applyContextSchemaV5}
...
func assertContextSchemaClean(t *testing.T, db *sql.DB) {
	...
	for _, table := range []string{"context_sessions", "chat_sessions", "chat_session_admissions", "chat_session_dirs"} {
		var count int
