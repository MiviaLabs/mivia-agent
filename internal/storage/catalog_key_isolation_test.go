...
func TestGenericCatalogAPIsRejectManagedStorageKey(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(context.Context, *SQLite, state.Principal, string) error
	}{
		{"save snapshot", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			return store.SaveSession(ctx, principal, key, []byte(`[{"unbound":true}]`), "other", "other", 9, 9, 1, state.SessionSaveOptions{Dir: "/unbound"})
		}},
		{"load snapshot", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			_, _, err := store.LoadSession(ctx, principal, key)
			return err
		}},
		{"delete snapshot", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			return store.DeleteSessionSnapshot(ctx, principal, key)
		}},
		{"prune snapshot", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			return store.PruneSessionSnapshots(ctx, principal, []string{key})
		}},
		{"save admission", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			return store.SaveSessionAdmission(ctx, principal, key, state.SessionAdmission{Agent: "other", Digest: "other", Names: []string{"other"}})
		}},
		{"delete admission", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			return store.SaveSessionAdmission(ctx, principal, key, state.SessionAdmission{})
		}},
		{"load admission", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			_, err := store.LoadSessionAdmission(ctx, principal, key)
			return err
		}},
		{"ensure directory", func(ctx context.Context, store *SQLite, principal state.Principal, key string) error {
			unbound, err := state.NewPrincipal(principal.WorkspaceID, key, principal.SubjectID)
			if err != nil {
				return err
			}
			return store.EnsureSession(ctx, state.EnsureSessionRequest{Principal: unbound, Binding: mustBinding(t), Dir: "/unbound"})
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			defer store.Close()
			if err := test.mutate(context.Background(), store, principal, key); !errors.Is(err, state.ErrWorktreeDeleted) {
...
func seedManagedCatalogKey(t *testing.T) (*SQLite, state.Principal, state.WorktreeInstance, string) {
	t.Helper()
	...
	if err := store.SaveSession(context.Background(), principal, "managed", []byte(`[{"managed":true}]`), "model", "provider", 1, 2, 1, state.SessionSaveOptions{Dir: "/managed", Worktree: instance.Worktree, WorktreeInstance: instance}); err != nil {
		store.Close()
		t.Fatal(err)
	}
	if err := store.SaveWorktreeSessionAdmission(context.Background(), principal, "managed", state.SessionAdmission{Agent: "managed", Digest: "managed", Names: []string{"managed"}}, instance); err != nil {
		store.Close()
		t.Fatal(err)
	}
	var key string
...
func assertManagedCatalogKeyState(t *testing.T, store *SQLite, principal state.Principal, instance state.WorktreeInstance, key string) {
	t.Helper()
