...
func (s *SQLite) DeleteSession(ctx context.Context, principal state.Principal, sessionID string) (state.DeleteResult, error) {
	if sessionID != principal.SessionID {
		return state.DeleteResult{}, state.ErrPrincipalMismatch
	}
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	tx, err := s.beginWrite(ctx)
	if err != nil {
		return state.DeleteResult{}, err
	}
	row, authErr := authorizeUnboundContextSessionTx(ctx, tx, principal, sessionID)
	if errors.Is(authErr, state.ErrSessionTombstoned) {
		_ = tx.Rollback()
		return s.existingDeleteResult(ctx, principal)
	}
	if authErr != nil {
		_ = tx.Rollback()
		return state.DeleteResult{}, authErr
	}
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO context_audits(audit_id,action,workspace_id,session_id,subject_id,revision,size,retention_class,expires_at,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`, auditID, string(state.AuditDelete), principal.WorkspaceID, sessionID, principal.SubjectID, row.SessionRevision+1, 0, string(state.RetentionCompliance), expires, created); err != nil {
		_ = tx.Rollback()
		return state.DeleteResult{}, err
	}
	if _, err := tx.ExecContext(ctx, `INSERT INTO context_tombstones(session_id,workspace_id,subject_id,revision,retention_class,expires_at,audit_id,created_at) VALUES(?,?,?,?,?,?,?,?)`, sessionID, principal.WorkspaceID, principal.SubjectID, row.SessionRevision+1, string(state.RetentionCompliance), expires, auditID, created); err != nil {
		_ = tx.Rollback()
		return state.DeleteResult{}, err
	}
	...
	return state.DeleteResult{SessionID: sessionID, TombstoneRevision: state.Revision{Session: row.SessionRevision + 1, Durable: row.DurableRevision, Source: row.SourceSequence}, RevokedRefs: int(revoked), AuditID: auditID}, nil
}

func (s *SQLite) ExportSession(ctx context.Context, principal state.Principal, sessionID string) (state.ExportResult, error) {
	if sessionID != principal.SessionID {
		return state.ExportResult{}, state.ErrPrincipalMismatch
	}
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	tx, err := s.beginWrite(ctx)
	if err != nil {
		return state.ExportResult{}, err
	}
	...
	envelope := contextExportEnvelope{Version: 1, SessionID: sessionID, Revision: state.Revision{Session: row.SessionRevision, Durable: row.DurableRevision, Source: row.SourceSequence}, Source: source, Payloads: payloads}
	records, err := state.MarshalCanonical(envelope)
	if err != nil {
		_ = tx.Rollback()
		return state.ExportResult{}, err
	}
	if state.Exceeds(len(records), state.CurrentLimits().ExportBytes) {
		_ = tx.Rollback()
		return state.ExportResult{}, state.ErrExportTooLarge
	}
	auditID, err := newContextID("ctxaudit_")
	if err != nil {
		_ = tx.Rollback()
	...
	created, expires := retentionWindow()
	if _, err := tx.ExecContext(ctx, `INSERT INTO context_audits(audit_id,action,workspace_id,session_id,subject_id,revision,size,retention_class,expires_at,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`, auditID, string(state.AuditExport), principal.WorkspaceID, sessionID, principal.SubjectID, row.DurableRevision, len(records), string(state.RetentionCompliance), expires, created); err != nil {
		_ = tx.Rollback()
		return state.ExportResult{}, err
	}
	if err := tx.Commit(); err != nil {
	...
	return state.ExportResult{SessionID: sessionID, Revision: envelope.Revision, Records: records, Count: len(source) + len(payloads), AuditID: auditID}, nil
}
...
type contextExportEnvelope struct {
	...
	Source    []state.SourceEvent    `json:"source"`
	Payloads  []contextExportPayload `json:"payloads"`
}

type contextExportPayload struct {
	Ref       state.ContentRef     `json:"ref"`
	Bytes     []byte               `json:"bytes,omitempty"`
	HashOnly  bool                 `json:"hash_only"`
	Retention state.RetentionClass `json:"retention"`
}
...
func readContextSourceEvents(ctx context.Context, q contextQueryer, principal state.Principal) ([]state.SourceEvent, error) {
...
func readContextExportPayloads(ctx context.Context, q contextQueryer, principal state.Principal) ([]contextExportPayload, error) {
	...
	for _, row := range parents {
		contentRef := state.ContentRef{Ref: row.ref, Namespace: row.namespace, SHA256: row.digest, WorkspaceID: principal.WorkspaceID, SessionID: principal.SessionID, SubjectID: principal.SubjectID, Size: row.size}
		...
		if data != nil {
			...
			if len(data) != row.size || hex.EncodeToString(payloadDigest[:]) != row.digest {
				return nil, fmt.Errorf("%w: export payload digest mismatch", state.ErrInvalidDTO)
			}
		}
		out = append(out, contextExportPayload{Ref: contentRef, Bytes: append([]byte(nil), data...), HashOnly: data == nil, Retention: state.RetentionClass(row.retention)})
	}
	return out, nil
}

func (s *SQLite) existingDeleteResult(ctx context.Context, principal state.Principal) (state.DeleteResult, error) {
	var result state.DeleteResult
	...
	if err == sql.ErrNoRows {
		return state.DeleteResult{}, state.ErrSessionNotFound
	}
	if err != nil {
		return state.DeleteResult{}, err
	}
	result.SessionID = principal.SessionID
	result.TombstoneRevision = state.Revision{Session: sessionRevision, Durable: durableRevision, Source: sourceSequence}
...
func (s *SQLite) PruneContextPayloads(ctx context.Context, now time.Time, limit int) (int, error) {
	if limit <= 0 || limit > 10_000 {
		return 0, fmt.Errorf("%w: invalid payload GC limit", state.ErrInvalidDTO)
	}
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	tx, err := s.beginWrite(ctx)
