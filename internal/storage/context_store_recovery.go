...
func (r contextSessionRow) Revision() state.Revision {
	return state.Revision{Session: r.SessionRevision, Durable: r.DurableRevision, Source: r.SourceSequence}
}

func (r contextSessionRow) Binding() state.BindingRevision {
	return state.BindingRevision{Provider: r.Provider, Model: r.Model, Generation: r.BindingGeneration}
}

func readContextHead(ctx context.Context, q contextRowQueryer, principal state.Principal) (contextHead, error) {
...
func readContextHeadTx(ctx context.Context, tx *sql.Tx, principal state.Principal) (contextHead, error) {
...
func scanContextHead(row *sql.Row, principal state.Principal) (contextHead, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return contextHead{}, state.ErrSessionNotFound
	}
	if err != nil {
		return contextHead{}, err
	}
	if subjectID != principal.SubjectID || capability != principal.CapabilityDigest() {
		return contextHead{}, state.ErrPrincipalMismatch
	}
	head.Tombstoned = tombstoned != 0
	return head, nil
}

func recoverActive(ctx context.Context, q contextRowQueryer, principal state.Principal, head contextHead) (state.CheckpointRecord, bool, error) {
	if head.ActiveCheckpointID.Valid {
		...
		if err != nil && !errors.Is(err, sql.ErrNoRows) {
			return state.CheckpointRecord{}, false, err
		}
	}
	...
	if errors.Is(err, sql.ErrNoRows) {
		return state.CheckpointRecord{}, false, nil
	}
	if err != nil {
		return state.CheckpointRecord{}, false, err
	}
...
func readCheckpoint(ctx context.Context, q contextRowQueryer, principal state.Principal, provider, model, predicate string, args ...any) (state.CheckpointRecord, error) {
	...
	rng := state.SourceRange{Start: state.SourceID{SessionID: principal.SessionID, Sequence: start}, End: state.SourceID{SessionID: principal.SessionID, Sequence: end}}
	id := state.CheckpointID{SessionID: principal.SessionID, SourceRange: rng, Algorithm: algorithm, SchemaVersion: schemaVersion, SummaryModel: summaryModel, IdempotencyKey: key}
	checkpoint := state.CheckpointRecord{ID: id, Revision: state.Revision{Session: sessionRevision, Durable: durableRevision, Source: end}, Binding: state.BindingRevision{Provider: provider, Model: model, Generation: generation}, SourceRange: rng, ActiveContext: append([]byte(nil), active...), SummaryMetadata: append([]byte(nil), metadata...), TurnID: turnID, Complete: complete != 0}
	if complete != 0 {
...
func validateStoredCheckpoint(checkpoint state.CheckpointRecord, fingerprint string) error {
	if len(fingerprint) != 64 {
		return fmt.Errorf("%w: checkpoint fingerprint is invalid", state.ErrInvalidDTO)
	}
	...
	if hex.EncodeToString(digest[:]) != strings.ToLower(fingerprint) {
		return fmt.Errorf("%w: checkpoint fingerprint does not match active context", state.ErrInvalidDTO)
	}
...
func checkpointStorageID(id state.CheckpointID) (string, error) {
	data, err := state.MarshalCanonical(id)
	if err != nil {
		return "", err
	}
