...
// TestHideCoveredWorktreeRoutes_OnlyInstanceBoundRowsCover pins the
// coverage rule the resume-routing gate depends on: a session row with
// bare worktree metadata and NO managed instance (legacy pre-instance
// sessions) resumes PLAIN, so it must not hide the route pseudo-row -
// that row is the only way to start an instance-scoped session in the
// worktree. Only a row that itself carries the instance covers the route.
func TestHideCoveredWorktreeRoutes_OnlyInstanceBoundRowsCover(t *testing.T) {
	route := contextstate.SessionCatalogInfo{
		Name: "worktree:wt-a", Worktree: "wt-a", Dir: "/repo/.mivia/worktrees/wt-a",
		WorktreeRoute: true,
	}
	legacy := contextstate.SessionCatalogInfo{
		Name: "old-session", Worktree: "wt-a", Dir: "/repo/.mivia/worktrees/wt-a",
	}
	bound := contextstate.SessionCatalogInfo{
		Name: "bound-session", Worktree: "wt-a", Dir: "/repo/.mivia/worktrees/wt-a",
		WorktreeInstance: contextstate.WorktreeInstance{Worktree: "wt-a", ID: "wt_1234567890abcdef"},
	}

	kept := hideCoveredWorktreeRoutes([]contextstate.SessionCatalogInfo{legacy, route})
	routeSurvives := false
	for _, info := range kept {
		routeSurvives = routeSurvives || info.WorktreeRoute
	}
	if !routeSurvives {
		t.Fatalf("legacy (instance-less) row hid the route pseudo-row: %+v", kept)
	}

	kept = hideCoveredWorktreeRoutes([]contextstate.SessionCatalogInfo{bound, route})
	for _, info := range kept {
		if info.WorktreeRoute {
			t.Fatalf("instance-bound row failed to cover the route pseudo-row: %+v", kept)
		}
	}
}
