...
type TaskSnapshot struct {
	...
	//
	// Only fields describing the WORK live here. Authority — permission, scope,
	// role, session, owner — is deliberately absent: the ledger is a file in the
	// workspace and the agent has file tools, so a persisted permission would be
	// a privilege grant the agent could write for itself. Resume re-derives
	// those from the caller performing it. See plan 12 §3.
