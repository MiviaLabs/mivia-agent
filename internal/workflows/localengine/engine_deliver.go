...
// Deliver implements workflowagenttools.Engine.
func (e *Engine) Deliver(ctx context.Context, runID string, allowPublish bool) (workflowagenttools.DeliverResult, error) {
	if e == nil || e.Repo == nil {
		return workflowagenttools.DeliverResult{}, fmt.Errorf("workflow engine is incomplete")
	}
	if !allowPublish {
		...
		return workflowagenttools.DeliverResult{RunID: runID, Refused: true, Reason: "delivery requires allow_publish=true"}, nil
	}
	run, err := e.Repo.GetRun(ctx, runID)
	if err != nil {
		if errors.Is(err, workflowledger.ErrNotFound) {
			return workflowagenttools.DeliverResult{}, fmt.Errorf("workflow run %q not found", runID)
		}
		return workflowagenttools.DeliverResult{}, err
	}
	...
	if run.Status != workflowledger.RunStatusDeliveryPending && run.Status != workflowledger.RunStatusDeliveryFailed {
		return workflowagenttools.DeliverResult{}, fmt.Errorf("run is not waiting for delivery (status %q)", run.Status)
	}
	return e.deliverPending(ctx, run)
}

func (e *Engine) replayDelivery(ctx context.Context, run workflowledger.RunSnapshot) (workflowagenttools.DeliverResult, error) {
	...
	if err != nil {
		...
		return workflowagenttools.DeliverResult{}, fmt.Errorf("replay delivery for %q: %w", run.RunID, err)
	}
	return workflowagenttools.DeliverResult{RunID: run.RunID, Status: string(run.Status), URL: rec.URL, Mode: rec.Mode}, nil
}

func (e *Engine) deliverPending(ctx context.Context, run workflowledger.RunSnapshot) (workflowagenttools.DeliverResult, error) {
...
func (e *Engine) deliverPendingDirect(ctx context.Context, run workflowledger.RunSnapshot) (workflowagenttools.DeliverResult, error) {
...
func (e *Engine) deliverPendingWithStackGate(ctx context.Context, run workflowledger.RunSnapshot, enforceStackGate bool) (workflowagenttools.DeliverResult, error) {
	runID := run.RunID
	...
	if enforceStackGate {
		if reason := e.undrivenPlanRunReason(ctx, e.Repo, runID, compiled); reason != "" {
			...
			return workflowagenttools.DeliverResult{}, fmt.Errorf("workflow run %q: %s", runID, reason)
		}
	}
	...
	if !ok {
		...
		return workflowagenttools.DeliverResult{}, fmt.Errorf("workflow delivery policy is not active for run %q", runID)
	}
	...
	if holder, busy := e.delivering[runID]; busy {
		e.mu.Unlock()
		return workflowagenttools.DeliverResult{}, fmt.Errorf("workflow run %q delivery already in progress (holder %s)", runID, holder)
	}
...
func (e *Engine) publishDelivery(ctx context.Context, run workflowledger.RunSnapshot, snapshot workflowledger.Snapshot, policy delivery.Policy) (workflowagenttools.DeliverResult, error) {
	runID := run.RunID
	...
	if err != nil {
		...
		if delivery.IsRefusal(err) {
			...
			return workflowagenttools.DeliverResult{RunID: runID, Status: string(workflowledger.RunStatusDeliveryFailed), Refused: true, Reason: err.Error()}, nil
		}
		return workflowagenttools.DeliverResult{}, err
	}
	...
	if err != nil {
		...
		if rerr != nil {
			return workflowagenttools.DeliverResult{}, rerr
		}
		return workflowagenttools.DeliverResult{}, err
	}
	...
	return workflowagenttools.DeliverResult{RunID: runID, Status: string(workflowledger.RunStatusSucceeded), URL: result.URL, Mode: result.Mode}, nil
}
...
func (e *Engine) settleDeliveryAttemptError(ctx, deliveryCtx context.Context, repo workflowledger.Repository, runID string, policy delivery.Policy, err error) (workflowagenttools.DeliverResult, bool, error) {
	if delivery.IsRefusal(err) {
		e.settleDeliveryFailed(ctx, repo, runID)
		return workflowagenttools.DeliverResult{RunID: runID, Status: string(workflowledger.RunStatusDeliveryFailed), Refused: true, Reason: err.Error()}, true, nil
	}
	...
	if deliveryCtx.Err() != nil {
		return workflowagenttools.DeliverResult{}, false, err
	}
...
// it.
func routeDeliveryRepair(ctx context.Context, repo workflowledger.Repository, runID string, policy delivery.Policy, err error) (workflowagenttools.DeliverResult, bool, error) {
	...
	if step == "" || provider.IsTransient(err) || delivery.IsTransportFault(err) {
		return workflowagenttools.DeliverResult{}, false, nil
	}
	if rerr := delivery.ReopenForRepair(ctx, repo, runID, step, policy.MaxRepairs, err, io.Discard); rerr != nil {
		return workflowagenttools.DeliverResult{}, false, rerr
	}
	fresh, gerr := repo.GetRun(ctx, runID)
	if gerr != nil {
		return workflowagenttools.DeliverResult{}, false, gerr
	}
	return workflowagenttools.DeliverResult{RunID: runID, Status: string(fresh.Status)}, true, nil
}
