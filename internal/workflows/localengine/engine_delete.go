...
// Delete implements workflowagenttools.Engine. It removes a run from the durable
...
// claims untouched.
func (e *Engine) Delete(ctx context.Context, runID string, force bool) (workflowagenttools.DeleteResult, error) {
	if e == nil || e.Repo == nil {
		return workflowagenttools.DeleteResult{}, fmt.Errorf("workflow engine is incomplete")
	}
	e.mu.Lock()
	...
	e.mu.Unlock()
	if delivering {
		return workflowagenttools.DeleteResult{}, fmt.Errorf("run %q is being delivered; delete refused", runID)
	}
	if active {
		return workflowagenttools.DeleteResult{}, fmt.Errorf("run %q is running in this engine; cancel it before delete", runID)
	}
