...
// Delete implements workflowledger.Engine. It removes a run from the durable
...
func (e *Engine) Delete(ctx context.Context, runID string, force bool) (workflowledger.DeleteResult, error) {
	if e == nil || e.Repo == nil {
		return workflowledger.DeleteResult{}, fmt.Errorf("workflow engine is incomplete")
	}
	e.mu.Lock()
	...
	e.mu.Unlock()
	if delivering {
		return workflowledger.DeleteResult{}, fmt.Errorf("run %q is being delivered; delete refused", runID)
	}
	if active {
		return workflowledger.DeleteResult{}, fmt.Errorf("run %q is running in this engine; cancel it before delete", runID)
	}
