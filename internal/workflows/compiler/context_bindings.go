...
func validateContextBindings(wf *definition.WorkflowFile, stepIDs map[string]bool, skipCycleValidation bool) []string {
	var errs []string
...
func validateContextBinding(s *definition.Step, cb definition.ContextBinding, wf *definition.WorkflowFile, stepIDs map[string]bool, skipCycleValidation bool) []string {
	var errs []string
	...
	case "steps":
		if len(parts) != 3 || parts[2] != "output" {
			...
			if !skipCycleValidation && cb.MaxBytes > definition.MaxEvidenceBindingBytes {
				errs = append(errs, fmt.Sprintf("step %q: context binding %q max_bytes %d exceeds maximum of %d for prior step evidence", s.ID, cb.From, cb.MaxBytes, definition.MaxEvidenceBindingBytes))
			}
		}
	case "delivery":
	...
	if cb.MaxBytes > definition.MaxInputBytes {
		errs = append(errs, fmt.Sprintf("step %q: context binding %q max_bytes %d exceeds maximum of %d", s.ID, cb.From, cb.MaxBytes, definition.MaxInputBytes))
	}
	return errs
}
