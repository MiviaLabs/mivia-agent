...
func newTwoStepWorkflow(name string, bindings []definition.ContextBinding) *definition.WorkflowFile {
...
// definition.MaxEvidenceBindingBytes (32KiB), so the compiler must reject
...
func TestCompile_EvidenceBindingMaxBytes(t *testing.T) {
	t.Run("steps output binding above 32KiB is rejected", func(t *testing.T) {
		wf := newTwoStepWorkflow("evidence-too-large", []definition.ContextBinding{
	...
	t.Run("steps output binding at exactly 32KiB compiles", func(t *testing.T) {
		wf := newTwoStepWorkflow("evidence-at-cap", []definition.ContextBinding{
			{From: "steps.plan.output", As: "plan_result", MaxBytes: definition.MaxEvidenceBindingBytes},
		})
		if _, err := Compile(wf); err != nil {
			t.Fatalf("expected steps output binding max_bytes=%d to compile: %v", definition.MaxEvidenceBindingBytes, err)
		}
	})

	t.Run("inputs binding above 32KiB still compiles", func(t *testing.T) {
		wf := newTwoStepWorkflow("input-at-60000", []definition.ContextBinding{
			{From: "inputs.spec", As: "spec", MaxBytes: 60000},
		})
		wf.Inputs = map[string]definition.InputDef{"spec": {Type: "string"}}
		if _, err := Compile(wf); err != nil {
...
func TestCompileForResumeEvidenceBindingMaxBytes(t *testing.T) {
	wf := newTwoStepWorkflow("evidence-resume-policy", []definition.ContextBinding{
		{From: "steps.plan.output", As: "plan_result", MaxBytes: 60000},
	})
