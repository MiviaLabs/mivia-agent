...
func TestCompile_DeliveryReentryStepMustBindDeliveryFailure(t *testing.T) {
	...
	t.Run("binding on the re-entry step compiles", func(t *testing.T) {
		wf := deliveryReentryWorkflow()
		wf.Steps[1].Context = []definition.ContextBinding{
...
func deliveryReentryWorkflow() *definition.WorkflowFile {
	wf := newMinimalWorkflow("delivery-reentry-hint")
	wf.Steps = append(wf.Steps, definition.Step{ID: "repair", Kind: "agent", Agent: "engineer"})
	wf.Transitions = append(wf.Transitions, definition.Transition{
