...
// the worktree as foreign. A record that already carries a deferred-file
// split (DeferredFiles != "") routes to resumeDeliveryCommitSplit instead of
// the single-commit paths: the split has TWO host commits and a deferred
// branch, so neither commitWorktreeFollowUp (which would fold the deferred
...
func commitOrResume(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, key string, existing ledger.DeliveryRecord, head string, porcelainEmpty bool, diffRef, title, body string) (string, string, error) {
	...
	if head != req.BaseCommit {
		...
		switch {
		case existing.DeferredFiles != "":
			// A previous attempt started (or finished) a deferred-file
			// split. Only the dedicated split-resume path may run: it knows
			// the two-commit/branch topology and keeps the deferred scope
			// off the delivered branch regardless of which window the
			// previous attempt died in.
			return resumeDeliveryCommitSplit(ctx, repo, git, req, existing, head, porcelainEmpty, diffRef, title, body)
...
func freshDeliveryCommit(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, key, diffRef, title, body string) (string, string, error) {
...
func freshDeliveryCommitSingle(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, key, diffRef, title, body string) (string, string, error) {
...
func commitWorktreeFollowUp(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, existing ledger.DeliveryRecord, diffRef, title, body string) (string, string, error) {
	if _, err := git.Run(ctx, req.GitCtx, "-c", "core.fsmonitor=false", "add", "-A"); err != nil {
		return "", "", err
	}
	treeOut, err := git.Run(ctx, req.GitCtx, "write-tree")
...
func adoptOwnDeliveryCommit(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, existing ledger.DeliveryRecord, head, headTree, diffRef string, porcelainEmpty bool) (string, string, error) {
	if !porcelainEmpty {
		return "", "", &RefusalError{Reason: "worktree has foreign commits or uncommitted changes"}
	}
	count, cerr := git.Run(ctx, req.GitCtx, "rev-list", "--count", req.BaseCommit+"..HEAD")
...
// and its actual tree.
func adoptOwnFollowUpCommit(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, existing ledger.DeliveryRecord, head, diffRef string, porcelainEmpty bool) (string, string, error) {
	if !porcelainEmpty {
		return "", "", &RefusalError{Reason: "worktree has foreign commits or uncommitted changes"}
	}
...
func commitStagedTree(ctx context.Context, repo LedgerRepository, git GitRunner, req Request, stage ledger.DeliveryRecord, treeSHA, msg string) (string, string, error) {
	// Commit through Git so pre-commit and commit-msg hooks can reject it.
	if _, err := git.Run(ctx, req.GitCtx, "-c", "core.fsmonitor=false",
		"-c", "user.name="+mviaCommitAuthorName, "-c", "user.email="+mviaCommitAuthorEmail,
		"commit", "--allow-empty-message", "-m", msg); err != nil {
