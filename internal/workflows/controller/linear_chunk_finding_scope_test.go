...
func runChunkScopeTest(t *testing.T, planJSON string, runner *scriptedRunner) (*LinearController, LedgerRepository, error) {
	t.Helper()
...
func runChunkScopeTestWithSink(t *testing.T, planJSON string, runner *scriptedRunner) (*LinearController, LedgerRepository, *recordingSink, error) {
	t.Helper()
	repo := workflowledger.NewMemoryRepository()
	ctrl, err := NewLinearController(repo, runner, chunkScopeWorkflow(t, -1), map[string]StepRuntime{
		"implement":           {Agent: agents.ResolvedAgent{Name: "dev"}},
