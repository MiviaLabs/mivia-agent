...
func seedFailedAttempt(t *testing.T, repo ledger.Repository, runID string) {
	t.Helper()
	...
	if err := repo.CompleteStepAttempt(ctx, runID, attempt.AttemptID, stored.Version, ledger.AttemptOutcome{
		Status: ledger.AttemptStatusFailed, ErrorRef: "sha256:errorbody",
...
func seedFailedAttemptWithErrorText(t *testing.T, repo ledger.Repository, runID string, errorText string) string {
	t.Helper()
	ctx := context.Background()
	snapshot, err := ledger.MarshalSnapshot(ledger.Snapshot{
		SchemaVersion: 1, DefinitionTOML: []byte("name=x"), DefinitionDigest: "digest",
	...
	ref := "sha256:" + ledger.DigestHex([]byte(errorText))
	if errorText != "" {
	...
	if err := repo.CompleteStepAttempt(ctx, runID, attempt.AttemptID, stored.Version, ledger.AttemptOutcome{
		Status: ledger.AttemptStatusFailed, ErrorRef: ref,
		ToStepID: "failure", TransitionIndex: -1,
		CoordinatorRunID: "coord-fail", TaskID: "task-fail",
	}); err != nil {
		t.Fatal(err)
