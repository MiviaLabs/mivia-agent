...
func stackingReservedInputs() map[string]InputDef {
	return map[string]InputDef{
...
func MergeStackingInputs(compiled *CompiledWorkflow) {
	...
	if compiled.Inputs == nil {
		compiled.Inputs = make(map[string]InputDef, len(reserved))
	}
...
func SynthesizeStacking(cw *CompiledWorkflow) (*CompiledWorkflow, error) {
	...
	inputs := make(map[string]InputDef, len(cw.Inputs)+len(stackingReservedInputs()))
...
func synthesizedStackingSteps(declared []Step, agent, planStep string) []Step {
	steps := make([]Step, 0, len(declared)+2)
	...
	steps = append(steps,
		Step{
			...
			Context: []ContextBinding{
				...
				{From: "inputs.remaining_scope", As: "remaining_scope", Optional: true},
			},
		},
		Step{
...
func synthesizeTransitions(cw *CompiledWorkflow, planStep, implementStep string) []Transition {
	anchor := stackingRouterAnchor(cw, planStep, implementStep)
	out := make([]Transition, 0, len(cw.Transitions)+8)
	...
	if rewirePlanToAnchor && !hasPlainSucceededEdge(cw, planStep, anchor) {
		out = append(out, Transition{From: planStep, To: anchor, Match: MatchCriteria{Status: "succeeded"}})
	}
	out = append(out,
		Transition{From: anchor, To: stepDecompose, Match: MatchCriteria{Status: "succeeded"}},
		Transition{From: stepDecompose, To: implementStep, Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"stack_mode": "single"}}},
		Transition{From: stepDecompose, To: "success", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"stack_mode": "no_bug"}}},
		Transition{From: stepDecompose, To: stepChunkPlanValidate, Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"stack_mode": "multi"}}},
		Transition{From: stepDecompose, To: "failure", Match: MatchCriteria{Status: "failed"}},
		Transition{From: stepChunkPlanValidate, To: "success", Match: MatchCriteria{Status: "succeeded"}},
		Transition{From: stepChunkPlanValidate, To: stepDecompose, Match: MatchCriteria{Status: "failed"}, Loop: loopDecomposeRepair, MaxIterations: 3},
	)
	return out
}
