...
func stackingFixture() *WorkflowFile {
	return &WorkflowFile{
		Version:     1,
		Name:        "stacked-fixture",
		InitialStep: "plan",
		Stacking:    &Stacking{PlanStep: "plan", ImplementStep: "implement"},
		Steps: []Step{
...
func TestCompile_StackingUnknownSteps(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "triage", ImplementStep: "implement"}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "code"}
				return wf
			}(),
...
func TestCompile_StackingBadConfig(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MaxChunks: 1000}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MaxFiles: -1}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", SoftLines: 500, HardLines: 400}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MergePolicy: "merge-queue"}
				return wf
			}(),
...
func TestCompile_StackingWaveConcurrencyBadConfig(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MaxTotalChunks: 10000}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MaxWaveChunks: -1}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MaxConcurrentChunks: 1000}
				return wf
			}(),
		...
		},
		{
			...
			wf: func() *WorkflowFile {
				wf := stackingFixture()
				wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement", MaxWaveChunks: 50, MaxTotalChunks: 10}
				return wf
			}(),
...
func TestCompile_StackingDeclaredTableUsesGlobalDefaults(t *testing.T) {
	...
	wf := stackingFixture()
	wf.Stacking = &Stacking{PlanStep: "plan", ImplementStep: "implement"}
	cw, err := Compile(wf)
	if err != nil {
		t.Fatalf("Compile failed: %v", err)
	}
	...
	if cw.Stacking.MaxChunks != DefaultStackingMaxChunks {
		t.Errorf("MaxChunks = %d, want global default %d", cw.Stacking.MaxChunks, DefaultStackingMaxChunks)
	}
	if cw.Stacking.MaxTotalChunks != DefaultStackingMaxTotalChunks {
		t.Errorf("MaxTotalChunks = %d, want global default %d", cw.Stacking.MaxTotalChunks, DefaultStackingMaxTotalChunks)
	}
	if cw.Stacking.MaxWaveChunks != DefaultStackingMaxWaveChunks {
		t.Errorf("MaxWaveChunks = %d, want global default %d", cw.Stacking.MaxWaveChunks, DefaultStackingMaxWaveChunks)
	}
	if cw.Stacking.MaxConcurrentChunks != DefaultStackingMaxConcurrentChunks {
		t.Errorf("MaxConcurrentChunks = %d, want global default %d", cw.Stacking.MaxConcurrentChunks, DefaultStackingMaxConcurrentChunks)
	}
}
...
func TestCompile_StackingEnabledRequiresExplicitSteps(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"both missing", &Stacking{}},
		{"plan_step missing", &Stacking{ImplementStep: "implement"}},
		{"implement_step missing", &Stacking{PlanStep: "plan"}},
	} {
...
func TestCompileForResume_SameOptInRuleAsAdmission(t *testing.T) {
	// Resume compiles under the same opt-in activation rule as admission: a
	// table-less definition stays single-PR even when its graph matches the
	// old inference shape, a step-less enabled table is rejected, and an
	// explicit table stays active.
	t.Run("no table stays single-PR", func(t *testing.T) {
		wf := stackingFixture()
		wf.Stacking = nil
		...
		if cw.Stacking != nil {
			t.Errorf("Stacking = %+v, want nil without a [stacking] table", cw.Stacking)
		}
	})

	t.Run("step-less table is rejected", func(t *testing.T) {
		wf := stackingFixture()
		wf.Stacking = &Stacking{}
		if _, err := CompileForResume(wf); err == nil || !strings.Contains(err.Error(), "plan_step and implement_step are required") {
			t.Fatalf("want required-steps error, got %v", err)
		}
	})
