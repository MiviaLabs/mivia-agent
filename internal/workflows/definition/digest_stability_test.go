...
func minimalWorkflow(t *testing.T) *WorkflowFile {
	t.Helper()
	return &WorkflowFile{
		Version: 1, Name: "digest-stability", InitialStep: "one",
		Inputs: map[string]InputDef{"task": {Type: "string", Required: true}},
		Limits: Limits{MaxStepAttempts: 4},
		Steps: []Step{
			{ID: "one", Kind: "agent", Agent: "dev"},
		},
		Transitions: []Transition{
			{From: "one", To: "success", Match: MatchCriteria{Status: "succeeded"}},
		},
	}
}
