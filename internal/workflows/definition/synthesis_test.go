...
func stepByID(t *testing.T, cw *CompiledWorkflow, id string) Step {
	t.Helper()
...
func TestSynthesizeStacking_RewiresPlanStepEdges(t *testing.T) {
	...
	wf.Transitions = []Transition{
		...
		{From: "plan", To: "failure", Match: MatchCriteria{Status: "failed"}},
...
func synthWorkflowFile(t *testing.T, cw *CompiledWorkflow) *WorkflowFile {
	t.Helper()
	return &WorkflowFile{
		Version:     cw.Version,
		Name:        cw.Name,
...
func TestSynthesizeStacking_RejectsReservedStepIDCollision(t *testing.T) {
	wf := stackingFixture()
	wf.Steps = append(wf.Steps, Step{ID: "decompose", Kind: "agent", Agent: "workflow-engineer"})
	wf.Transitions = append(wf.Transitions,
		Transition{From: "implement", To: "decompose", Match: MatchCriteria{Status: "failed"}},
		Transition{From: "decompose", To: "success", Match: MatchCriteria{Status: "succeeded"}},
	)
	cw, err := Compile(wf)
	if err != nil {
		t.Fatalf("Compile failed: %v", err)
...
func TestSynthesizedInputs(t *testing.T) {
	cfg := &StackingConfig{Enabled: true}
	...
	if got := SynthesizedInputs(&StackingConfig{Enabled: false}); got != nil {
...
func TestMergeStackingInputs(t *testing.T) {
	...
	cw = compileStackingFixture(t)
	cw.Inputs = map[string]InputDef{"chunk": {Type: "integer", Required: true}}
	MergeStackingInputs(cw)
...
func hasBinding(bindings []ContextBinding, from, as string) bool {
