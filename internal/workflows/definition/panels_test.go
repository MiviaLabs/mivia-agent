...
var panelValidationCases = []panelValidationCase{
	{
		...
		mutate:  func(s *Step) { s.Panel.Members = s.Panel.Members[:1] },
	...
	},
	{
		...
		mutate:  func(s *Step) { s.Panel.Members[0].ID = " " },
		wantErr: "member[0]: id is required",
	},
	{
		name:    "duplicate member ID",
		mutate:  func(s *Step) { s.Panel.Members[1].ID = s.Panel.Members[0].ID },
		wantErr: "duplicate member id",
	},
	{
		name:    "missing member agent",
		mutate:  func(s *Step) { s.Panel.Members[0].Agent = "" },
		wantErr: "agent is required",
	},
	{
		name:    "missing provider",
		mutate:  func(s *Step) { s.Panel.Members[0].Provider = "" },
		wantErr: "provider and model must both be set",
	},
	{
		name:    "missing model",
		mutate:  func(s *Step) { s.Panel.Members[0].Model = "" },
		wantErr: "provider and model must both be set",
	},
	{
		name:    "missing skill",
		mutate:  func(s *Step) { s.Panel.Members[0].Skill = "" },
		wantErr: "skill is required",
	},
	{
		name:    "missing template",
		mutate:  func(s *Step) { s.Panel.Members[0].Template = "" },
		wantErr: "template is required",
	},
	{
		name:    "missing output schema",
		mutate:  func(s *Step) { s.Panel.Members[0].OutputSchema = "" },
		wantErr: "output_schema is required",
	},
	{
		name:    "unsupported failure policy",
		mutate:  func(s *Step) { s.Panel.FailurePolicy = "best_effort" },
		wantErr: "failure_policy must be \"require_all\" or \"allow_partial\"",
	},
	{
		name:    "distinct bindings required",
		mutate:  func(s *Step) { s.Panel.RequireDistinctBindings = false },
	...
	// this is rejected here, at compile time.
	{
		...
		mutate:  func(s *Step) { s.Panel.Members[0].ID = "sec\x1eurity" },
...
func TestCompileRejectsPanelOnNonPanelStep(t *testing.T) {
	wf := &WorkflowFile{
		...
		InitialStep: "approve",
		Steps:       []Step{{ID: "approve", Kind: "human_gate", Panel: &AgentPanel{}}},
		Transitions: []Transition{{From: "approve", To: "success", Match: MatchCriteria{Status: "approved"}}},
	}
	_, err := Compile(wf)
...
`)
	wf, _, err := ParseWorkflowTOML(data, "decoded-panel.toml")
	if err != nil {
...
func newAgentPanelWorkflow() *WorkflowFile {
	return &WorkflowFile{
		Version:     1,
		Name:        "agent-panel",
		InitialStep: "review",
		...
		Transitions: []Transition{{From: "review", To: "success", Match: MatchCriteria{Status: "succeeded"}}},
	}
}
