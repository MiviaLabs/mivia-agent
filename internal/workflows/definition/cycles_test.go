...
func TestCompile_UnnamedCycleWithoutGlobalLimitRejected(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Transitions: []Transition{
			...
			{From: "review", To: "implement", Match: MatchCriteria{Status: "succeeded"}},
...
func TestCompile_UnnamedCycleWithGlobalLimitAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Version:     1,
		InitialStep: "implement",
		Limits:      Limits{MaxStepAttempts: 16},
		Steps: []Step{
			{ID: "implement", Kind: "agent", Agent: "go-engineer"},
			{ID: "review", Kind: "agent", Agent: "go-engineer"},
		},
...
func TestCompile_UnlimitedAttemptsWithUnlimitedLoopAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Version:     1,
		InitialStep: "implement",
		Limits:      Limits{MaxStepAttempts: 0, MaxDurationSeconds: 3600},
		Steps: []Step{
			{ID: "implement", Kind: "agent", Agent: "go-engineer"},
			{ID: "review", Kind: "agent", Agent: "go-engineer"},
		},
		Transitions: []Transition{
			{From: "implement", To: "review", Match: MatchCriteria{Status: "succeeded"}},
			{From: "review", To: "implement", Match: MatchCriteria{Status: "succeeded"}, Loop: "repair", MaxIterations: -1},
			{From: "review", To: "success", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"verdict": "approved"}}},
		},
	}
	if _, err := Compile(wf); err != nil {
...
func TestCompile_FiniteCappedLoopWithoutGlobalLimitAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Transitions: []Transition{
			{From: "implement", To: "review", Match: MatchCriteria{Status: "succeeded"}},
			{From: "review", To: "implement", Match: MatchCriteria{Status: "succeeded"}, Loop: "repair", MaxIterations: 3},
			{From: "review", To: "success", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"verdict": "approved"}}},
		},
	}
	if _, err := Compile(wf); err != nil {
...
func TestCompile_MixedUnlimitedAndCappedCycleAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Transitions: []Transition{
			{From: "implement", To: "review", Match: MatchCriteria{Status: "succeeded"}, Loop: "cap", MaxIterations: 3},
			{From: "review", To: "implement", Match: MatchCriteria{Status: "succeeded"}, Loop: "unlimited", MaxIterations: -1},
			{From: "review", To: "success", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"verdict": "approved"}}},
		},
	}
	if _, err := Compile(wf); err != nil {
