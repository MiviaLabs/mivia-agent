...
func TestCompile_TransitionsSameStatusDifferentOutputArity(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Transitions: []Transition{
			{From: "s", To: "failure", Match: MatchCriteria{Status: "failed", Output: map[string]string{"code": "1"}}},
			{From: "s", To: "success", Match: MatchCriteria{Status: "failed"}},
		},
	}
...
func TestCompile_TransitionsSameStatusDisjointOutputKeysRejected(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Steps:       []Step{{ID: "s", Kind: "agent", Agent: "go-engineer"}},
		Transitions: []Transition{
			{From: "s", To: "a", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"a": "1"}}},
			{From: "s", To: "b", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"b": "2"}}},
		},
	}
	if _, err := Compile(wf); err == nil {
...
func TestCompile_TransitionsSameStatusConflictingValuesAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Steps:       []Step{{ID: "s", Kind: "agent", Agent: "go-engineer"}},
		Transitions: []Transition{
			{From: "s", To: "a", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"verdict": "approved"}}},
			{From: "s", To: "b", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"verdict": "changes_requested"}}},
			{From: "s", To: "success", Match: MatchCriteria{Status: "failed"}},
		},
	}
	if _, err := Compile(wf); err != nil {
...
func TestCompile_TransitionsDifferentStatusesAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Steps:       []Step{{ID: "s", Kind: "agent", Agent: "go-engineer"}},
		Transitions: []Transition{
			{From: "s", To: "a", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"code": "1"}}},
			{From: "s", To: "success", Match: MatchCriteria{Status: "failed"}},
		},
	}
	if _, err := Compile(wf); err != nil {
...
func TestCompile_TransitionsStrictSupersetAccepted(t *testing.T) {
	...
	wf := &WorkflowFile{
		...
		Steps:       []Step{{ID: "s", Kind: "agent", Agent: "go-engineer"}},
		Transitions: []Transition{
			{From: "s", To: "specific", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"a": "1", "b": "2"}}},
			{From: "s", To: "fallback", Match: MatchCriteria{Status: "succeeded", Output: map[string]string{"a": "1"}}},
			{From: "s", To: "success", Match: MatchCriteria{Status: "failed"}},
		},
	}
	if _, err := Compile(wf); err != nil {
