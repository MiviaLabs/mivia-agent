...
func TestCompile_CommandGateCompiles(t *testing.T) {
	wf := &WorkflowFile{
		...
		Steps: []Step{{
			...
			Command: &StepCommand{Check: "gate", Program: "python3", Args: []string{"scripts/gate.py"}},
		}},
...
func TestCompile_CommandGatePathProgramRejected(t *testing.T) {
	wf := &WorkflowFile{
		...
		Steps: []Step{{
			ID: "verify", Kind: "evidence_gate",
			Command: &StepCommand{Check: "gate", Program: "/usr/bin/python3"},
		}},
		Transitions: []Transition{
			{From: "verify", To: "success", Match: MatchCriteria{Status: "succeeded"}},
		},
...
func TestCompile_EvidenceGateRequiresVerifierOrCommand(t *testing.T) {
	wf := &WorkflowFile{
		...
		Version:     1,
		InitialStep: "verify",
		Steps:       []Step{{ID: "verify", Kind: "evidence_gate"}},
		Transitions: []Transition{
			{From: "verify", To: "success", Match: MatchCriteria{Status: "succeeded"}},
		},
	}
