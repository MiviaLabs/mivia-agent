...
func formatInputs(b *strings.Builder, c *CompiledWorkflow) {
	if len(c.Inputs) == 0 {
		return
	}
	b.WriteString("\nInputs:\n")
...
func formatLimits(b *strings.Builder, c *CompiledWorkflow) {
...
func formatSteps(b *strings.Builder, c *CompiledWorkflow) {
...
func formatTransitions(b *strings.Builder, c *CompiledWorkflow) {
	b.WriteString(fmt.Sprintf("\nTransitions (%d):\n", len(c.Transitions)))
	for _, t := range c.Transitions {
		loopInfo := ""
		if t.Loop != "" {
			if t.MaxIterations == UnlimitedIterations {
...
func formatDelivery(b *strings.Builder, c *CompiledWorkflow) {
	if c.Delivery == nil {
		return
	}
	b.WriteString("\nDelivery:\n")
