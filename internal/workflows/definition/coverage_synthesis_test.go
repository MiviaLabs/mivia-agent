...
func TestCoverageSynthesizeStackingFailClosed(t *testing.T) {
	for _, tt := range []struct {
		name    string
		apply   func(*WorkflowFile)
		wantErr string
	}{
		{"plan step with no agent and empty stacking.agent", func(w *WorkflowFile) { w.Steps[0].Agent = "" }, "no agent"},
		{"reserved loop name decompose_repair", func(w *WorkflowFile) {
			w.Transitions = append(w.Transitions, Transition{From: "plan", To: "plan", Match: MatchCriteria{Status: "failed"}, Loop: "decompose_repair", MaxIterations: 3})
		}, "engine-reserved"},
		{"reserved step id chunk_plan_validate", func(w *WorkflowFile) {
			w.Steps = append(w.Steps, Step{ID: "chunk_plan_validate", Kind: "agent", Agent: "workflow-engineer"})
			w.Transitions = append(w.Transitions, Transition{From: "implement", To: "chunk_plan_validate", Match: MatchCriteria{Status: "failed"}}, Transition{From: "chunk_plan_validate", To: "success", Match: MatchCriteria{Status: "succeeded"}})
		}, "engine-reserved"},
	} {
...
func TestCoverageSynthesizeStackingDisabledOptOut(t *testing.T) {
	wf := stackingFixture()
	wf.Stacking = &Stacking{Enabled: boolPtr(false)}
	cw := covCompile(t, wf)
	if cw.Stacking != nil {
