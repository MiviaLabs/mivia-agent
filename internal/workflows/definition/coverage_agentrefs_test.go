...
func TestCoverageValidateAgentReferencesTable(t *testing.T) {
	two := func(a, b string) *WorkflowFile {
		return &WorkflowFile{
			Version: 1, Name: "two-step-agent-refs", InitialStep: "step1",
			Steps:       []Step{{ID: "step1", Kind: "agent", Agent: a}, {ID: "step2", Kind: "agent", Agent: b}},
			Transitions: []Transition{{From: "step1", To: "step2", Match: MatchCriteria{Status: "succeeded"}}, {From: "step2", To: "success", Match: MatchCriteria{Status: "succeeded"}}},
		}
	}
