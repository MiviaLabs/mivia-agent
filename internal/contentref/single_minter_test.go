...
// allowlist looks like it builds a "ref:…" reference. Comments are invisible to
// it (it walks the AST, not the bytes), so prose describing the format is free.
//
// Invariant: "A reference handed to the model resolves, or it is not handed to
// the model." Corollary: minting lives in exactly one function.

// theMinter is the only file permitted to build a reference string.
const theMinter = "internal/contentref/contentref.go"
...
var allowedOtherMinters = map[string]string{
	...
	// 256-byte column bound into "ref:sha256:<digest>". It is a storage-bound
	// guard, not a content minter, and it is unreachable for canonical
	// references, which are 75 bytes.
	"internal/ledger/memory.go": "ledger overlong-reference storage guard",
}

// buildsReference reports whether a string literal looks like it constructs a
// reference rather than merely describing one. Model-facing tool descriptions
// have to tell an agent what a "ref:<kind>:<digest>" handle is, and that prose
// is not a minter. The discriminator is whitespace: a reference is a single
// token, so every format string that builds one ("ref:%s:%x") is whitespace-free,
// while a sentence explaining the format is not.
...
func TestReferenceHasSingleMinter(t *testing.T) {
	...
	// Without this the test goes vacuous the moment the minter is renamed or
