...
// chat.LookupSessionBus so the test proves the alias reaches the SAME
...
func TestCLI_RegisterSessionBus_BindUnbindRebind(t *testing.T) {
	...
	if got := chat.LookupSessionBus(id); got != bus1 {
		t.Fatalf("after bind: LookupSessionBus = %v, want bus1", got)
	}

	release1()
	if got := chat.LookupSessionBus(id); got != nil {
	...
	if got := chat.LookupSessionBus(id); got != bus2 {
