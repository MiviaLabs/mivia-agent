...
func TestSlashCatalogIncludesAgent(t *testing.T) {
	if _, ok := findSlashCommand("/agent", slashSurfaceTUI, nil); !ok {
...
func TestClassicSlashAgentDirect(t *testing.T) {
	...
	classicAgentState = state
	defer func() { classicAgentState = nil }()
	res := &config.Resolved{Model: "m", ProviderName: "p"}
	sess := chat.NewSession(res, stubAgentCompleter{})
	term, err := NewTerminal()
	if err != nil {
		// Non-TTY environments still exercise apply via handleSlashAgent.
		handled, _, herr := handleSlashAgent([]string{"/agent", "beta"}, sess, res, nil, state)
		if !handled || herr != nil {
			t.Fatalf("handled=%v err=%v", handled, herr)
		}
	} else {
		defer term.Close()
		handled, _, herr := handleSlash("/agent beta", sess, res, false, term)
		if !handled || herr != nil {
			t.Fatalf("handled=%v err=%v", handled, herr)
		}
	}
