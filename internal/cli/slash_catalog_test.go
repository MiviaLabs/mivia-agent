...
func TestSlashCatalogSeparatesSurfacesAndIncludesSearch(t *testing.T) {
	...
	for _, name := range []string{"/exit", "/quit", "/q", "/provider", "/workspace"} {
		if _, ok := findSlashCommand(name, slashSurfacePlain, nil); !ok {
	...
	if command, ok := findSlashCommand("/h", slashSurfaceTUI, nil); !ok || command.Name != "/help" || !command.AutoExecute {
...
func TestSlashCatalogAddsInvocableSkillsAndRejectsBuiltinCollision(t *testing.T) {
	...
	command, ok := findSlashCommand("/bug-audit", slashSurfaceTUI, reg)
	if !ok || command.Kind != slashKindSkill || command.SkillName != "bug_audit" || command.ArgsHint != "<path>" {
		t.Fatalf("skill command = %#v ok=%v", command, ok)
	}
	if _, ok := findSlashCommand("/hidden", slashSurfaceTUI, reg); ok {
		t.Fatal("non-invocable skill reached slash catalog")
	}
	if command, ok := findSlashCommand("/help", slashSurfaceTUI, reg); !ok || command.Kind != slashKindBuiltin {
...
func TestSlashCatalogProjectWinsNormalizedSkillTokenCollision(t *testing.T) {
	...
	for _, command := range slashCommands(slashSurfaceTUI, reg) {
		if command.Name == "/a-b" {
