...
func thinkingScrollModel(t *testing.T, text string, collapsed bool) *tuiModel {
	...
	m.height = 40
	m.blocks = []ChatBlock{
		{ID: "th-1", Kind: ChatBlockThinking, Text: text, Collapsed: collapsed},
	}
	m.renderVP()
...
func TestBlockScrollKeysFallThroughWhenBlockCannotScroll(t *testing.T) {
	cases := []struct {
		...
		{"collapsed thinking", ChatBlock{ID: "th-1", Kind: ChatBlockThinking, Text: strings.Repeat("line\n", 20), Collapsed: true}},
		{"non-overflowing thinking", ChatBlock{ID: "th-1", Kind: ChatBlockThinking, Text: "short\nreasoning", Collapsed: false}},
		{"tool block", ChatBlock{ID: "t1", Kind: ChatBlockTool, ToolName: "read_file", Text: strings.Repeat("tool output\n", 20), Collapsed: false}},
		{"user conversation", ChatBlock{ID: "u1", Kind: ChatBlockUser, Text: strings.Repeat("question\n", 20)}},
		{"assistant conversation", ChatBlock{ID: "a1", Kind: ChatBlockAssistant, Text: strings.Repeat("answer\n", 20)}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.block.Kind == ChatBlockUser || tc.block.Kind == ChatBlockAssistant {
...
func TestMouseWheelScrollsSelectedExpandedThinkingBlock(t *testing.T) {
	...
	m.height = 40
	m.blocks = []ChatBlock{
		{ID: "th-1", Kind: ChatBlockThinking, Text: strings.Repeat("line of reasoning\n", 20), Collapsed: false},
	}
	m.renderVP()
...
func TestMouseWheelOverCollapsedThinkingBlockFallsThrough(t *testing.T) {
	...
	m.height = 40
	for i := 0; i < 30; i++ {
		m.blocks = append(m.blocks, ChatBlock{ID: "b-" + itoa(i), Kind: ChatBlockAssistant, Text: "history line " + itoa(i)})
	}
	m.blocks = append(m.blocks, ChatBlock{ID: "th-1", Kind: ChatBlockThinking, Text: strings.Repeat("line of reasoning\n", 20), Collapsed: true})
	for i := 30; i < 60; i++ {
		m.blocks = append(m.blocks, ChatBlock{ID: "b-" + itoa(i), Kind: ChatBlockAssistant, Text: "history line " + itoa(i)})
	}
	m.renderVP()
