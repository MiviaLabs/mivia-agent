...
func TestHistoryOverlaySentHistoryCap(t *testing.T) {
	...
	if len(m.sentHistory) != MaxHistorySize {
		t.Fatalf("appendSentHistory cap: expected len == %d, got %d", MaxHistorySize, len(m.sentHistory))
	}
...
//
// Contract being locked in:
//   - scopeHistory declares exactly four rows whose keys are exactly
...
func TestHistoryOverlayScopeRegistered(t *testing.T) {
	...
	for _, b := range keyRegistry {
		if b.scope == scopeHistory {
			rows = append(rows, b)
		}
	}
	if len(rows) != 4 {
		t.Fatalf("scopeHistory must declare exactly 4 rows, got %d", len(rows))
	}
	...
	for k := range wantKeys {
		if !seen[k] {
			t.Errorf("history key %q is not registered in scopeHistory", k)
		}
	}
}

// TestHistoryOverlayRegisteredKeysReallyBound: every key the registry
// advertises for scopeHistory must actually do real work through
