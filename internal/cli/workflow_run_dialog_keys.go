...
func (m *tuiModel) workflowDialogActionCmd(action workflowConfirmAction) tea.Cmd {
	...
	case workflowConfirmCleanup:
		return func() tea.Msg {
			...
			err := executeWorkflowCleanup(runID, root, configPath, &out, &errOut)
...
func resolveWorkflowDialogApproval(runID, approvalID, root, configPath, actor string, reject bool) error {
	releaseExecution, repo, closeFn, err := openWorkflowResolutionContextBounded(root, configPath, runID, workflowResolutionLockWait)
	if err != nil {
		return err
	}
	defer closeFn()
