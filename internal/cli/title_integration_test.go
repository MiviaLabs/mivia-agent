...
func TestIntegrationTUITitleRejectsSavedSnapshot(t *testing.T) {
	...
	if len(m.blocks) == 0 || !strings.Contains(stripANSI(m.blocks[len(m.blocks)-1].Text), "titles are not available for saved snapshots") {
...
func TestIntegrationTUITitleUsesLoadedWorktreeInstance(t *testing.T) {
	...
	principal, err := contextstate.NewPrincipal(contextWorkspaceID(root), loaded.SessionID, "local-user")
	if err != nil {
		t.Fatal(err)
	}
