...
func TestExecuteWorkflowResumeRecoversUnclaimedNonterminalRun(t *testing.T) {
	...
	if err := executeWorkflowResume(run.RunID, root, configPath, false, false, &stdout, io.Discard); err != nil {
...
func TestExecuteWorkflowResumeEarlyErrors(t *testing.T) {
	if err := executeWorkflowResume("wfr-test", filepath.Join(t.TempDir(), "missing"), "", false, false, io.Discard, io.Discard); err == nil {
	...
	if err := executeWorkflowResume("wfr-test", root, badConfig, false, false, io.Discard, io.Discard); err == nil {
