...
func TestSidebarDeletesBrokenWorktreeRoute(t *testing.T) {
	...
	worktree, err := createManagedWorktree(repoRoot, "stuck-route", "HEAD", "mivia/")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSidebarKeepsHealthyWorktreeRouteOnWorktreesNotice(t *testing.T) {
	repoRoot := newWorktreeCommandRepo(t)
	writeWorktreeStoreConfig(t, repoRoot, "repository.db")
	worktree, err := createManagedWorktree(repoRoot, "healthy-route", "HEAD", "mivia/")
	if err != nil {
		t.Fatal(err)
	}
	store, err := openRepositoryContextStore(repoRoot)
...
func TestSidebarRefusesStaleRouteDeletingReplacementWorktree(t *testing.T) {
	repoRoot := newWorktreeCommandRepo(t)
	writeWorktreeStoreConfig(t, repoRoot, "repository.db")
	worktree, err := createManagedWorktree(repoRoot, "replaced", "HEAD", "mivia/")
	if err != nil {
		t.Fatal(err)
	}
	store, err := openRepositoryContextStore(repoRoot)
	...
	oldInstance, err := beginManagedWorktreeRemoval(repoRoot, worktree)
	if err != nil {
		t.Fatal(err)
	}
	if err := vcs.RemoveWithPrefix(context.Background(), repoRoot, worktree.Name, "mivia/"); err != nil {
	...
	if _, err := createManagedWorktree(repoRoot, "replaced", "HEAD", "mivia/"); err != nil {
		t.Fatal(err)
	}
...
func TestSidebarDeletesStaleInstanceBoundRouteWhenInstanceGone(t *testing.T) {
	repoRoot := newWorktreeCommandRepo(t)
	writeWorktreeStoreConfig(t, repoRoot, "repository.db")
	worktree, err := createManagedWorktree(repoRoot, "gone-route", "HEAD", "mivia/")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSidebarDeletesGhostBoundRouteRow(t *testing.T) {
	repoRoot := newWorktreeCommandRepo(t)
	writeWorktreeStoreConfig(t, repoRoot, "repository.db")
	worktree, err := createManagedWorktree(repoRoot, "ghost-bound", "HEAD", "mivia/")
	if err != nil {
		t.Fatal(err)
	}
	store, err := openRepositoryContextStore(repoRoot)
	...
	checkStore, err := openRepositoryContextStore(repoRoot)
	if err != nil {
		t.Fatal(err)
	}
