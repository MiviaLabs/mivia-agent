...
// autoDeliveryAttemptTimeout bounds one stack-drive attempt (merge-wait
...
var autoDeliveryAttemptTimeout = 30 * time.Minute

// sessionAutoDeliveryRepairLoopImpl runs one session-engine controller pass, then
...
func sessionAutoDeliveryRepairLoopImpl(runCtx context.Context, repo workflowledger.Repository, root string, res *config.Resolved, store *storage.SQLite, runID string, advance func(context.Context) (workflowledger.RunSnapshot, error), driveStack func(context.Context) (bool, error), deliverPlanRun bool) {
	...
	for {
		...
		if driveStack != nil {
			...
			driveCtx, cancelDrive := context.WithTimeout(runCtx, autoDeliveryAttemptTimeout)
...
// SessionAutoDeliveryRepairLoop is the session-engine repair loop entry the
// tool engine invokes; a package variable so tests can stub the loop.
var SessionAutoDeliveryRepairLoop = sessionAutoDeliveryRepairLoopImpl
