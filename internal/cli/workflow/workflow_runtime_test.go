...
func TestPanelAuthorityRequiresExactFinalToolNames(t *testing.T) {
	...
	opts := cliagents.SessionDispatcherOpts{Registry: registry, AuthorityRegistry: registry, Config: config.DefaultSubagentConfig, SkillReg: skillRegistry}
...
func newPanelAuthorizationFixture(t *testing.T) *panelAuthorizationFixture {
	...
	return &panelAuthorizationFixture{workflow: workflow, agents: registry, snapshot: snapshot, opts: cliagents.SessionDispatcherOpts{
...
// source. Same reasoning as the sibling gate in internal/cli/agents.
