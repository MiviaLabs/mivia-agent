...
import (
	...
	providerregistry "github.com/MiviaLabs/mivia-agent/internal/provider/registry"
...
func prepareWorkflowRuntime(root, refBase string, wf *definition.CompiledWorkflow, registry *agents.AgentRegistry, prior *workflowledger.Snapshot, priorRaw []byte, definitionTOML []byte, InputSnapshot map[string]string, dispatcherOpts cliagents.SessionDispatcherOpts) (preparedWorkflowRuntime, error) {
...
func authorizeWorkflowPanelBindings(wf *definition.CompiledWorkflow, registry *agents.AgentRegistry, snapshot workflowledger.Snapshot, resume bool, opts cliagents.SessionDispatcherOpts) error {
	if wf == nil {
		return nil
	}
...
func resolveWorkflowPanelSynthesisBindings(wf *definition.CompiledWorkflow, registry *agents.AgentRegistry, prior *workflowledger.Snapshot, snapshot workflowledger.Snapshot, opts cliagents.SessionDispatcherOpts) error {
	for _, step := range wf.Steps {
		if step.Kind != "agent_panel" || step.Panel == nil {
			continue
		}
		...
		var binding cliagents.AgentBinding
		if prior != nil {
...
func workflowRuntimeBinding(agent agents.ResolvedAgent, pinned workflowledger.AgentSnapshot, resume bool, opts cliagents.SessionDispatcherOpts) (cliagents.AgentBinding, error) {
	if resume {
