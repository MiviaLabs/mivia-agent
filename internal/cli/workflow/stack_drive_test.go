...
func seedStackDriveFailSettleFixture(t *testing.T) (*PreparedWorkflowRun, string) {
	...
	seedDrivePlanRunDeliveryPending(t, prepared.Repo, planRunID, prepared.Compiled.Digest)
...
// seedDrivePlanRunDeliveryPending creates the plan run row and moves it to
// delivery_pending.
func seedDrivePlanRunDeliveryPending(t *testing.T, repo workflowledger.Repository, planRunID, digest string) {
	t.Helper()
...
// seedDriveGateFixtureBase builds a delivery_pending mini-stack plan run
...
func seedDriveGateFixtureBase(t *testing.T, planRunID string) *PreparedWorkflowRun {
	t.Helper()
	root := t.TempDir()
	storePath := filepath.Join(root, "db")
	writeWorkflowRunFixture(t, root, "https://example.com", storePath)
...
// seedDriveGateFixtureFailed builds a delivery_pending plan run whose
...
func seedDriveGateFixtureFailed(t *testing.T) (*PreparedWorkflowRun, string) {
	t.Helper()
	const planRunID = "wfr-drive-failed-gate"
	prepared := seedDriveGateFixtureBase(t, planRunID)
	ledger := workflowledger.NewStore(prepared.Store)
...
// seedDriveGateFixtureIncomplete builds a delivery_pending plan run whose
...
func seedDriveGateFixtureIncomplete(t *testing.T) (*PreparedWorkflowRun, string) {
	t.Helper()
	const planRunID = "wfr-drive-incomplete-gate"
	return seedDriveGateFixtureBase(t, planRunID), planRunID
}
...
func TestSettleStackPlanRunIfCompleteFailedGateRefusesAndSettles(t *testing.T) {
	prepared, planRunID := seedDriveGateFixtureFailed(t)
...
func TestSettleStackPlanRunIfCompleteUnknownGateFailsClosed(t *testing.T) {
	prepared, planRunID := seedDriveGateFixtureIncomplete(t)
