...
func TestLoadAllStackChunksForDriveRefusesOverCapStack(t *testing.T) {
	...
	createContinuationRunFixture(t, repo, stackID, 1, "wfr-wave1-ok", workflowledger.RunStatusSucceeded, time.Now().Add(-time.Minute))
...
func TestLoadAllStackChunksForDriveAllowsAtCapStack(t *testing.T) {
	...
	if !chunkIDListEqual(chunks, "c1", "c2", "c3", "c4") {
		t.Fatalf("chunks = %v, want all four at exactly the cap", chunkIDList(chunks))
	}
}
