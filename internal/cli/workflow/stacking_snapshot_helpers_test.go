...
// stackingSnapshotResumeFixture builds the admitted run row and raw snapshot for a
// stacking workflow at the given inputs.
func stackingSnapshotResumeFixture(t *testing.T, toml string, inputs map[string]string) (workflowledger.RunSnapshot, []byte) {
	t.Helper()
	name := stackingSnapshotTOMLName(t, toml)
...
// stackingSnapshotTOMLName extracts the workflow's declared name from its TOML text
// so the snapshot filename matches the definition's in-file name.
func stackingSnapshotTOMLName(t *testing.T, toml string) string {
	t.Helper()
