...
func newStackDriveIT(t *testing.T, mergePolicy, planOutput string) *stackDriveIT {
	t.Helper()
	root := t.TempDir()
	storePath := filepath.Join(root, "db")
...
// productionStackSeams pins the package-level stack seams to the moved
// production implementations; the package TestMain still overrides several
// of them with simplified unit-test locals, which the integration fixture
// must undo.
func (it *stackDriveIT) productionStackSeams() {
	t := it.t
	prevParse := ParseStackPlanOutputFunc
	prevSeed := SeedStackLedgerFunc
	prevInputs := StackPlanInputsFunc
	prevTasks := StackTaskMapFunc
	prevMerged := StackMergedSetFunc
	prevAllMerged := AllChunksMergedFunc
	prevLoadOut := LoadStackPlanOutputFunc
	prevLoadChunks := LoadAllStackChunksFunc
	prevRunRef := StackRunRefFunc
	prevPushed := StackRunPushedFunc
	prevHeadCommit := StackRunHeadCommitFunc
	prevHeadBranch := StackHeadBranchFunc
	prevDecomposed := StackDecomposedChunksFunc
	prevAllowPublish := StackingDriveAllowPublishFunc
	prevGate := ClassifyStackPlanRunDeliveryFn
	prevReason := StackPlanRunFailureReasonFn
	prevFail := ErrFailedStackPlanRunFunc
	prevUndriven := ErrUndrivenStackPlanRunFunc
	prevSettle := SettleStackPlanRunIfCompleteFn
	prevWithheld := StackRunPublishWithheldFunc
	prevDriveChunks := LoadAllStackChunksForDriveFunc
	t.Cleanup(func() {
		ParseStackPlanOutputFunc = prevParse
		SeedStackLedgerFunc = prevSeed
		StackPlanInputsFunc = prevInputs
		StackTaskMapFunc = prevTasks
		StackMergedSetFunc = prevMerged
		AllChunksMergedFunc = prevAllMerged
		LoadStackPlanOutputFunc = prevLoadOut
		LoadAllStackChunksFunc = prevLoadChunks
		StackRunRefFunc = prevRunRef
		StackRunPushedFunc = prevPushed
		StackRunHeadCommitFunc = prevHeadCommit
		StackHeadBranchFunc = prevHeadBranch
		StackDecomposedChunksFunc = prevDecomposed
		StackingDriveAllowPublishFunc = prevAllowPublish
		ClassifyStackPlanRunDeliveryFn = prevGate
		StackPlanRunFailureReasonFn = prevReason
		ErrFailedStackPlanRunFunc = prevFail
		ErrUndrivenStackPlanRunFunc = prevUndriven
		SettleStackPlanRunIfCompleteFn = prevSettle
		StackRunPublishWithheldFunc = prevWithheld
		LoadAllStackChunksForDriveFunc = prevDriveChunks
	})
	...
	SettleStackPlanRunIfCompleteFn = settleStackPlanRunIfComplete
...
func (it *stackDriveIT) startPlanRun(bound time.Duration) string {
	...
	t.Helper()
	prevBound := autoDeliveryAttemptTimeout
	t.Cleanup(func() { autoDeliveryAttemptTimeout = prevBound })
	autoDeliveryAttemptTimeout = bound
	...
	// runs live in root/db (the fixture's config.toml store_path) -
