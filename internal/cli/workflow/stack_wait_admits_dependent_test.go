...
func TestWaitForChunkMergesReturnsWhenADependentBecomesAdmissible(t *testing.T) {
	...
	for _, id := range []string{"c1", "c2"} {
		...
		seedGitMergeDeliveryPendingRun(t, repo, workflowledger.RunSnapshot{RunID: "wfr-" + id, InvocationKey: stackID + ":" + id, WorktreeName: "wt-" + id, BaseCommit: "deadbeef"}, []byte("snapshot"))
