...
func TestStackRunAutoPublishAllowed(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			planRun, planRaw := stackingSnapshotResumeFixture(t, toml, map[string]string{"task": "add feature"})
			...
			chunkRun, chunkRaw := stackingSnapshotResumeFixture(t, toml, map[string]string{
...
func TestStackRunAutoPublishAllowedIntegrationRun(t *testing.T) {
	...
	intRun, intRaw := stackingSnapshotResumeFixture(t, toml, map[string]string{"task": "add feature", "stack_mode": "single"})
...
func TestStackRunAutoPublishAllowedNonStackRun(t *testing.T) {
	...
	run, raw := stackingSnapshotResumeFixture(t, miniStackWorkflowTOML, map[string]string{"task": "add feature"})
...
// false-positive regression: workflowledger.StartRequest.InvocationKey is a
...
status = "succeeded"
`
	unrelated, unrelatedRaw := stackingSnapshotResumeFixture(t, plainTOML, map[string]string{"task": "x"})
...
func TestStackRunAutoPublishAllowedInvocationKeyEmbedsRealPlanRunID(t *testing.T) {
	...
	planRun, planRaw := stackingSnapshotResumeFixture(t, tomlApprove, map[string]string{"task": "add feature"})
	...
	seedGitMergeDeliveryPendingRun(t, repo, run, raw)
...
func TestStackRunAutoPublishAllowedChunkRunOwnInputs(t *testing.T) {
	...
	chunkRun, chunkRaw := stackingSnapshotResumeFixture(t, tomlApprove, map[string]string{
		"task": "add feature", "stack_mode": "chunk", "chunk": "c1", "stack_part": "1/2",
	})
	chunkRun.RunID = "wfr-own-inputs-chunk"
	chunkRun.InvocationKey = planRun.RunID + ":c1"
	seedGitMergeDeliveryPendingRun(t, repo, chunkRun, chunkRaw)
...
func TestStackRunAutoPublishAllowedMissingPlanRunAutoPolicyFailsClosed(t *testing.T) {
	...
	planRun, planRaw := stackingSnapshotResumeFixture(t, miniStackWorkflowTOML, map[string]string{"task": "add feature"})
	...
	chunkRun, chunkRaw := stackingSnapshotResumeFixture(t, miniStackWorkflowTOML, map[string]string{
		"task": "add feature", "stack_mode": "chunk", "chunk": "c1", "stack_part": "1/1",
	})
...
func TestPlanMissingOrCorruptDirect(t *testing.T) {
	...
	run, runRaw := stackingSnapshotResumeFixture(t, miniStackWorkflowTOML, map[string]string{"task": "add feature"})
