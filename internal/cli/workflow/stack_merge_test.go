...
// attempt passes (autoDeliveryAttemptTimeout) can never actually stop
...
func TestWaitForIntegrationMergeHonorsCancelledContext(t *testing.T) {
	...
	seedGitMergeDeliveryPendingRun(t, repo, run, []byte("{}"))
...
// seedMergeIntegrationRunAdmitted seeds a delivery_pending integration run row with
// the stable admission key <stackID>:integration.
func seedMergeIntegrationRunAdmitted(t *testing.T, repo workflowledger.Repository, stackID string, noWorktree bool) workflowledger.RunSnapshot {
	t.Helper()
	worktreeName := ""
	if !noWorktree {
...
func TestAutoDeliverReviewedChunksRetriesOrphanedDelivery(t *testing.T) {
	...
	seedGitMergeDeliveryPendingRun(t, repo, run, snapshotJSON)
...
func TestWaitIntegrationRunSettledAutoPolicyDeliversWithoutAllowPublish(t *testing.T) {
	...
	seedMergeIntegrationRunAdmitted(t, repo, stackID, false)
...
func TestWaitIntegrationRunSettledAutoPolicyMergesExternallyDeliveredRun(t *testing.T) {
	...
	run := seedMergeIntegrationRunAdmitted(t, repo, stackID, false)
...
func TestWaitIntegrationRunSettledNoDiffIntegrationCompletesWithoutMerge(t *testing.T) {
	...
	run := seedMergeIntegrationRunAdmitted(t, repo, stackID, true)
	stored, err := repo.GetRun(context.Background(), run.RunID)
	if err != nil {
		t.Fatal(err)
	}
