...
// seedGitMergeDeliveryPendingRun admits a run row and settles it to delivery_pending
// through the valid pending->running->delivery_pending transitions.
func seedGitMergeDeliveryPendingRun(t *testing.T, repo workflowledger.Repository, run workflowledger.RunSnapshot, snapshotJSON []byte) {
	t.Helper()
