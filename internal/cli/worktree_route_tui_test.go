...
func TestWorktreeDialogCreateStoresRouteInMainRepositoryCatalog(t *testing.T) {
	...
	sessionStore, err := openContextStorePath(storePath)
	if err != nil {
		t.Fatal(err)
	}
	...
	contextStore, err := setupRepositorySessionContext(restarted.session, repoRoot, storePath, &config.Resolved{Subagents: config.DefaultSubagentConfig})
	if err != nil {
		t.Fatal(err)
	}
