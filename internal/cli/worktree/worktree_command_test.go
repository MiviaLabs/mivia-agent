...
// TestExecuteWorktreeCommandIsRegistered lives in
// worktree_command_external_test.go (package cliworktree_test): it needs
// internal/cli's Execute, and internal/cli imports internal/cli/worktree, so
// an in-package (white-box) test file here cannot import cli without the Go
// toolchain rejecting it as an import cycle. An external test package has no
// such restriction and links into the same test binary, so its import of cli
// also satisfies this package's OpenRepositoryContextStoreFunc wiring (see
// that var's doc comment) for every test in this binary, including the ones
// below.
...
func TestWorktreeCommandCreateListRemove(t *testing.T) {
	...
	routeStore, err := storage.OpenSQLite(storePath)
	if err != nil {
