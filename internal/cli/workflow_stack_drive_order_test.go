...
// with a stored output on the given run, the shape loadStackPlanOutput reads.
func seedSucceededDecomposeAttempt(t *testing.T, repo workflowledger.Repository, runID string, output []byte) {
	...
	attempt := workflowledger.StepAttempt{AttemptID: "wfa-decompose-1", RunID: runID, StepID: stackDecomposeStepID, AttemptNo: 1}
