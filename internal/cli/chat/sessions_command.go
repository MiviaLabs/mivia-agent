...
func runSessionsWithIO(args []string, stdout, stderr io.Writer) error {
	...
	default:
		return fmt.Errorf("sessions: unknown subcommand %q", agents.SafeCatalogText(subcommand, 80))
	}
}
...
func parseSessionsWorkspaceAndJSON(cmdLabel string, args []string, allowPositional int) (workspaceRoot string, jsonFlag bool, positional []string, err error) {
	rest := args
	workspaceRoot, rest, _, err = FlagValueFunc(rest, "--workspace")
	if err != nil {
	...
	for _, arg := range rest {
		...
		case strings.HasPrefix(arg, "-"):
			return "", false, nil, fmt.Errorf("%s: unknown flag %q", cmdLabel, agents.SafeCatalogText(arg, 80))
		default:
...
func runSessionsShow(args []string, stdout io.Writer) error {
	...
	var limitStr string
	var err error
	limitStr, rest, _, err = FlagValueFunc(rest, "--limit")
	if err != nil {
