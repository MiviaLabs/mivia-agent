...
func TestSettleIntegrationRunBlockedByUnsettledWave(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			waitIntegrationRunSettledFn = func(_ context.Context, _ *workflow.PreparedWorkflowRun, _ *workflowledger.Store, _ MergeChecker, _, _ string, _ bool, _, _ io.Writer) error {
				admitted = true
				return nil
			}
...
func seedMergedChunks(t *testing.T, prepared *workflow.PreparedWorkflowRun, stackID string, chunks []ChunkPlan) *workflowledger.Store {
	t.Helper()
