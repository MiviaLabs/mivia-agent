...
func TestFinalAuditDoctorHostileResolverKeylessAndCloud(t *testing.T) {
	...
	if err := orchestrate.RunDoctorWithIO([]string{"--config", loopbackCfg, "--workspace", t.TempDir()}, &loopbackOut, &loopbackErr); err != nil {
	...
	err := orchestrate.RunDoctorWithIO([]string{"--config", cloudCfg, "--workspace", t.TempDir()}, &cloudOut, &cloudErr)
	if err == nil {
