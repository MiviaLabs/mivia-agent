...
// halfBlockRender is an alternate denser-color path: each char = 2 vertical pixels
// using ▀ with fg/bg. Useful later for truecolor; braille preferred for mono fidelity.
func (g *pixelGrid) renderHalfBlocks(fg, bg string) string {
	// Even height; each row of chars covers 2 pixel rows.
	rows := (g.h + 1) / 2
	var b strings.Builder
	esc := func(on bool) string {
		if on {
			return "\x1b[38;5;" + fg + "m"
		}
		return "\x1b[38;5;" + bg + "m"
	}
	reset := "\x1b[0m"
	for row := 0; row < rows; row++ {
		if row > 0 {
