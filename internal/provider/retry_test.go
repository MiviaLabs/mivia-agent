...
func TestRetryRoundTripper_RetryOn429ThenSucceed(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		n := calls.Add(1)
		if n <= 2 {
			// Headers only reach the wire when set before WriteHeader.
...
func TestDefaultRetryOptionsAreSensible(t *testing.T) {
	...
	// Four retries plus the initial request is the five-attempt budget.
...
func TestRetryRoundTripper_NewOpenAICompatUsesRetry(t *testing.T) {
	...
	if rt.opts.MaxRetries != 4 {
		t.Fatalf("expected 4 retries, got %d", rt.opts.MaxRetries)
	}
}
