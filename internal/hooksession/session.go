// Package hooksession owns the running session's resolved lifecycle-hook
...
var sessionHookState atomic.Pointer[Session]

// Current returns the running session's hook state, or nil if none is
// installed. Every method on *Session is nil-safe, so callers may use the
// result directly without a nil check.
func Current() *Session { return sessionHookState.Load() }
...
func (s *Session) NoteRunWarnings(warnings []string) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	s.runWarnings = append(s.runWarnings, warnings...)
	if extra := len(s.runWarnings) - maxRunWarnings; extra > 0 {
		s.runWarnings = append([]string{}, s.runWarnings[extra:]...)
	}
}

// Install resolves this session's lifecycle hooks and publishes the result
// for /hooks and the dispatcher wiring. The returned function releases the
// handle at session end.
//
// Notices (load warnings, the armed-hook summary, and the project-hook
// disclosure) are returned rather than printed, so this package stays a leaf:
// printing is the caller's job (internal/cli's wrapper calls
// cliagents.WarnHookLoad with them, matching the pre-migration behavior).
//
// quiet (--quiet) suppresses the armed notice and the project-hook
// disclosure: the operator explicitly asked for quieter startup, and /hooks
// still lists every armed hook on demand. Genuine load warnings always print.
func Install(workspaceRoot string, staleBypass, quiet bool) (release func(), notices []string, err error) {
	...
	return func() { sessionHookState.Store(nil) }, notices, nil
}
...
func SetForTest(s *Session) (restore func()) {
	...
	sessionHookState.Store(s)
	return func() { sessionHookState.Store(previous) }
}
...
func (s *Session) ArmedNotice() []string {
	...
	labels := make([]string, 0, len(s.groups))
...
func (s *Session) RunnableGroups() []hooks.Group {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	return append([]hooks.Group(nil), s.groups...)
}
...
func (s *Session) List() string {
	...
	for _, warning := range append(append([]string{}, s.warnings...), s.runWarnings...) {
