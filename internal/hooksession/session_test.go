...
func loadHooksIn(t *testing.T, ws string) *Session {
	t.Helper()
	session, err := Load(ws)
	if err != nil {
		t.Fatalf("Load: %v", err)
...
func TestEveryConfiguredHookRunsWithoutConfirmation(t *testing.T) {
	...
	if got := len(session.RunnableGroups()); got != 2 {
...
func TestHooksTrustExplainsThatConfirmationIsGone(t *testing.T) {
	out := strings.ToLower(SlashOutput([]string{"/hooks", "trust", "1"}))
...
func TestProjectHooksLoadAlongsideUserHooks(t *testing.T) {
	...
	if got := len(session.RunnableGroups()); got != 3 {
...
func TestUserOnlyHooksGetNoProjectNotice(t *testing.T) {
	...
	if strings.Contains(session.List(), "[project]") {
...
func TestHooksListOnNoHooksSaysSo(t *testing.T) {
	_, ws := hookHome(t, "[provider]\nname = \"openai\"\n")
	if out := loadHooksIn(t, ws).List(); !strings.Contains(strings.ToLower(out), "no lifecycle hooks") {
		t.Fatalf("an empty listing must say so, got %q", out)
	}
}

// Install must return notices rather than print them, so this package stays a
// leaf with no dependency on how a caller reports startup diagnostics.
func TestInstall_ReturnsNoticesRatherThanPrinting(t *testing.T) {
	_, ws := hookHome(t, twoHookConfig)
	writeWorkspaceHooks(t, ws, projectHookConfig)

	release, notices, err := Install(ws, false, false)
	if err != nil {
	...
	if !strings.Contains(joined, "lifecycle hooks armed") {
		t.Fatalf("notices must include the armed summary; got %v", notices)
	}
	if !strings.Contains(joined, "someone else wrote them") {
		t.Fatalf("notices must include the project-hook disclosure; got %v", notices)
	}

	release2, quietNotices, err := Install(ws, false, true)
	if err != nil {
		t.Fatalf("Install (quiet): %v", err)
	}
	defer release2()
	quietJoined := strings.Join(quietNotices, "\n")
	if strings.Contains(quietJoined, "lifecycle hooks armed") || strings.Contains(quietJoined, "someone else wrote them") {
		t.Fatalf("quiet must suppress the armed summary and project disclosure; got %v", quietNotices)
	}
}

func TestInstall_LoadWarningsSurviveQuiet(t *testing.T) {
	_, ws := hookHome(t, twoHookConfig)
	writeWorkspaceHooks(t, ws, "[[hooks]]\nevent = \"SessionStart\"\n\n  [[hooks.handlers]]\n  type = \"command\"\n  argv = [\"./h.sh\"]\n")

	release, notices, err := Install(ws, false, true)
	if err != nil {
		t.Fatalf("Install: %v", err)
	}
	defer release()
	if !strings.Contains(strings.Join(notices, "\n"), "ignoring lifecycle hooks") {
		t.Fatalf("a load warning must survive --quiet, got %v", notices)
	}
}
