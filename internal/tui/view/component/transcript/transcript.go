...
// It handles every uievent.Kind exhaustively, mirroring internal/tui/view/
// stream's plain-text renderer but styled through internal/tui/view/theme.
//
...
type Model struct {
	...
	// DurationMS, but no producer sets it (adapter.translateToolEnd and
...
func (m Model) Clear() Model {
	...
	// next. /clear is accepted mid-turn (adapter's handleClear does
...
func (m Model) handleToolOutput(b uievent.ToolOutputBody) (Model, tea.Cmd) {
	...
	// sidebar panel's job now (internal/tui/view/screen/conversation/filespanel.go
...
// internal/tui/view/screen/conversation); this only lets a running tool
