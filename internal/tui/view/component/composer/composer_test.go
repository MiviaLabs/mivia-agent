...
func TestMentionMenuActiveOnAt(t *testing.T) {
	...
	m.SetMentions([]Mention{
		{Path: "internal/tui/view/component/composer/composer.go", Display: "composer.go"},
	})
	m.SetValue("fix @comp")
...
func TestMentionMenuAcceptInsertPath(t *testing.T) {
	m := New(loadTheme(t), theme.TierASCII, 80)
	m.SetMentions([]Mention{{Path: "internal/tui/view/component/composer/mention.go"}})
	...
	if !strings.Contains(m.Value(), "@internal/tui/view/component/composer/mention.go") {
