...
import (
	...
	"github.com/MiviaLabs/mivia-agent/internal/tui/view/screen/transcript"
)
...
func TestViewHasAOneColumnGutter(t *testing.T) {
	...
	// The persistent pieces are present: top bar (row 1 after top gutter),
	// framed composer at the bottom.
	if !strings.Contains(ansi.Strip(rows[1]), "Replay Session") && !strings.Contains(ansi.Strip(rows[1]), "mivia") {
		t.Errorf("row 1 is not the top bar: %q", rows[1])
	}
	var foundInput bool
...
func TestConversationScreenMentionMenuKeys(t *testing.T) {
	...
	s.SetMentions([]composer.Mention{
		{Path: "internal/tui/view/component/composer/composer.go"},
		{Path: "internal/tui/view/component/composer/mention.go"},
	})
	next, _ := s.Update(tea.WindowSizeMsg{Width: 80, Height: 24})
	scr := next.(Screen)
	...
	if !strings.Contains(scr.composer.Value(), "@internal/tui/view/component/composer/composer.go") {
