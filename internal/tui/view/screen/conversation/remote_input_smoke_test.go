...
// internal/tui/adapter/conversation.go): 1 turn.start + 19 tool.start + 19
...
// internal/tui/adapter.Conversation.Send derives its synthetic turn.start
...
// internal/tui/adapter's Conversation.Send does in production. A consumer that
...
func (c *oversizedTurnConversation) Send(_ context.Context, in intent.Send) (ports.TurnHandle, error) {
	...
	// Same derivation internal/tui/adapter/conversation.go's real Send uses:
...
// (forwardRemoteInputs, internal/tui/adapter/session_pool.go pre-0a709d80)
