...
// see internal/tui/adapter/conversation.go's Conversation.Send, which uses
...
// docs on internal/tui/adapter's SessionPool.RemoteInputs (SessionPool itself
// never calls Send).
//
...
// (internal/tui/adapter/conversation.go), not from the ctx argument itself.
//
