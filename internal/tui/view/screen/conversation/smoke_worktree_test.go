...
// offline-smoke-test): the REAL *adapter.CommandRunner wired into a
// REAL conversation Screen - the exact tui.buildApp construction chain
...
// by field, not by position (see worktreeCatalog in internal/tui/adapter).
...
// buildSmokeScreen mirrors tui.buildApp: pool-sourced conversation,
// real CommandRunner as the screen's command seam, mandatory setters only.
func buildSmokeScreen(t *testing.T, sess *chat.Session, res *config.Resolved) (Screen, *adapter.CommandRunner) {
	t.Helper()
	...
	conv, ok := convPort.(*adapter.Conversation)
	if !ok {
		t.Fatalf("pool returned %T, want *adapter.Conversation", convPort)
	}
