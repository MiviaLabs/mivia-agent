...
func TestFormatDiagnosticsOutput(t *testing.T) {
	th := loadTheme(t)
	raw := "internal/tui/view/app.go:10:5: syntax error\ninternal/tui/view/app.go:15:2: warning: unused variable"
...
func TestFormatGrepOutput_StandardRipgrep(t *testing.T) {
	th := loadTheme(t)
	raw := "internal/tui/view/app.go:12:func New() Model {\ninternal/tui/view/app.go:45:return m\ninternal/tui/view/render/syntax.go:8:func HighlightCode("
	summary, lines := FormatGrepOutputWithContext(th, theme.TierTrueColor, "func", raw, 80)
	if !strings.Contains(summary, "3 matches in 2 files") {
		t.Errorf("unexpected summary: %q", summary)
	}
	plain := ansi.Strip(strings.Join(lines, "\n"))
	if !strings.Contains(plain, "internal/tui/view/app.go (2 matches)") {
