...
func TestFormatToolDetail(t *testing.T) {
	tests := []struct {
		...
		{
			name: "read_file",
			args: map[string]any{"file_path": "internal/tui/view/app.go"},
			want: "internal/tui/view/app.go",
		},
		{
			name: "view_file",
...
func TestApprovalDetailRow(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "edit_file",
			args:      map[string]any{"path": "internal/tui/view/app.go"},
			wantLabel: "File",
			wantValue: "internal/tui/view/app.go",
			wantOK:    true,
		},
		{
			name:      "view_file",
