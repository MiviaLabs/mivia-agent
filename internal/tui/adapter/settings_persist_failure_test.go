...
func settingsStoreWithUnwritableConfig(t *testing.T) *adapter.SettingsStore {
	t.Helper()
	res := &config.Resolved{Model: "test-model", ConfigPath: unwritableConfigPath(t)}
	sess := chat.NewSession(res, nil)
	return adapter.NewSettingsStore(sess, res, nil)
}
...
func TestSettingsGeneralEditRollbackRestoresConversationState(t *testing.T) {
	store := settingsStoreWithUnwritableConfig(t)
	conv := adapter.NewConversation(chat.NewSession(&config.Resolved{Model: "test-model"}, nil))
