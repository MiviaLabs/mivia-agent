...
func New(ctx context.Context, in Input) (*Adapter, func(), error) {
	...
	if err != nil {
		...
		return nil, nil, fmt.Errorf("adapter: build session: %w", err)
	}
...
func validateInput(in Input) error {
	if in.Resolved == nil {
		return fmt.Errorf("adapter: resolved config is required")
	}
	if in.StorePath == "" {
		return fmt.Errorf("adapter: store path is required")
	}
	if in.HooksConfigured && in.WorkspaceRoot == "" {
		return fmt.Errorf("adapter: workspace root is required when hooks are configured")
	}
	return nil
}
