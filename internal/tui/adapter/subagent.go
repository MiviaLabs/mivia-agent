...
// internal/ledger, because internal/ledger is not in adapter's
...
// indirection shape, and for the same reason - internal/tui/adapter must
// never import internal/cli* (INV-TUI-29). Only internal/tui/run, which
	...
	case uievent.KindToolEnd:
		...
		for i := range c.history[lastIdx].ToolCalls {
			if c.history[lastIdx].ToolCalls[i].ID == body.ToolCallID {
				...
				// The outcome rides the same event that settles the call,
				// so a later reader of history (the dispatch row's child
				// tree) never has to guess it from the output text.
				c.history[lastIdx].ToolCalls[i].OK = body.OK
				if body.Diff != nil {
