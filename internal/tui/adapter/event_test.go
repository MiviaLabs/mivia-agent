...
func runMappingCases(t *testing.T, cases []mappingCase) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := adapter.TranslateEvent(tc.ev)
...
func TestTranslateEvent_NoticeOptions(t *testing.T) {
	...
	// When enabled:
	opts := adapter.TranslateOptions{
		ShowIterationNotices:   true,
		ShowPromptCacheNotices: true,
	}

	gotStep := adapter.TranslateEventWithOptions(stepEv, opts)
	...
	gotCache := adapter.TranslateEventWithOptions(cacheEv, opts)
...
func TestTranslateEvent_LogsAndDropsUnknownKind(t *testing.T) {
	...
	got := adapter.TranslateEvent(agent.Event{Kind: agent.EventKind("definitely_not_in_the_switch")})
	if got != nil {
...
func safeTranslate(t *testing.T, ev agent.Event) (out []uievent.Event) {
	...
	return adapter.TranslateEvent(ev)
}
...
func TestParseArgs(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := adapter.ParseArgsForTest(tc.in)
...
// adapter commit and what this test keeps under coverage.
func TestErrFromDetail(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := adapter.ErrFromDetailForTest(tc.detail, tc.ok)
			if got != tc.want {
