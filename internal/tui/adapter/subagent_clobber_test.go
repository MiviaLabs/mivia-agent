...
// live path has.
func startLiveThread(t *testing.T, threads *adapter.SubagentThreads, taskID, callID, agentName string) ports.Conversation {
	t.Helper()
	...
	stc, ok := conv.(*adapter.SubagentTranscriptConversation)
	if !ok {
...
func TestPopulateFromToolCalls_DoesNotClobberInFlightLiveThread(t *testing.T) {
	...
	adapter.PopulateFromToolCalls(threads, dispatchMsg("call_d1", "t1", "builder", ""))
...
func TestPopulateFromToolCalls_DoesNotClobberCompletedLiveThread(t *testing.T) {
	threads := adapter.NewSubagentThreads()
	live := startLiveThread(t, threads, "t1", "call_d1", "builder")
	if stc, ok := live.(*adapter.SubagentTranscriptConversation); ok {
		stc.RecordEvent(uievent.Event{
	...
	adapter.PopulateFromToolCalls(threads, dispatchMsg("call_d1", "t1", "builder", output))
...
func TestPopulateFromToolCalls_ReconstructionDoesNotStealLiveAgentNameKey(t *testing.T) {
	...
	adapter.PopulateFromToolCalls(threads, dispatchMsg("call_old", "t-old", "builder", output))
