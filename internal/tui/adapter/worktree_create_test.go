...
func TestCommandRunner_StartInNewWorktree_GuardArms(t *testing.T) {
	...
	// No active session.
	r := adapter.NewCommandRunner(nil, res, nil)
	...
	sess.SessionID = "s0"
	r2 := adapter.NewCommandRunnerWithPool(sess, nil, res, nil)
	...
	pool := adapter.NewSessionPool(sess, res, nil, false)
	r3 := adapter.NewCommandRunnerWithPool(sess, pool, res, nil)
	...
	r4 := adapter.NewCommandRunner(sess4, res, nil)
