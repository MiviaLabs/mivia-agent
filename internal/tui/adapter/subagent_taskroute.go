...
func (s *SubagentThreads) resolveTaskRoute(callID string) (SubagentTaskCoordinator, *coordinator.RunHandle, string, error) {
	...
	if route.coord == nil {
		return nil, nil, "", fmt.Errorf("adapter: no coordinator wired to reach subagent task %q", callID)
	}
	h := route.coord.HandleForRun(route.runID)
	if h == nil {
		return nil, nil, "", fmt.Errorf("adapter: run %q for subagent task %q is no longer active", route.runID, callID)
	}
