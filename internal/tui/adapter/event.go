// Package adapter translates domain events (internal/agent.Event) into
// the canonical UI event stream (internal/tui/kit/uievent.Event). It is the
...
func TranslateEventWithOptions(ev agent.Event, opts TranslateOptions) []uievent.Event {
	...
	case agent.EventAssistantReset:
		return translateAssistantReset(ev)
	...
	log.Printf("adapter: TranslateEvent has no case for agent.EventKind %q; dropping", string(ev.Kind))
	return nil
}
