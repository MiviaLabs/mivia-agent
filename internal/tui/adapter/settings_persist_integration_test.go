...
func TestSettingsStore_ApplyGeneral_LiveSyncAndPersist(t *testing.T) {
	...
	store := adapter.NewSettingsStore(nil, res, state)

	conv := adapter.NewConversation(nil)
...
func TestSettingsStore_ApprovalDefault_ReloadsFromConfig(t *testing.T) {
	...
	fresh := adapter.NewSettingsStore(nil, reloadedRes, state)
...
func applyProjectTestEdits(t *testing.T, store *adapter.SettingsStore) {
	t.Helper()
...
// a recording notifier into the store (mirroring how tui/run.go
...
func TestSettingsStore_ApplySync_FiresLiveReArmNotifier(t *testing.T) {
	...
	// tui/run.go does this for real; tests wire their own to assert
...
func TestSettingsStore_ApplySync_NotFiredOnPersistFailure(t *testing.T) {
	...
	sess := chat.NewSession(res, nil)
	store := adapter.NewSettingsStore(sess, res, nil)
