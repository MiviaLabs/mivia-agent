...
func TestSubagentThreads_RegisterAndLookup(t *testing.T) {
	...
	conv := adapter.NewSubagentTranscriptConversation("subagent-1", modelInfo, history)
...
func TestSubagentTranscriptConversation_ActiveTurn(t *testing.T) {
	conv := adapter.NewSubagentTranscriptConversation("worker", ports.ModelInfo{}, nil)
...
// object-reuse path: getOrCreate (internal/tui/adapter/subagent.go) returns the
...
func TestSubagentTranscriptConversation_EmptyTitle(t *testing.T) {
	conv := adapter.NewSubagentTranscriptConversation("", ports.ModelInfo{}, nil)
...
func TestPopulateFromToolCalls_DispatchTasks(t *testing.T) {
	...
	adapter.PopulateFromToolCalls(threads, msgs)
...
func TestPopulateFromToolCalls_PreFeatureJSONShapeFallsBackCleanly(t *testing.T) {
	...
	reconstruct := func(t *testing.T, output string) ports.Message {
		t.Helper()
		threads := adapter.NewSubagentThreads()
		adapter.PopulateFromToolCalls(threads, buildMsgs(output))
...
func TestNamespacedTaskID(t *testing.T) {
	if got := adapter.NamespacedTaskIDForTest("", "raw"); got != "raw" {
		t.Errorf("empty namespace: got %q, want raw", got)
	}
	if got := adapter.NamespacedTaskIDForTest("ns", ""); got != "" {
		t.Errorf("empty rawID: got %q, want empty", got)
	}
	if got := adapter.NamespacedTaskIDForTest("ns", "raw"); got != "ns:raw" {
...
func TestListenerOverflowIsCountedNotSilent(t *testing.T) {
	c := adapter.NewSubagentTranscriptConversation("worker", ports.ModelInfo{}, nil)
