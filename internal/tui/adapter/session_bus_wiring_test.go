...
// stubBusRegistrar is a test double for adapter.SessionBusRegistrar that
// records every bind/unbind call. internal/tui/adapter must never import
// internal/cli (INV-TUI-29), so this test exercises the indirection var
// directly, exactly as internal/tui/run/run.go's production wiring does.
...
// adapter.SessionBusRegistrar for the test and restores the previous
// value (nil, in every other test in this package) on cleanup.
func withStubBusRegistrar(t *testing.T) *stubBusRegistrar {
	t.Helper()
	prev := adapter.SessionBusRegistrar
	stub := newStubBusRegistrar()
	adapter.SessionBusRegistrar = stub.register
	t.Cleanup(func() { adapter.SessionBusRegistrar = prev })
	return stub
}
...
// never wires internal/tui/run) never panics and simply produces no
...
func TestSessionPool_BusWiring_NilRegistrarIsNoOp(t *testing.T) {
	...
	installTestAuthToken(t)
	prev := adapter.SessionBusRegistrar
	adapter.SessionBusRegistrar = nil
	t.Cleanup(func() { adapter.SessionBusRegistrar = prev })
