...
func setupSessionStoreFixture(t *testing.T) (*chat.Session, *config.Resolved, *storage.SQLite, func()) {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "adapter-runner-test-*")
	if err != nil {
		t.Fatal(err)
	}
	comp := &nullCompleter{}
...
func TestCommandRunner_Effort(t *testing.T) {
	...
	runnerNoReasoning := adapter.NewCommandRunner(sessNoReasoning, resNoReasoning, nil)
	...
	runnerWithReasoning := adapter.NewCommandRunner(sessWithReasoning, resWithReasoning, nil)
...
func TestCommandRunner_SelectModel_InResumedSession(t *testing.T) {
	...
	pool := adapter.NewSessionPool(sess1, res, nil, false)
	runner := adapter.NewCommandRunnerWithPool(sess1, pool, res, nil)
...
func TestCommandRunner_SetActiveSessionID(t *testing.T) {
	...
	pool := adapter.NewSessionPool(sess1, res, nil, false)
	runner := adapter.NewCommandRunnerWithPool(sess1, pool, res, nil)
	selectModel := func(r *adapter.CommandRunner, name string) {
		t.Helper()
	...
	var nilRunner *adapter.CommandRunner
	...
	noPoolRunner := adapter.NewCommandRunnerWithPool(sess1, nil, res, nil)
...
func TestCommandRunner_SkillsIntegration(t *testing.T) {
	...
	runner := adapter.NewCommandRunner(nil, nil, state)
...
func TestCommandRunner_SkillInvocationPersistsTheShortForm(t *testing.T) {
	...
	runner := adapter.NewCommandRunner(nil, nil, &cliagents.AgentSessionState{SkillRegFull: skillReg})
