...
func setupTestSettingsStore(t *testing.T) *adapter.SettingsStore {
	t.Helper()
	...
	return adapter.NewSettingsStore(nil, res, state)
}
...
func TestProviderSettings_ActivateModelWithSession(t *testing.T) {
	...
	store := adapter.NewSettingsStore(sess, res, state)
	settings := store.Settings()
...
func TestSettingsStore_SetActiveSession_SwitchesModelOnActiveSession(t *testing.T) {
	...
	store := adapter.NewSettingsStore(sess1, res, state)
