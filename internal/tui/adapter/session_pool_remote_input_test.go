...
// withFixedAuthorUserIDProvider overrides adapter.AuthorUserIDProvider for
...
func withFixedAuthorUserIDProvider(t *testing.T, id string) {
	t.Helper()
	orig := adapter.AuthorUserIDProvider
	adapter.AuthorUserIDProvider = func() chatsync.AuthorUserIDProvider {
		return func(context.Context) (string, error) { return id, nil }
	}
	t.Cleanup(func() { adapter.AuthorUserIDProvider = orig })
}
...
func newRemoteInputPool(t *testing.T, srv *httptest.Server, sessionID string, created *int32) *adapter.SessionPool {
	t.Helper()
