...
// and one noop tool, wrapped in adapter.NewConversation. t.Cleanup
// closes any goroutines that may still be alive.
func newTestConversation(t *testing.T, completer provider.Completer, msgs ...provider.Message) *adapter.Conversation {
	t.Helper()
	...
	conv := adapter.NewConversation(sess)
	return conv
}
...
func TestSend_FullTurn_RestoresPriorHandler(t *testing.T) {
	...
	c := adapter.NewConversation(sess)
...
func TestSend_GoroutineLeak(t *testing.T) {
	...
	c := adapter.NewConversation(sess)
	var wg sync.WaitGroup
	adapter.SetTurnWaiterForTest(&wg)
	defer adapter.SetTurnWaiterForTest(nil)
	const n = 20
	for i := 0; i < n; i++ {
		wg.Add(1)
		h, err := c.Send(context.Background(), intent.Send{Text: "x"})
...
func TestConversation_DoesNotImportCLIFamily(t *testing.T) {
	...
	cmd := exec.Command("go", "list", "-deps", "./internal/tui/adapter")
	out, err := cmd.Output()
	if err != nil {
...
// other half of the isolation contract: ./internal/tui/view/... and
// ./internal/tui/kit/... must not import adapter (that would create a
// reverse dependency). Not wired to make verify.
func TestUIPackages_DoNotImportUIAdapter(t *testing.T) {
	if testing.Short() {
		t.Skip("invariants skip in -short")
	}
	for _, target := range []string{"./internal/tui/view/...", "./internal/tui/kit/..."} {
		...
		for _, l := range strings.Split(string(out), "\n") {
			if l == "github.com/MiviaLabs/mivia-agent/internal/tui/adapter" {
				t.Fatalf("%s transitively imports adapter", target)
			}
		}
	}
}
...
//     internal/tui/view/stream/stream.go's defensive guard).
//
...
func registerCapturingSubagentProgress(t *testing.T) (registered func() bool, capturedFn func(agent.Event), clearedCh chan struct{}) {
	...
	prev := adapter.SubagentProgressRegistrar
	adapter.SubagentProgressRegistrar = func(fn func(agent.Event)) func() {
		realFn = fn
		return func() { close(clearedCh) }
	}
	t.Cleanup(func() { adapter.SubagentProgressRegistrar = prev })
...
func newTestModelSwitchFixture(t *testing.T, comp provider.Completer) (*chat.Session, *config.Resolved, *adapter.CommandRunner, *adapter.Conversation) {
	t.Helper()
	res := &config.Resolved{
		ProviderName: "ollama",
		Model:        "model-1",
