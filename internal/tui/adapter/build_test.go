// build_test.go exercises internal/tui/adapter.New end-to-end against a
...
func TestNew_BuildsCleanSession(t *testing.T) {
	...
	adapter, cleanup, err := adapter.New(context.Background(), adapter.Input{
...
func TestNew_RequiresResolved(t *testing.T) {
	_, _, err := adapter.New(context.Background(), adapter.Input{
		Resolved:  nil,
		Completer: &nullCompleter{},
		StorePath: filepath.Join(t.TempDir(), "ckpt.sqlite"),
	})
