...
func buildRestartScenario(t *testing.T) restartScenario {
	...
	runnerMain := adapter.NewCommandRunner(main, res, nil)
	...
	pool2 := adapter.NewSessionPool(restart, res, nil, false)
	runner := adapter.NewCommandRunnerWithPool(restart, pool2, res, nil)
	t.Chdir(mainDir)
...
func seedBoundWorktreeSession(t *testing.T, store *storage.SQLite, mainDir, canonicalWt string, res *config.Resolved) {
	...
	convBound := adapter.NewConversation(bound)
...
func seedTurnOnlyWorktreeSession(t *testing.T, store *storage.SQLite, mainDir, canonicalWt string, res *config.Resolved) string {
	...
	installCtx(t, bound, store, mainDir)
	turn(t, adapter.NewConversation(bound), "wt turn one")
...
func TestResumeInWorktree_TurnOnlySessionFromListingRestoresHistory(t *testing.T) {
	...
	withTools(restart)
	installCtx(t, restart, store2, mainDir)
	pool := adapter.NewSessionPool(restart, res, nil, false)
	runner := adapter.NewCommandRunnerWithPool(restart, pool, res, nil)
	t.Chdir(mainDir)
