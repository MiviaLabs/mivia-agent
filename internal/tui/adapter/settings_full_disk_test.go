...
func fullDiskFixture(t *testing.T) (store *adapter.SettingsStore, wsConfig, userConfig string) {
	t.Helper()
	home := t.TempDir()
	t.Setenv("HOME", home)
	ws := t.TempDir()
	...
	return adapter.NewSettingsStore(nil, res, state), wsConfig, userConfig
}
...
func TestSettingsGeneralViewSeedsFullDiskFromUserConfig(t *testing.T) {
	...
	store2 := adapter.NewSettingsStore(nil, res2, state)
