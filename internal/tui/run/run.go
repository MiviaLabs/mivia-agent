...
import (
	...
	"github.com/MiviaLabs/mivia-agent/internal/tui/kit/termprobe"
...
func registerSubagentProgress() {
	adapter.SubagentProgressRegistrar = func(fn func(agent.Event)) func() {
		token := cli.SetSubagentProgress(fn)
		return func() {
			cli.ClearSubagentProgress(token)
		}
	}
	adapter.SessionBusRegistrar = cli.RegisterSessionBus
	...
	adapter.SubagentTaskRouteRegistrar = func(sink func(coord adapter.SubagentTaskCoordinator, callID, runID, taskID string)) {
		cli.SetSubagentTaskRouteSink(func(coord cli.OrchestrationCoordinator, callID, runID, taskID string) {
			...
			if c, ok := coord.(adapter.SubagentTaskCoordinator); ok {
...
func wireMouseNotifier(store *adapter.SettingsStore, p *tea.Program) {
	if store != nil {
...
func wireFullDiskNotifier(store *adapter.SettingsStore, p *tea.Program) {
	if store != nil {
...
func wireSyncOptsNotifier(store *adapter.SettingsStore, pool *adapter.SessionPool) {
...
func persistTheme(store *adapter.SettingsStore, name string) tea.Cmd {
	return func() tea.Msg {
...
// automationSessionSpawner adapts *adapter.SessionPool to
// automation.SessionSpawner, the standard Go composition-root adapter:
// this package (internal/tui/run) is the only place that names both
// adapter.BindFunc (a named type) and automation.SessionSpawner's
...
// wrapping *adapter.SessionPool, not a bare func type, so it can also
...
func (a automationSessionSpawner) CreateFreshInDir(bind func(*chat.Session) (string, error), dir string) (ports.Conversation, error) {
	...
	return a.pool.CreateFreshBackgroundInDir(adapter.BindFunc(bind), dir)
}
...
func (a automationSessionSpawner) GetOrResumeInDir(id string, dir string) (ports.Conversation, *chat.Session, error) {
	...
	c, ok := conv.(*adapter.Conversation)
	if !ok {
		return nil, nil, fmt.Errorf("automation spawner: pooled conversation is %T, want *adapter.Conversation", conv)
	}
	if !c.IsForeground() {
...
func newAutomationSpawner(pool *adapter.SessionPool) automation.SessionSpawner {
...
func wireAutomationBackend(store *adapter.SettingsStore, pool *adapter.SessionPool, sess *chat.Session, agentState *cli.AgentSessionState, res *config.Resolved) (closeFn func(), svc *automation.Service) {
...
func buildApp(sess *chat.Session, res *config.Resolved, toolsOn bool, agentState *cli.AgentSessionState, resumeSessionName string) (tea.Model, *adapter.SettingsStore, *adapter.CommandRunner, func(), error) {
	registerSubagentProgress()
	approver := adapter.NewApprover(sess)
	...
	runner := adapter.NewCommandRunner(sess, res, agentState)
	pool := runner.Pool()
	threads := pool.Threads()
	// NewCommandRunner's pool pre-registers sess under its own SessionID
	// (NewSessionPool) as a *adapter.Conversation (NewConversation's
	...
	conv := convPort.(*adapter.Conversation)

	settingsStore := adapter.NewSettingsStore(sess, res, agentState)
	...
	// remote input (internal/tui/adapter/remote_input.go); the screen is the
	...
	// (internal/tui/adapter/workflow_notices.go). It had no reader here at all,
