...
// built with a nil config (adapter.NewSessionPool(nil, nil, nil, ...))
...
func TestNewAutomationSpawnerReachesPoolCreateFreshInDir(t *testing.T) {
	pool := adapter.NewSessionPool(nil, nil, nil, false)
...
// adapter.BindFunc conversion forwards both
...
// since automationSessionSpawner now wraps a concrete *adapter.SessionPool
...
func TestWireAutomationBackendInstallsBackendOnSuccess(t *testing.T) {
	...
	store := adapter.NewSettingsStore(sess, res, agentState)
...
func TestWireAutomationBackendFallbackOpensSharedContextStorePath(t *testing.T) {
	...
	store := adapter.NewSettingsStore(sess, res, agentState)
	pool := adapter.NewCommandRunner(sess, res, agentState).Pool()
...
func TestAutomationSpawnerSpawnsBackgroundConversation(t *testing.T) {
	...
	if !ok {
		t.Fatalf("spawner returned %T, want *adapter.Conversation", conv)
	}
	if !c.IsBackground() {
...
func TestAutomationSpawnerGetOrResumeInDirRestoresSessionIdentity(t *testing.T) {
	...
	pool := adapter.NewCommandRunner(seed, res, nil).Pool()
	...
	c, ok := conv.(*adapter.Conversation)
	if !ok {
		t.Fatalf("restored conversation type = %T, want *adapter.Conversation", conv)
	}
	if !c.IsBackground() {
...
// observably acquired.
func foregroundLiveEnv(t *testing.T, sessionID string) (runner *adapter.CommandRunner, entered func() <-chan struct{}) {
	t.Helper()
...
func TestAutomationSpawnerGetOrResumeInDirPreservesForegroundSession(t *testing.T) {
	...
	c, ok := convPort.(*adapter.Conversation)
	if !ok {
		t.Fatalf("conversation type = %T, want *adapter.Conversation", convPort)
	}
	if !c.IsForeground() {
