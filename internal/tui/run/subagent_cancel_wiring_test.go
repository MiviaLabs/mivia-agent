...
// adapter.NewSubagentThreads hands its route table to the CLI side
...
// share one route lookup (adapter's resolveTaskRoute), so an empty route
// table made BOTH inert, not just the whole-task one.
//
// This package is where the test belongs: it is the only one that imports
// both halves (internal/cli and internal/tui/adapter), which is exactly why
// it is the package that wires them - internal/tui/adapter must never import
...
// internal/tui/view/screen/conversation's dispatchTaskIDsAndNames builds live,
// and what internal/tui/adapter/subagent_reconstruct.go rebuilds after a
...
// restoreRegistrars puts every package-level adapter registrar back the
// way it was; registerSubagentProgress writes three of them.
func restoreRegistrars(t *testing.T) {
	t.Helper()
	prevProgress := adapter.SubagentProgressRegistrar
	prevBus := adapter.SessionBusRegistrar
	prevRoutes := adapter.SubagentTaskRouteRegistrar
	t.Cleanup(func() {
...
// holding it.
func dispatchOneBlockingTask(t *testing.T) (*adapter.SubagentThreads, ledger.LedgerRepository) {
	t.Helper()
	restoreRegistrars(t)
	...
	threads := adapter.NewSubagentThreads()
...
func TestSubagentCancelWiring_ToolCallCancelResolvesTheSameRoute(t *testing.T) {
	...
	c, _ := coord.(adapter.SubagentTaskCoordinator)
