...
// path (internal/tui/adapter/event_kind.go's translateToolEnd, which is
// gated on the agent loop's own Detail string) and the historical
// replay path (internal/tui/adapter's History() and
// internal/tui/view/screen/conversation/session_state_builder.go's replay),
