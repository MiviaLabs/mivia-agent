...
> **Rule-60 note.** The predecessor plan repeatedly self-declared runtime-injected role text "rule-60-exempt". No such exemption exists in `.mivia/rules/60-*.md`, which draws no compiled-vs-runtime distinction for `Description()`. Rule 60's own "When editing tools" step says to update the rule when the generic contract changes, so a `chore(ai)` amendment is required — **but it ships with `05`**, which is the first plan to inject workspace text into a compiled description. By the time `07` runs, the amendment exists.
