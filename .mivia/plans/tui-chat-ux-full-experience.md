...
| Invariants | `.mivia/invariants.md`, `Makefile` `invariants` target |
...
1. **Update `.mivia/invariants.md`** with new tests (exact `func Test` names).
...
- Follow `AGENTS.md` + `.mivia/` + engineering-working-contract.
...
```
Read and execute .mivia/plans/tui-chat-ux-full-experience.md end-to-end (Phases A–G).
