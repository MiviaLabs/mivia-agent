...
| `.mivia/plans/37-reasoning-effort/` | ⛔ **Blocked after 2026-08-02 Step 0 re-audit** - current provider docs invalidate its provider-wide dialect/sampling premise; requires a capability-aware replacement before implementation |
...
| `.mivia/plans/51-harness-context-economics/` | 📋 **Draft program - Step 0 NOT run on any member.** Nine plans moving context cost control out of the prompt and into the harness: passive memory recall (`01` lexical, `02` static embeddings), structure- and seen-aware search (`03`), and six economy measures (`04`-`09`: per-class calibration, tool-schema gating, token-capped tail, pageable truncated remainders, a dispatcher shaping stage, superseded-result eviction). Program invariants in `00-overview.md` §4; INV-CE-A (one cost truth) and INV-CE-B (prefix stability) bind every member. `07`/`08` sit on plan `48` §3.1; `09` reuses plan `49`'s replacement path. `01` is HIGH blast radius and Step 0 should be willing to reject it outright. Do not implement from any of it |
