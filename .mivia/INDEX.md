...
| `.mivia/plans/33-lifecycle-hooks/` | ✅ **IMPLEMENTED** (`INV-AG-34`) - deterministic `PreToolUse`/`PostToolUse`/`Stop` layer, shipped as eight slices. Hooks load from **both** `~/.mivia/mivia.toml` and the workspace's own `.mivia/mivia.toml`, additively and user-first - the one setting mivia merges across layers, because a project's formatter and a user's global gate are two hooks rather than competing answers. Plan 44 phase 1 chose this knowing the cost: a cloned repository can execute its hooks on first launch, disclosed at startup, per-hook in `/hooks` (`[user]`/`[project]`) and per execution in the transcript, rather than gated behind a prompt. A faulty workspace config never fails startup, so no clone can deny service in its own directory. Plan 44 phase 1 removed the confirmation step: a declared hook runs, interactively and headless alike, because the only file that can arm one is outside every workspace and beyond the reach of the workspace-confined file tools. Disclosure replaces the prompt - startup names every armed hook, `/hooks` lists them, and every execution gets a transcript row. Arming names a PROGRAM and does not track that file's contents, which `/hooks` and the docs state outright. Two Step 0 corrections to the plan: the **managed tier is cut from v1** - `~/.mivia/managed.toml` is user- and agent-writable so it cannot be auto-trusted, and a replacement path is an install surface nothing in this product creates, so a non-interactive run executes zero hooks with no exception until a follow-up plan owns that story; and the re-entrancy guard is context-scoped rather than process-wide, which would have let one call's hook suppress a concurrent call's gate. Docs at `docs/development/lifecycle-hooks.md` (NOT `hooks.md`, which is Git hooks) |
