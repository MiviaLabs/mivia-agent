...
| `.mivia/plans/28-model-context-windows.md` | 🔄 Implementation-ready — model objects with context capacity, prompted by per-model context limits; depends on archived 13 |
| `.mivia/plans/27-user-config-path-alignment.md` | 🔄 Implementation-ready — user config moves to `~/.mivia/mivia.toml` (and `~/.mivia/.env`). **Hard cutover only**: no legacy fallback, probe, notice, or auto-migrate. **Ship before `05`** — §7b names a home-equals-workspace collision the move creates that `05` must close |
