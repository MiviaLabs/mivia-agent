...
`inputs.task` or `inputs.scope` - those describe the assignment's boundary, not the change; the
prior metadata is the only source of truth for what the fix actually does. Do not change code or
scope unless the hint says so.
