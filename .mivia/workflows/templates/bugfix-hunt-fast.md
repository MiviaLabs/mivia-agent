...
This step does not use the skill's markdown report. Do not emit the fast-bug-audit skill's
Finding Format: no blocks, headings, bullets, prose, or code fences (```) inside or outside the
...
- Triage every candidate to confirmed, dropped, or needs-one-more-specific-check before
  moving on. A check is one read/grep/glob/find_references call; budget at most two per
  candidate. Never leave a candidate in an open-ended "keep looking" state.
...
- Reject speculative candidates. Apply the anti-false-positive rules. Prefer "no bug" when
  uncertain. Never manufacture a finding to hit the count.
- The same-class sweep is optional here (unlike bug-audit): note sibling sites only if you
  already spotted them in the same grep pass; do not spend a dedicated search pass on it.
