...
| Workspace agent file `.agents/agents/<name>.md` `system_prompt` **in a user project** | Yes - that project's conventions |
| Workspace agent file `.agents/agents/<name>.md` `system_prompt` **in this repo** | Yes - *this* product's Go build/test knowledge |
...
- Go tests: `internal/cli/chat/prompt_generic_test.go` - fails if compiled-in default prompts are stack-biased.
...
3. Run `go test ./internal/tools/ ./internal/cli/chat/ -count=1`.
...
- Workspace-local agent definitions (`.agents/agents/*.md` `system_prompt`) may document this repo's `go test` / `make verify` workflow.
