...
`.mivia/mivia.toml.example`. Rationale: `docs/security/overview.md`.
...
- **`prompt` and `reasoning` are never redacted.** A user may add them to their
  own key list; nothing shipped will.
...
- Hook handlers parse structured input and reject malformed protected-action payloads.
...
- Disabling it logs a warning once at workflow-controller startup (`applyHarnessSandboxSetting` in `internal/cli/workflow/workflow_run_build.go`).
